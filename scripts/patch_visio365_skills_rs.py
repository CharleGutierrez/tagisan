#!/usr/bin/env python3
"""
scripts/patch_visio365_skills_rs.py

Patches src/ecc/skills.rs to:
1. Append all 350 `pub fn visio_*() -> EccSkill` constructors.
2. Register all 350 in `all_built_in_skills()`.
3. Add high-leverage alias lookups to `find_built_in_skill()`.
4. Add domain prefixes ("visio-", "visio-365-", "visio-365", "msvisio-", "ms-visio-") to `infer_domain()`.
"""
...
from scripts.generate_visio365_skills import get_all_skills

def main():
    skills = get_all_skills()
    print(f"Loaded {len(skills)} Visio 365 skills from generator.")
    ...
    calls_block = "\n        // Top 350 Skills for Microsoft Visio 365 & Vibe Code Developers\n" + "\n".join(calls) + "\n"
    
    target_all_skills = "        acc_deploy_decommissioning_and_data_archival(),\n    ]"
    if target_all_skills not in content:
        raise ValueError("Could not find insertion point 'acc_deploy_decommissioning_and_data_archival(),\\n    ]' in all_built_in_skills()")
    
    replacement_all_skills = f"        acc_deploy_decommissioning_and_data_archival(),{calls_block}    ]"
    ...
    alias_lines = ["    // Top 350 Skills for Microsoft Visio 365 & Vibe Code Developers Aliases"]
    for s in skills:
        target_skill = s["id"]
        triggers = s["triggers"]
        if triggers:
    ...
    domain_prefixes = """        ("visio-", "visio-365"),
        ("visio-365-", "visio-365"),
        ("visio-365", "visio-365"),
        ("msvisio-", "visio-365"),
        ("ms-visio-", "visio-365"),
"""
    ...
    # 4. Generate all 350 pub fn visio_*() definitions
    fn_defs = []
    for i, s in enumerate(skills, 1):
        ...
        # Determine safest raw string delimiter
        if 'r###"' in skill_content or '"###' in skill_content:
            r_open = 'r####"'
            r_close = '"####'
        elif 'r##"' in skill_content or '"##' in skill_content:
            r_open = 'r###"'
            r_close = '"###'
...
"""
    ...
    print("[+] Successfully patched src/ecc/skills.rs with all 350 Visio 365 skills!")
