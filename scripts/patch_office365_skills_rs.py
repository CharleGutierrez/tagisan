#!/usr/bin/env python3
"""
scripts/patch_office365_skills_rs.py

Patches src/ecc/skills.rs to:
1. Append all 350 `pub fn o365_*() -> EccSkill` constructors.
2. Register all 350 in `all_built_in_skills()`.
3. Add high-leverage alias lookups to `find_built_in_skill()`.
4. Add domain prefixes ("o365-", "office-365-", "office-365", "office-") to `infer_domain()`.
"""
...
def main():
    ...
    print(f"Loaded {len(skills)} Office 365 skills from generator.")
    ...
    calls_block = "\n        // Top 350 Skills for Microsoft Office 365 & Vibe Code Developers\n" + "\n".join(calls) + "\n"
    
    target_all_skills = "        pp_gov_capacity_storage_quota_allocation(),\n    ]"
    if target_all_skills not in content:
        raise ValueError("Could not find insertion point 'pp_gov_capacity_storage_quota_allocation(),\\n    ]' in all_built_in_skills()")
    
    replacement_all_skills = f"        pp_gov_capacity_storage_quota_allocation(),{calls_block}    ]"
    ...
    alias_lines = ["    // Top 350 Skills for Microsoft Office 365 & Vibe Code Developers Aliases"]
    for s in skills:
        target_skill = s["id"]
        triggers = s["triggers"]
        if triggers:
    ...
    domain_prefixes = """        ("o365-", "office-365"),
...
"""
    ...
    # 4. Generate all 350 pub fn o365_*() definitions
    fn_defs = []
...
"""
    ...
    print("[+] Successfully patched src/ecc/skills.rs with all 350 Office 365 skills!")
