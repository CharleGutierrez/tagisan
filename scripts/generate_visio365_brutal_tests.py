#!/usr/bin/env python3
"""
scripts/generate_visio365_brutal_tests.py

Generates tests/visio365_skills_brutal_tests.rs with exhaustive testing:
1. Discovery of all 350 Visio 365 skills on disk (.ecc/skills/visio-*)
2. Registration of all 350 skills in built-in registry (assert >= 2045 total skills)
3. Alias resolution across all 35 clusters
4. Invariants enforcement (ALWAYS, NEVER, MANDATORY)
5. Semantic intent dispatching across all 35 clusters
...
TARGET_TEST = TESTS_DIR / "visio365_skills_brutal_tests.rs"
...
def main():
    ...
    for s in skills:
        if s["cluster"] != current_cluster:
            ...
            skill_list_lines.append(f"    // Cluster {current_cluster}: {s['cluster_title']}")
    ...
    for c in range(1, 36):
        ...
        if c_skills:
            rep = c_skills[0]
            if rep["triggers"]:
                query_tests.append((rep["triggers"][0], rep["id"]))
    ...
    # Query list for multithreaded stress test
    stress_queries = []
    for q, _ in query_tests:
        stress_queries.append(f'        "{q}",')
    stress_queries_block = "\n".join(stress_queries)

    rust_code = f"""//! Brutal Integration & Verification Tests for Top 350 Skills in Microsoft Visio 365 & Vibe Code Development in Tagisan (tgs)
