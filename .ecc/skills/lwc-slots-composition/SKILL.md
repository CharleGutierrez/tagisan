...
description: "Use when a Lightning Web Component needs to let a parent inject markup into predefined\
  \ regions using `<slot>` \u2014 default and named slots, `slotchange` wiring, fallback\
  \ content, and detecting slot emptiness. NOT for rendering a dynamic component whose\
  \ tag name is chosen at runtime (that is `lwc:is`) \u2014 use lwc/lwc-dynamic-components.\
  \ NOT for third-party custom-element interop \u2014 use lwc/lwc-web-components-interop."
---
