---
name: linux-prctl-no-new-privs
description: "Syscall privilege elevation prevention for child processes spawned by agents"
version: 1.0.0
...
triggers:
  ...
  - "kernel"
  - "linux"
  - "linux prctl no new privs"
  - "linux-prctl-no-new-privs"
  - "linux-prctl-no-new-privs execute"
  - "linux-prctl-no-new-privs verify"
  - "low-level"
  - "new"
---

# Linux Prctl No New Privs Skill

The `linux-prctl-no-new-privs` skill equips Tagisan (`tgs`) autonomous agents and developers with authoritative capabilities sourced from **Linux Kernel Docs (kernel.org)** (Tier 3: Low-Level Systems, Linux Kernel & Hardware Acceleration). It integrates directly into `AgentShield / Subprocess`.
...
- **Subsystem Binding**: `AgentShield / Subprocess`
...
```json
{
  "skill": "linux-prctl-no-new-privs",
  "tier": "Tier 3: Low-Level Systems, Linux Kernel & Hardware Acceleration",
  "source": "Linux Kernel Docs (kernel.org)",
  "integration_target": "AgentShield / Subprocess",
