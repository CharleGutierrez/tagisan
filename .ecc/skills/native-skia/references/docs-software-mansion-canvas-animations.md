...
For full API details, consult the
[official documentation](https://shopify.github.io/react-native-skia) and the
target repo's installed package files.

**Version requirements:** Expo SDK 56 bundles
`@shopify/react-native-skia@2.6.2` with React Native 0.85 and React 19.2.
Upstream latest checked on 2026-06-04 was `2.6.4`; do not override Expo's
bundled version in Expo Go-compatible apps without native rebuild proof.
Current docs require `react-native@>=0.79` and `react@>=19`. For
`react-native@<=0.78` and React 18 stacks, use `@shopify/react-native-skia`
`1.12.4` or below.

Install through the repo's package policy. With Expo, use Expo-compatible
installation so the package resolves to the SDK-bundled native module.
...
Skia components accept Reanimated shared values and derived values as props
directly. There is no need for `createAnimatedComponent` or
`useAnimatedProps`.
...
Use `useCanvasSize()` only when JS-thread layout state is needed. Keep layout
and domain state outside the canvas because route changes, conditional
rendering, and app backgrounding can remount the native surface and recreate GPU
resources.
...
Issue drawing commands directly to a canvas on every frame. Use when the number
of drawing commands changes dynamically (the number of elements is itself
animated). Avoid allocating paints, paths, shaders, or large arrays inside the
frame path unless profiling proves it is acceptable.
...
Skia provides hooks for efficient path animation on the UI thread. For full API
details, consult the
[hooks documentation](https://shopify.github.io/react-native-skia/docs/animations/hooks).
...
Skia provides a shading language (SKSL) similar to GLSL for per-pixel effects.
Compile shaders with `Skia.RuntimeEffect.Make` and apply them as children of
drawing elements or as image filters.

For full SKSL syntax, consult the
[shading language docs](https://shopify.github.io/react-native-skia/docs/shaders/overview).
...
For full API, consult the
[textures docs](https://shopify.github.io/react-native-skia/docs/animations/textures).
...
- Load images and textures through supported Skia APIs before animation starts;
  do not fetch, decode, or allocate asset bytes inside per-frame worklets.
