...
description: "Use when designing or configuring public pages on an Experience Cloud site \u2014\
  \ guest user profile setup, page-level access settings in Experience Builder, object/field\
  \ visibility for unauthenticated visitors, and explicit sharing rules that expose\
  \ data on public pages. Triggers: 'configure public pages Experience Cloud', 'guest\
  \ user profile setup', 'unauthenticated site access', 'public community page visibility',\
  \ 'make page visible without login'. NOT for authenticated user features (use admin/experience-cloud-member-management).\
  \ NOT for security hardening or remediation of guest user exposure (use security/guest-user-security)."
---
