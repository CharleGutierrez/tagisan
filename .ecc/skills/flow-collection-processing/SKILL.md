...
description: "Use when building or reviewing Flow logic that processes lists of records using Loop,\
  \ Assignment, Collection Filter, Collection Sort, or Transform elements. Triggers:\
  \ 'iterate over collection in flow', 'flow loop add to collection', 'collection\
  \ filter element', 'transform element flow', 'update records from collection variable',\
  \ 'collection sort flow'. NOT for pulling DML or SOQL out of a Loop element \u2014\
  \ use flow/flow-loop-element-patterns. NOT for transaction-budget and bulkification\
  \ analysis \u2014 use flow/flow-bulkification."
---
