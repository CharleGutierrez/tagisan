...
description: "Use when Apex code must create or update metadata programmatically at runtime \u2014\
  \ custom fields, objects, picklist values, labels, or other supported types \u2014\
  \ using the Metadata namespace (Metadata.Operations, Metadata.DeployCallback). Triggers:\
  \ 'Metadata.Operations', 'Metadata.CustomField from Apex', 'deploy metadata from\
  \ Apex', 'enqueueDeployment', 'create a custom field programmatically', 'post-install\
  \ script metadata setup'. NOT for zip-based Metadata API REST/SOAP deploys \u2014\
  \ use apex/metadata-api-and-package-xml. NOT for reading Custom Metadata Type configuration\
  \ rows in Apex \u2014 use apex/custom-metadata-in-apex. NOT for compiling a class\
  \ or querying ApexClass / ApexCodeCoverage from outside the org \u2014 use apex/tooling-api-patterns."
---
# Apex Metadata API
