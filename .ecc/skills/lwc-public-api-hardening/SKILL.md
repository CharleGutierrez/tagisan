...
description: "Use when an LWC exposes `@api` properties, `@api` methods, or design attributes in\
  \ `<targetConfig>` and you need a defensive, predictable public contract. Covers\
  \ runtime type coercion (the `@api` decorator does NOT validate types), required-prop\
  \ checks, and design attributes. NOT for component-to-component messaging (use lwc/component-communication)\
  \ or reactive state \u2014 use lwc/lwc-reactive-state-patterns."
---
