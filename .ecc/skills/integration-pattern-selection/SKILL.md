...
description: "Use this skill to select the right Salesforce integration pattern \u2014 from point-to-point\
  \ to event-driven to hub-and-spoke \u2014 by applying the official Salesforce two-axis\
  \ decision framework (integration type \xD7 timing) to a business integration scenario.\
  \ Trigger keywords: integration pattern decision, choose integration approach, Salesforce\
  \ integration architecture, when to use platform events vs API, integration type\
  \ selection. NOT for implementation of any specific integration pattern (use domain-specific\
  \ integration skills), MuleSoft architecture (use architect/mulesoft-anypoint-architecture),\
