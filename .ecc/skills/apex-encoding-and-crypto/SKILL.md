...
description: "Use when Apex must sign, verify, encrypt, hash, encode, or decode payloads \u2014\
  \ including HMAC for webhook signatures, RSA/ECDSA signing for JWT bearer flows,\
  \ AES for stored secrets, base64/hex/URL encoding, and digest comparisons for integration\
  \ integrity. Triggers: 'Crypto.sign', 'Crypto.generateMac', 'EncodingUtil.base64Encode',\
  \ 'JWT signing in Apex', 'verify webhook signature'. NOT for where the signing key\
  \ or API secret is stored \u2014 use apex/apex-secrets-and-protected-cmdt. NOT for\
  \ setting up Named Credentials or OAuth flows end-to-end \u2014 use apex/apex-named-credentials-patterns.\
