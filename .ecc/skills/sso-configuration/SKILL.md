...
description: 'Use when configuring single sign-on into or out of a Salesforce org. Trigger keywords:
  SAML SSO setup, configure single sign-on, identity provider metadata, Just-in-Time
  provisioning, My Domain SSO, OpenID Connect, SSO-only login, Salesforce as identity
  provider. NOT for SSO failures - use security/sso-saml-troubleshooting. NOT for
  OAuth integration access - use admin/connected-apps-and-auth.'
---
# SSO Configuration
