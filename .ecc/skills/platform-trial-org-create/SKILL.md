...
description: 'Use this skill to create a Salesforce trial, developer, or Trialforce org against
  an already-authenticated host org, the same way a developer/Trialforce web signup
  form provisions one. INVOKE when the user asks to: create a trial org, sign up a
  new trial or developer org, provision a Trialforce org from a template, or check
  the status of a trial-org signup they started. Trigger phrases: ''create a trial
  org'', ''sign up a trial org'', ''spin up a dev org'', ''Trialforce signup'', ''provision
  trial from template'', ''check my trial org signup status''. Do NOT use for scratch
  orgs (use dx-org-manage) or for checking trial expiration dates of existing orgs
  (use dx-org-trial-expiration-check).'
---
