---
name: improve-ui
description: 'Read-only design-system drift audit: compare an implementation against its own documented
  design contract, prove each deviation, and plan verified refinements without replacing
  product identity. Scope is system consistency only -- it deliberately excludes accessibility,
  semantics, and copy, so it is not a general UI review. Use when asking whether a
  surface has drifted from the design system. For a holistic cross-discipline interface
