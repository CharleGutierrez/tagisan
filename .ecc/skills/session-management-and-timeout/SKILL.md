...
description: "Use this skill when configuring session timeout values, concurrent session limits,\
  \ session IP locking, or logout behavior in Salesforce. Covers org-wide session\
  \ settings, profile-level overrides, Connected App session. NOT for OAuth token\
  \ refresh flows, login IP ranges, or MFA/identity-provider configuration \u2014\
  \ use security/oauth-token-management."
---
