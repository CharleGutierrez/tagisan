...
description: "Use to bind agent users into a Salesforce Queue via GroupMember Data API POSTs, with\
  \ SOQL-based idempotency (safe re-run \u2014 only inserts missing bindings). Binds\
  \ either the generated demo agents (agent{1..N}.<suffix>@example.com) or an explicit\
  \ list of real agents passed as usernames/User Ids. Triggers: add agents to a Case/Voice\
  \ queue, assign agent users to a queue, populate queue membership for Omni routing.\
  \ Do not use to create the queue (service-omni-queue-deploy), to create the agent\
  \ users (service-omni-agent-users-create), or to assign permission sets (service-omni-permission-set-assign)."
---
