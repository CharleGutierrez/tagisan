...
description: "Use this skill when configuring or troubleshooting the Salesforce CPQ Guided Selling\
  \ wizard: Quote Process records, ProcessInput questions that filter products by\
  \ field value, mapping answers to Product2 classification fields, and search type\
  \ choice (Standard, Enhanced, Custom). Trigger keywords: CPQ guided selling, quote\
  \ process wizard, product selection wizard, SBQQ__QuoteProcess__c, SBQQ__ProcessInput__c,\
  \ guided product selection, auto select product, product search plugin. NOT for\
  \ bundles, product options, or product rules \u2014 use admin/cpq-product-catalog-setup.\
  \ NOT for writing a custom ProductSearchPlugin in Apex \u2014 use apex/cpq-apex-plugins."
---
# CPQ Guided Selling
