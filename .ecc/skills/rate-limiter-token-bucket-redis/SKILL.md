---
name: rate-limiter-token-bucket-redis
description: "Distributed token bucket rate limiter tracking requests across all swarm workers"
version: 1.0.0
...
triggers:
  ...
  - "rate-limiter-token-bucket-redis"
  - "rate-limiter-token-bucket-redis execute"
  - "rate-limiter-token-bucket-redis verify"
---

# Rate Limiter Token Bucket Redis Skill

The `rate-limiter-token-bucket-redis` skill equips Tagisan (`tgs`) autonomous agents and developers with authoritative capabilities sourced from **Distributed Rate Limiting Specs** (Tier 9: Distributed Systems, Telemetry & SRE Resiliency). It integrates directly into `Provider Hub`.
...
```json
{
  "skill": "rate-limiter-token-bucket-redis",
  "tier": "Tier 9: Distributed Systems, Telemetry & SRE Resiliency",
  "source": "Distributed Rate Limiting Specs",
