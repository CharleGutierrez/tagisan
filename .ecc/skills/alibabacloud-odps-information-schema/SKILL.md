...
description: "Query MaxCompute (ODPS) Information Schema metadata views. Tenant-level (SYSTEM_CATALOG.INFORMATION_SCHEMA.*,\
  \ recommended) or project-level (Information_Schema.*, deprecated). NL\u2192SQL\
  \ for IS views: tables, columns, partitions, tasks_history, tunnels_history, table_privileges,\
  \ users, user_roles, quota_usage, etc. NOT for: DDL/DML, listing tables via MCP,\
  \ running ad-hoc SQL, general MaxCompute questions."
---
