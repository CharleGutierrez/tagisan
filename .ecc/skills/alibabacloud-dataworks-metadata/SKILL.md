...
description: "DataWorks metadata Skill for Alibaba Cloud \u2014 browse Data Map metadata and perform\
  \ non-destructive writes via Aliyun CLI. READ scope: list/get catalogs, databases,\
  \ tables, columns, partitions; query data lineage (upstream/downstream impact);\
  \ list/get datasets & versions; list/get metadata collections (Category/Album) and\
  \ entities inside them; preview dataset version content. WRITE scope (non-destructive\
  \ only): update table & column business metadata; register lineage relationships;\
  \ create/update datasets and versions; create/update metadata collections and add\
  \ entities to them. This Skill exposes NO delete or remove APIs \u2014 every `delete-*`\
  \ and `remove-*` operation is intentionally out of scope. For deletions, use the\
  \ DataWorks console. Triggers: \"dataworks metadata\", \"data map\", \"data lineage\"\
  , \"meta collection\", \"dataset\", \"catalog\", \"table info\", \"column info\"\
  , \"partition\", \"impact analysis\", \"register lineage\", \"create dataset\",\
