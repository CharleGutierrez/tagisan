...
description: "Configure Einstein Activity Capture (EAC) for an org: enable the feature, create\
  \ Configuration profiles, set auth type (User-Level or Org-Level OAuth), define\
  \ email and calendar sync scope, excluded domains and privacy settings, assign users,\
  \ verify sync health. NOT for querying synced emails or events in SOQL, Apex or\
  \ reports \u2014 use apex/einstein-activity-capture-api. NOT for Opportunity Scoring\
