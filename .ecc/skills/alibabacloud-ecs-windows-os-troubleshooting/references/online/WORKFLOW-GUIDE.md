...
- **Direct execution channel** (PowerShell command execution on local instance) → PowerShell Collection Script Rules section below, §12 Command Transmission Mode (Direct Execution Channel)
- **Remote execution channel** (PowerShell command execution via `aliyun ecs run-command` on remote instance) → PowerShell Collection Script Rules section below, §13 Remote Execution Channel Rules + [REMOTE-EXECUTION.md](references/REMOTE-EXECUTION.md)
...
- **Channel-layer failure** (Cloud Assistant unreachable, network error, authorization failure, instance not found, Cloud Assistant agent not running) → Enter fallback: degrade to next level
...
3. **Dynamic planning** (only when classification does not match any problem domain, or the classified sequence completes without finding a root cause):
...
4. Output: Ordered list of problem domain files (typically 2-5, avoid too many) + planning rationale
...
7. **Platform-side cross-validation (remote execution channel only)**: When the remote execution channel is active, load [platform-metrics.md](references/online/platform-metrics.md) and, for the matched problem domain group only, fetch platform-side data (instance/disk monitoring metrics, system events, console screenshot) as auxiliary evidence. Platform data corroborates or bounds in-instance findings but never replaces intra-instance collection and never changes domain classification; label the evidence source (platform-side vs in-instance) in the Evidence Review. In direct execution channel mode, skip this rule
...
3. Rank by relevance to the user's problem: **relation (Direct > Contributing > Unrelated) × severity (Critical > Warning > Info) dual-dimension sorting**; Unrelated issues are listed separately as "other findings", not mixed into the main fix plan
...
5. **Causal edge evidence triple (mandatory)**: Every edge in the causal chain ("A caused B") must carry all three of: ① the collection command plus the output excerpt it produced, ② the documented judgment it matched (the "Abnormal: X → Root cause: Y" pattern in the problem domain file), and ③ a time-window check — the timestamps of the cause-side evidence must precede or overlap the symptom's occurrence window (event log times, service start time, config last-write time, etc.). An edge missing any of the three MUST be downgraded to a speculative hypothesis in the Evidence Review and never presented as a conclusion
...
- Once "Evidence Review" is completed, **MUST directly enter fix plan output within the same response**, MUST NOT stop after only describing the root cause in text
- Delayed behaviors such as "let me confirm first... then generate the script" or "wait for the user to ask before providing the plan" are not allowed
- Fix scripts MUST be **directly copyable and executable complete PowerShell one-click scripts**

**Fix reference loading contract (mandatory)**:
- Before outputting a fix plan, MUST first load the fix reference for the problem domain where the root cause is located: when the problem domain file's "Fix recommendations" section is a single-line pointer, MUST load the corresponding fix file under `references/online/fixes/`; when it is an inline fix section, directly output based on that section's content
...
8. **Fix impact standard fields (required)**: Each fix plan's risk notes section MUST include the following three items:
...
> Instance is "Running" but GuestOS did not boot properly (`GuestOS.SystemBootstrapFailed`) belongs to offline diagnostics, see [WORKFLOW-GUIDE.md](references/offline/WORKFLOW-GUIDE.md).
>
> **Why "Running" and "cannot boot" are not contradictory**: console instance status reflects the virtualization-layer lifecycle, not the operating system. "Running" indicates the platform's control and virtualization layers have finished preparing hardware resources — equivalent to power-on; it only means the instance has at least *begun attempting* to load and start the OS. Underlying hardware or software faults, system misconfiguration, or file corruption can all stop the GuestOS from booting while the platform status remains Running, so users typically report "cannot boot / cannot connect / business down" rather than a status anomaly — and the two observations are fully consistent. In this situation the console VNC output is the direct evidence of where boot stopped (boot phase anomaly vs OS loading phase anomaly), and the case routes to offline diagnostics. Conversely, an instance remaining in "Starting" for an abnormally long time indicates a platform-side startup anomaly (non-GuestOS primary cause), handled per the row above.
...
> Crash / Hang scenarios on the online side only do historical event query and information collection, with no real-time analysis capability; NMI core on-site collection when instance is currently still Frozen (user manual operation) and in-depth diagnosis of inability to boot or repeated BSOD after crash / Hang (offline system disk mount analysis) belong to offline scenarios, see [WORKFLOW-GUIDE.md](references/offline/WORKFLOW-GUIDE.md) "Crash / Hang Offline Branch".
...
> BitLocker offline branch (offline disk partition unreadable / recovery mode popup after spec change) see [WORKFLOW-GUIDE.md](references/offline/WORKFLOW-GUIDE.md).
...
|---|---|---|
| 1 | `Get-WmiObject`, `net user`, `net localgroup`, `netstat`, `ipconfig`, `tasklist`, `sc query` | Replace with native cmdlets from the §1 mapping table; only keep and pass through raw output for those marked "no direct replacement" |
| 2 | Any pipeline outputting to the console | Confirm `Select-Object` is used to limit key fields; add per §2 if not limited |
| 3 | Pipeline from `Select-Object` / `ForEach-Object { [PSCustomObject]@{...} }` | Append `| Format-Table` or `| Format-List` at the end of the pipeline, see §2 |
| 4 | `Get-ItemProperty` | Confirm `PSPath` and other metadata fields are filtered, see §2 |
| 5 | Custom variable names | Compare against built-in keywords / automatic variables list in §3; rename if colliding |
| 6 | `slmgr`, `winver` | Change to `cscript //Nologo ...vbs` form, see §4 |
| 7 | `Get-WindowsFeature`, `Invoke-WebRequest`, `Invoke-RestMethod`, `Expand-Archive`, `Start-BitsTransfer`, `Copy-Item -ToSession/-FromSession` | Prepend `$ProgressPreference = 'SilentlyContinue'`, see §5 |
| 8 | `$variable:` in double-quoted strings | Rewrite as `${variable}:`, see §6 |
| 9 | Pipe `|` immediately after the closing brace of a `foreach (` statement | Use `ForEach-Object` instead, see §7 |
| 10 | exe calls like `reg`, `bcdedit`, `diskpart`, `dism`, `netsh` | Confirm using `$LASTEXITCODE` or stderr for error detection instead of `try/catch`, see §8 |
| 11 | `Substring(`, `CurrentVersion`, `ProductName` | Change to version number comparison or SKU branching, and null-check `CurrentVersion`, see §9 |
| 12 | `Set-Content`, `[System.IO.File]::WriteAllText` | Confirm the same file does not mix both methods, see §10 |
| 13 | Full command text intended for user manual execution | Pass through §11 display gating before display (pure ASCII + placeholder compliance + no local path dependency + can be copied and executed as a whole); fix any non-compliant item before display |
| 14 | The command payload itself about to be executed | **Local mode**: Multi-line scripts go through Base64 channel; inline single-line commands use single quotes, see §12. **Remote mode**: Deliver via `aliyun ecs run-command` — plaintext by default (omit `--content-encoding`); Base64 with `--content-encoding Base64` only when quoting is impractical, see §13 |
| 15 | `-ErrorAction SilentlyContinue`, `2>$null`, `\| Out-Null`, `2>/dev/null` | Applies to BOTH Local and Remote mode payloads. Suppression is allowed only for state-table collection where absence of output is itself the finding; otherwise keep stderr, error text, and exit codes in the returned output (`$LASTEXITCODE` locally, `ExitCode` + `Output` remotely) and analyze them as potential root cause, see [REMOTE-EXECUTION.md](references/REMOTE-EXECUTION.md) (Output Size Management → error preservation rule). The skill's bundled scripts (`scripts/*.ps1`) are stricter: zero `-ErrorAction SilentlyContinue`, every step guarded by §8 try/catch section guards |
...
- `--type RunPowerShellScript` is required for Windows instances; flag spellings are kebab-case per [REMOTE-EXECUTION.md](references/REMOTE-EXECUTION.md) §CLI Flag Reference
...
Fix scripts follow the §13.3 loop plus: MUST be presented to the user for confirmation BEFORE execution (per SKILL.md Principle 6: "Fix requires confirmation"); use a longer timeout (180-300 seconds); send a verification command afterwards through the same loop to confirm the fix took effect.
...
Windows Server images ship with PowerShell 5.1 by default; when passing arguments to native executables, embedded **double quotes are stripped** and never reach the target CLI. Symptom: a native CLI call (`reg`, `netsh`, `bcdedit`, `dism`, `diskpart`, ...) fails with parameter-parsing errors — "The syntax of the command is incorrect", values split at spaces, flags reported as missing — even though the command text looks correct. This is a **script-layer error**, not a target-system fault: do NOT reclassify it as target abnormality and do NOT switch channels. Fix: rewrite the script so it contains **single-quoted strings only**, then retry on the **same channel** (per §13.5 failure classification). When a value genuinely needs quoting for the target CLI, prefer restructure-into-single-quotes over embedded double quotes.
...
```powershell
# ✗ Prohibited: two network round-trips
...
# ✓ Correct: single query + variable reuse
$allDevices   = Get-CimInstance Win32_PnPEntity
