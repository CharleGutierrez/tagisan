...
| Any ping / connectivity failure | FIRST read the per-reply error text from Step 6 raw output (never summarize it away to "100% loss"): "General failure / 一般故障" → quick Step 1-3 sanity glance → [networking-firewall.md Step 7](references/online/networking-firewall.md) immediately (highest priority); "Request timed out / 请求超时" → Step 4 → firewall Step 7; "Destination host unreachable / 无法访问目标主机" → Step 4; "Transmission failed / 传输失败" → Step 3/4 |
...
- PowerShell script: [networking-tcpip.ps1](references/online/scripts/networking-tcpip.ps1) §Step 1
...
- PowerShell script: [networking-tcpip.ps1](references/online/scripts/networking-tcpip.ps1) §Step 2
...
- PowerShell script: [networking-tcpip.ps1](references/online/scripts/networking-tcpip.ps1) §Step 3
...
- PowerShell script: [networking-tcpip.ps1](references/online/scripts/networking-tcpip.ps1) §Step 4 (includes additional collection)
...
- PowerShell script: [networking-tcpip.ps1](references/online/scripts/networking-tcpip.ps1) §Step 5
...
- PowerShell script: [networking-tcpip.ps1](references/online/scripts/networking-tcpip.ps1) §Step 6
...
1. Read the per-reply error text in the raw ping output FIRST — it is the primary triage signal; the "100% loss" summary line alone is NOT sufficient for any conclusion:
   ...
   | Error text (EN / ZH-CN) | Meaning | Direction |
   |---|---|---|
   | General failure / 一般故障 | Packet rejected by local stack or filtering layer before leaving the host | Quick Step 1-3 sanity glance → [networking-firewall.md Step 7](references/online/networking-firewall.md) (WFP / local filtering) immediately |
   | Request timed out / 请求超时 | Packet left the host, no reply returned | Step 4 (route) → firewall Step 7, then path/peer/platform direction |
   | Destination host unreachable / 无法访问目标主机 | No route or ARP resolution failed | → Step 4 (route/gateway) |
   | Transmission failed / 传输失败 | Source address or interface problem (typical of `ping -S` with wrong source) | → Step 3/4 |
...
- PowerShell script: [networking-tcpip.ps1](references/online/scripts/networking-tcpip.ps1) §Step 7
...
- PowerShell script: [networking-tcpip.ps1](references/online/scripts/networking-tcpip.ps1) §Step 8
...
- PowerShell script: [networking-tcpip.ps1](references/online/scripts/networking-tcpip.ps1) §Step 9
...
- PowerShell script: [networking-tcpip.ps1](references/online/scripts/networking-tcpip.ps1) §Step 10
...
| Mandatory jump | Ping reports "General Failure / 一般故障" (any localized equivalent) | → [networking-firewall.md Step 7](references/online/networking-firewall.md) (WFP packet drop localization) |
