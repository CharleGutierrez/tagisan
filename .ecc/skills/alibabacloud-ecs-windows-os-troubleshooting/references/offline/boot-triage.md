...
1. **WinRE Recovery Interface**: "Choose an option" / "选择一个选项" etc.
...
5. Identify digital signature verification failure:
   - Characteristics: `无法验证此文件的数字签名` / `windows cannot verify the digital signature for this file` + `\Windows\System32\Drivers\`
...
7. Identify Recovery Environment (WinRE):
   - Characteristics: `Choose an option` / `选择一个选项` / `疑难解答` / `Recovery` / `选择键盘布局` / `Choose your keyboard layout` / `Windows Error Recovery`
...
8. Identify Sysprep interruption:
   - Characteristics: `计算机意外地重新启动或遇到错误` / `Windows安装无法继续` / `Windows installation cannot proceed`
...
11. Identify black screen:
    - Blinking cursor → OS partially loaded but Shell not started → [system-config.md](references/offline/system-config.md)
    - No output at all → Boot phase failed → [bcd-boot.md](references/offline/bcd-boot.md)
