...
description: "Query DTS (Data Transmission Service) task status and details across all Alibaba\
  \ Cloud regions. **v12.1: Enhanced reliability** - Timeout increased to 10s, exponential\
  \ backoff (0.2s, 0.4s) for better timeout handling. Parallel execution remains **6-8x\
  \ faster** than v10 (39s \u2192 6s with --workers 16). **API retry logic ensures\
  \ consistent results (no count variations)**. Supports filtering by instance ID\
  \ or job name. Automatically polls all 27 regions and 3 job types. Strictly filters\
  \ for PrePaid/PostPaid tasks and outputs a full Chinese report with Region information.\
  \ Tasks are grouped by type (Migration/Sync/Subscribe) and sorted by CreateTime\
  \ within each group. **Use this skill when: checking DTS task status, finding migration/sync\
  \ tasks, verifying task counts, or filtering tasks by instance ID or job name.**"
---
