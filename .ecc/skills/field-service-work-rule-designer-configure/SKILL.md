...
description: "Designs the work rules for a Salesforce Field Service scheduling policy \u2014 the\
  \ hard filters that determine which resources and time slots are eligible for an\
  \ appointment. Step 2 of the three-part scheduling policy design flow; receives\
  \ the policy block from sfs-scheduling-policy-designer and delegates to sfs-service-objective-designer\
  \ when complete. Use this skill when a user wants to design or configure the work\
  \ rules (hard eligibility filters) for a Field Service scheduling policy."
---
