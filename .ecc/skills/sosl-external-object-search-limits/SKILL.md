...
description: "Use when a SOSL search (or global search) against a Salesforce Connect external object\
  \ misbehaves: it returns no records, rejects an operator/function/clause, or silently\
  \ drops matches. Covers the query-level rules that apply only to external objects\
  \ \u2014 searchable field types (text only), the mandatory RETURNING clause, the\
  \ 100-character search-string cap, unsupported operators (INCLUDES / LIKE / EXCLUDES),\
  \ unsupported functions (toLabel / convertCurrency), unsupported clauses (UPDATE\
  \ TRACKING / UPDATE VIEWSTAT / WITH DATA CATEGORY / WITH), and adapter-scoped limits\
  \ (OData logical operators in FIND; custom-adapter convertCurrency and WITH). NOT\
  \ for enabling search in Setup (use admin/global-search-configuration), NOT for\
  \ SOSL against standard/custom sObjects (use data/sosl-search-patterns), NOT for\
  \ general Salesforce Connect / external-object setup (use integration/salesforce-connect-external-objects),\
  \ and NOT for SOQL of external objects."
---
