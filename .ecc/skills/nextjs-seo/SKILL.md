---
name: nextjs-seo
description: "Next.js App Router SEO optimization and auditing. Use when implementing or fixing\
  \ SEO in a Next.js app \u2014 metadata and generateMetadata, viewport/themeColor,\
  \ Open Graph and og/twitter images (file conventions + ImageResponse), web app manifest,\
  \ favicons/icons, sitemap.xml, robots.txt, canonical URLs, hreflang/i18n alternates,\
  \ JSON-LD structured data and rich results, Core Web Vitals (LCP/INP/CLS), AI search/GEO\
  \ and AI crawler rules (GPTBot, OAI-SearchBot), or diagnosing Google indexing problems\
  \ (Search Console, \"Discovered/Crawled - currently not indexed\"). Also use to\
  \ run an SEO audit checklist. Not for general Next.js feature work unrelated to\
  \ SEO."
---
