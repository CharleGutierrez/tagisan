...
description: "Structure and evaluate a consequential Salesforce choice before the team commits:\
  \ frame the decision, separate hard constraints from preferences, compare viable\
  \ options with evidence, test score sensitivity, and recommend a reversible next\
  \ step. Trigger keywords: compare Salesforce options, evaluate alternatives, make\
  \ a platform decision, decision matrix, weighted decision. NOT for documenting an\
  \ already accepted choice \u2014 use architect/architecture-decision-records. NOT\
  \ for a narrow technology branch already covered by a canonical decision tree \u2014\
  \ apply that tree first."
---
