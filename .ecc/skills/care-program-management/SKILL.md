...
description: 'Configure or troubleshoot Health Cloud Care Programs: CareProgram hierarchy, patient
  enrollment (CareProgramEnrollee), consent prerequisites, program products (CareProgramEnrolleeProduct),
  provider associations (CareProgramProvider). Trigger keywords: care program, program
  enrollment, enrollee, program product, consent for enrollment, patient program outcome.
  NOT for care plan templates, problems and goals - use admin/care-plan-configuration.
  NOT for patient records, person accounts and care team roles - use admin/health-cloud-patient-setup.'
---
