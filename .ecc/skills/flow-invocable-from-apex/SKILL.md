...
description: "Author @InvocableMethod Apex classes that Flow can call as Actions. Design the input\
  \ / output variable contract, bulk semantics (one list in, one list out), null handling,\
  \ and error surfacing. Also covers the inverse direction: calling a flow from Apex\
  \ via Flow.Interview. NOT for wiring the action into Flow Builder and mapping flow\
  \ variables \u2014 use flow/flow-action-framework. NOT for exposing the same Apex\
