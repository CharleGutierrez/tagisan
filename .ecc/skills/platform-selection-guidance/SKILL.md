...
description: "Use when choosing which Salesforce platform capability to use for a requirement \u2014\
  \ covering metadata storage (Custom Metadata vs Custom Settings vs Custom Objects),\
  \ UI framework (LWC vs Aura), integration approach (Platform Events vs Change Data\
  \ Capture vs Outbound Messaging), and data extension (OmniStudio vs standard automation).\
  \ Triggers: custom metadata vs custom settings, LWC vs Aura, which salesforce feature\
  \ to use, platform events vs change data capture, omnistudio vs flow. NOT for automation-specific\
  \ tool selection between Flow / Apex / Workflow (use admin/process-automation-selection)\
  \ or for implementing the chosen platform feature."
---
