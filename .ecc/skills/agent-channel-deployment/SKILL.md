...
description: "Use when deploying an Agentforce agent to a channel surface: Embedded Service web\
  \ chat, Slack, Experience Cloud, Agent REST API, or Salesforce Mobile. Triggers:\
  \ 'deploy agent to website', 'add Agentforce to Slack', 'enable agent for mobile\
  \ app', 'configure embedded service for agent', 'agent API for custom integration',\
  \ 'multi-channel agent deployment'. NOT for agent logic design \u2014 use agentforce/agent-topic-design\
  \ or agentforce/agent-actions for topics, instructions, and action contracts."
---
