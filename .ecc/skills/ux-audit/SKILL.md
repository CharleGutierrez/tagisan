---
name: ux-audit
description: "Walk through a live web app AS a real user to find usability + behavioural bugs that\
  \ static reviews miss. REQUIRES proof of interaction (typing, clicking, sending,\
  \ observing) before any verdict \u2014 a sweep that didn't interact terminates with\
  \ verdict 'Incomplete'. Walks threads, exercises every element, runs the multi-pane\
  \ stress matrix, visual polish sweep, component perfection checklist, automated\
  \ a11y (axe-core), pragmatic performance budget (LCP/CLS/INP), scenario battery\
  \ (11 scenarios), and stress recipes including the real-flavour data battery. Hard\
  \ gates: console errors/warnings = 0, network 5xx = 0, layout collapse = 0, axe\
  \ Critical/Serious = 0, perf budget green. Audit-the-audit meta-check rejects rushed\
  \ reports. Each finding has reproduction steps, evidence path, and suspected code\
  \ location. Trigger with 'ux audit', 'walkthrough', 'qa sweep', 'audit the app',\
  \ 'dogfood this', 'check all pages', 'find what's broken', 'stress the UI'."
---
# UX Audit
