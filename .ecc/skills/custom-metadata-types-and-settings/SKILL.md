...
description: "Use when choosing between Custom Metadata Types and Custom Settings, understanding\
  \ hierarchical vs list settings, deployment behavior, governor limit implications,\
  \ or accessing either from Apex and Flow. Trigger keywords: 'custom metadata vs\
  \ custom settings', 'hierarchical settings per user profile', 'deployable config\
  \ vs runtime settings', 'getValues getInstance in apex', 'flow get records custom\
  \ settings'. NOT for modelling the CMT itself or protecting packaged defaults \u2014\
  \ use admin/custom-metadata-types. NOT for records users edit as business data \u2014\
