...
description: "Use this skill when users need to create, generate, or validate a Salesforce global\
  \ value set or customize a standard value set. Trigger when users mention a global\
  \ value set, GlobalValueSet, standard value set, StandardValueSet, a reusable picklist,\
  \ a picklist value set shared across fields, or customizing standard picklists like\
  \ Industry, Lead Source, or Opportunity Stage. Also use when users hit deployment\
  \ errors adding values to a standard picklist, referencing a value set from a custom\
  \ field, or working with .globalValueSet-meta.xml or .standardValueSet-meta.xml\
  \ files. DO NOT TRIGGER for an inline one-off picklist on a single field with no\
  \ reuse, or for general custom field metadata work that does not involve a GlobalValueSet\
  \ or StandardValueSet \u2014 use platform-custom-field-generate instead."
---
## Overview
