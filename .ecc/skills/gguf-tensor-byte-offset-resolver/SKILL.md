---
name: gguf-tensor-byte-offset-resolver
description: "Direct physical byte slice calculation for tensor weight chunks without buffer copies"
version: 1.0.0
tier: "Tier 5: Local Tensor Engines, Quantization & GGUF Acceleration"
source: "GGML/GGUF Spec"
tags:
  - ai
  - byte
  - cuda
...
triggers:
  ...
  - "ggml/gguf"
  - "gguf"
  - "gguf tensor byte offset resolver"
  - "gguf-tensor-byte-offset-resolver"
  - "gguf-tensor-byte-offset-resolver execute"
  - "gguf-tensor-byte-offset-resolver verify"
---

# Gguf Tensor Byte Offset Resolver Skill

The `gguf-tensor-byte-offset-resolver` skill equips Tagisan (`tgs`) autonomous agents and developers with authoritative capabilities sourced from **GGML/GGUF Spec** (Tier 5: Local Tensor Engines, Quantization & GGUF Acceleration). It integrates directly into `In-Process GGUF Tensor Engine`.
...
- **Subsystem Binding**: `In-Process GGUF Tensor Engine`
...
```json
{
  "skill": "gguf-tensor-byte-offset-resolver",
  "tier": "Tier 5: Local Tensor Engines, Quantization & GGUF Acceleration",
  "source": "GGML/GGUF Spec",
  "integration_target": "In-Process GGUF Tensor Engine",
