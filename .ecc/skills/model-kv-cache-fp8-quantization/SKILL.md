---
name: model-kv-cache-fp8-quantization
description: "Quantizing cached key and value tensors to FP8 halving memory requirements"
version: 1.0.0
...
triggers:
  ...
  - "fp8"
  - "gguf"
  - "model kv cache fp8 quantization"
  - "model-kv-cache-fp8-quantization"
  - "model-kv-cache-fp8-quantization execute"
  - "model-kv-cache-fp8-quantization verify"
---

# Model Kv Cache Fp8 Quantization Skill

The `model-kv-cache-fp8-quantization` skill equips Tagisan (`tgs`) autonomous agents and developers with authoritative capabilities sourced from **vLLM Documentation** (Tier 5: Local Tensor Engines, Quantization & GGUF Acceleration). It integrates directly into `Tensor Engine / KV Cache`.
...
- **Subsystem Binding**: `Tensor Engine / KV Cache`
...
```json
{
  "skill": "model-kv-cache-fp8-quantization",
  "tier": "Tier 5: Local Tensor Engines, Quantization & GGUF Acceleration",
  "source": "vLLM Documentation",
  "integration_target": "Tensor Engine / KV Cache",
