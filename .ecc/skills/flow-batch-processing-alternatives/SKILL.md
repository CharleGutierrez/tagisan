...
description: "Use when a Scheduled Flow or Record-Triggered Flow needs to process more records\
  \ than Flow can safely handle in a single run. Covers Flow limit realities, scheduled-path\
  \ chunking, Data Cloud batch transforms, and Apex. NOT for choosing async across\
  \ a general workflow (see async-selection decision tree) \u2014 use flow/flow-large-data-volume-patterns."
---
