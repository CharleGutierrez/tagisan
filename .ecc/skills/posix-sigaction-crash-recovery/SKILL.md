---
name: posix-sigaction-crash-recovery
description: "Crash handler installation with SIGSEGV/SIGBUS recovery and register state dump"
version: 1.0.0
...
triggers:
  ...
  - "low-level"
  - "performance"
  - "posix sigaction crash recovery"
  - "posix-sigaction-crash-recovery"
  - "posix-sigaction-crash-recovery execute"
  - "posix-sigaction-crash-recovery verify"
---

# Posix Sigaction Crash Recovery Skill

The `posix-sigaction-crash-recovery` skill equips Tagisan (`tgs`) autonomous agents and developers with authoritative capabilities sourced from **POSIX IEEE 1003.1** (Tier 3: Low-Level Systems, Linux Kernel & Hardware Acceleration). It integrates directly into `Engine / FaultRecovery`.
...
- **Subsystem Binding**: `Engine / FaultRecovery`
...
```json
{
  "skill": "posix-sigaction-crash-recovery",
  "tier": "Tier 3: Low-Level Systems, Linux Kernel & Hardware Acceleration",
  "source": "POSIX IEEE 1003.1",
  "integration_target": "Engine / FaultRecovery",
