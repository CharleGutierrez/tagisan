...
description: "Use this skill when building custom channel integrations with the Agentforce Agent\
  \ API or BYOC (Bring Your Own Channel) for CCaaS \u2014 covering session lifecycle\
  \ management, webhook handling, externalSessionKey design, sequenceId sequencing,\
  \ BYOC Interaction API integration, and conversation state management. NOT for standing\
  \ up a standard channel (Embedded Service, Slack, Experience Cloud, Messaging) \u2014\
  \ use agentforce/agent-channel-deployment. NOT for writing the Apex actions the\
  \ agent invokes \u2014 use agentforce/custom-agent-actions-apex."
---
