---
name: pypi-mcp-server-polars
description: "Blazing-fast multi-threaded Arrow DataFrame query execution and transforms"
version: 1.0.0
...
triggers:
  ...
  - "polars"
  - "pypi"
  - "pypi mcp server polars"
  - "pypi-mcp-server-polars"
  - "pypi-mcp-server-polars execute"
  - "pypi-mcp-server-polars verify"
  - "server"
---

# Pypi Mcp Server Polars Skill

The `pypi-mcp-server-polars` skill equips Tagisan (`tgs`) autonomous agents and developers with authoritative capabilities sourced from **PyPI** (Tier 1: Model Context Protocol (MCP) & Web Registries). It integrates directly into `MCP stdio client / ToolRegistry`.
