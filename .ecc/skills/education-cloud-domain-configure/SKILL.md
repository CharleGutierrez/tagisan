...
description: "Use this skill to configure Education Cloud domains (Student Success, Recruitment\
  \ & Admissions, Academic Operations, Alumni Relations, Student Management, or Mentoring)\
  \ through conversational setup. Triggers when a user wants to enable or configure\
  \ Education Cloud features. Guides step-by-step: enables Person Accounts prerequisite,\
  \ activates target domain, then configures domain sub-features. Covers support processes,\
  \ case record types, success teams, care plans, applications, admissions, course\
  \ catalog, alumni engagement, fundraising, enrollment, programs, and mentoring.\
  \ Explains each feature, confirms before proceeding, verifies every change took\
  \ effect, and warns that domain and feature toggles are IRREVERSIBLE. DO NOT TRIGGER\
  \ for metadata generation, custom objects, or package deployment \u2014 this configures\
  \ existing platform features via org settings, not metadata files."
---
