...
description: "Safe construction of dynamic SOQL \u2014 Database.query bind variables (:varName,\
  \ API 60+ semantics), Database.queryWithBinds(query, Map<String,Object>, AccessLevel)\
  \ (API 55+), field-name allowlisting, ORDER BY direction whitelist, LIMIT/OFFSET\
  \ typing, and WITH USER_MODE interaction (WITH SECURITY_ENFORCED was removed in\
  \ API 67.0). NOT for reviewing a query for injection or CRUD/FLS generally \u2014\
  \ use apex/soql-security. NOT for Schema.describe and runtime type inspection \u2014\
  \ use apex/dynamic-apex."
---
