...
description: "Use when a SOQL SELECT clause or SOSL FIND/RETURNING clause must return number, date,\
  \ time, or currency fields already localized to the running user's locale (e.g.\
  \ $44,000.00 or 4/10/2025, 3:31 PM instead of a raw decimal or ISO 8601 string).\
  \ Covers the FORMAT() function: its four supported field categories, the aliasing\
  \ rule when the same field is queried twice, and nesting FORMAT() inside aggregate\
  \ functions or convertCurrency(). NOT for grouping or filtering by date period \u2014\
  \ use apex/soql-date-functions. NOT for multi-currency setup and conversion rates\
  \ \u2014 use data/currency-management-patterns."
---
