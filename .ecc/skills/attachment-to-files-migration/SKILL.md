...
description: 'Migrating Classic Notes & Attachments to Salesforce Files (ContentDocument / ContentVersion
  / ContentDocumentLink): bulk extraction, owner and parent preservation, sharing
  translation, idempotent re-runs, and post-migration cleanup. Triggers: ''attachments
  to files'', ''notes and attachments migration'', ''ContentDocument from Attachment'',
  ''enable Files for Salesforce''. NOT for general file storage strategy (use data/salesforce-files-architecture)
  or for ContentVersion API patterns in new code (use integration/file-and-document-integration).'
---
