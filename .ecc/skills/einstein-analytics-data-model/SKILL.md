...
description: "Use this skill when working with CRM Analytics (Einstein Analytics) extended metadata\
  \ (XMD) \u2014 the multi-layer metadata system controlling field display labels,\
  \ aliases, number and date formatting, measure/dimension classification, and dataset\
  \ versioning. Trigger keywords: XMD API, dataset field formatting CRM Analytics,\
  \ wave dataset labels, main XMD update, dataset versioning Analytics. NOT for recipe\
  \ node configuration \u2014 use admin/analytics-recipe-design. NOT for dataflow\
