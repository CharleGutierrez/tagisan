---
name: npm-mcp-server-google-drive
description: "Google Drive file discovery, export to text, and document metadata extraction"
version: 1.0.0
...
triggers:
  ...
  - "npm-mcp-server-google-drive"
  - "npm-mcp-server-google-drive execute"
  - "npm-mcp-server-google-drive verify"
---

# Npm Mcp Server Google Drive Skill

The `npm-mcp-server-google-drive` skill equips Tagisan (`tgs`) autonomous agents and developers with authoritative capabilities sourced from **NPM Registry** (Tier 1: Model Context Protocol (MCP) & Web Registries). It integrates directly into `MCP stdio client / ToolRegistry`.
...
```json
{
  "skill": "npm-mcp-server-google-drive",
  "tier": "Tier 1: Model Context Protocol (MCP) & Web Registries",
  "source": "NPM Registry",
