---
name: npm-mcp-server-memory
description: "Graph-based persistent knowledge graph for agent session entities"
version: 1.0.0
...
triggers:
  ...
  - "model-context-protocol"
  - "npm"
  - "npm mcp server memory"
  - "npm-mcp-server-memory"
  - "npm-mcp-server-memory execute"
  - "npm-mcp-server-memory verify"
  - "server"
---

# Npm Mcp Server Memory Skill

The `npm-mcp-server-memory` skill equips Tagisan (`tgs`) autonomous agents and developers with authoritative capabilities sourced from **NPM Registry** (Tier 1: Model Context Protocol (MCP) & Web Registries). It integrates directly into `MCP stdio client / MemoryBackend`.
...
- **Subsystem Binding**: `MCP stdio client / MemoryBackend`
...
```json
{
  "skill": "npm-mcp-server-memory",
  "tier": "Tier 1: Model Context Protocol (MCP) & Web Registries",
  "source": "NPM Registry",
  "integration_target": "MCP stdio client / MemoryBackend",
