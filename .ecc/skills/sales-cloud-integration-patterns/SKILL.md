...
description: 'Use when designing or reviewing integrations between Sales Cloud and external systems
  such as ERP, marketing automation, CPQ, or partner portals. Triggers: ''sync Accounts
  and Products with ERP'', ''integrate marketing leads with Salesforce'', ''quote-to-order
  flow between Salesforce and ERP'', ''CPQ integration patterns'', ''partner portal
  data access design''. NOT for generic integration framework design (use architect/integration-framework-design),
  NOT for Apex HTTP callout mechanics (use apex/callouts-and-http-integrations), NOT
  for MuleSoft or middleware configuration.'
---
