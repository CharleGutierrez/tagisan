...
description: "Use this skill when managing data record dependencies during deployments: resolving\
  \ org-specific record type IDs that differ between source and target orgs, remapping\
  \ queue IDs and user IDs in data plans, seeding Custom Settings values post-deployment,\
  \ and using external IDs or DeveloperName references instead of hardcoded org-specific\
  \ IDs. Trigger keywords: INVALID_CROSS_REFERENCE_KEY Record Type ID, hardcoded record\
  \ type IDs deployment, deployment data record org-specific IDs, Custom Settings\
  \ not deployed, queue ID remap after sandbox refresh. NOT for eliminating hardcoded\
  \ record type IDs in Apex or metadata \u2014 use admin/record-type-id-management.\
  \ NOT for designing the external ID field and upsert key itself \u2014 use data/external-id-strategy."
---
