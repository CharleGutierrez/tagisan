...
description: "Plan and execute entire application builds. Generates phased delivery roadmaps, then\
  \ executes them autonomously \u2014 phase by phase, committing at milestones, deploying,\
  \ testing, and continuing until done or stuck. Modes: plan (generate roadmap), start\
  \ (begin executing), resume (continue from where you left off), status (show progress).\
  \ Triggers: 'roadmap', 'start building', 'resume the build', 'keep going', 'build\
  \ the whole thing', 'execute the roadmap', 'what phase are we on'."
---
# Roadmap
