---
name: postman-pingdom-checks
description: "Pingdom synthetic check response time monitoring and probe outage alerts"
version: 1.0.0
...
triggers:
  ...
  - "pingdom"
  - "postman"
  - "postman pingdom checks"
  - "postman-pingdom-checks"
  - "postman-pingdom-checks execute"
  - "postman-pingdom-checks verify"
---

# Postman Pingdom Checks Skill

The `postman-pingdom-checks` skill equips Tagisan (`tgs`) autonomous agents and developers with authoritative capabilities sourced from **Postman API Network** (Tier 2: Enterprise Actions & API Orchestration (Composio / Postman / Zapier)). It integrates directly into `ToolRegistry / MCP Connector`.
