---
name: linux-vfio-iommu-userspace-pci
description: "Virtual Function I/O userspace DMA mapping with IOMMU protection"
version: 1.0.0
...
triggers:
  ...
  - "kernel"
  - "linux"
  - "linux vfio iommu userspace pci"
  - "linux-vfio-iommu-userspace-pci"
  - "linux-vfio-iommu-userspace-pci execute"
  - "linux-vfio-iommu-userspace-pci verify"
  - "low-level"
---

# Linux Vfio Iommu Userspace Pci Skill

The `linux-vfio-iommu-userspace-pci` skill equips Tagisan (`tgs`) autonomous agents and developers with authoritative capabilities sourced from **Linux Kernel Docs (kernel.org)** (Tier 3: Low-Level Systems, Linux Kernel & Hardware Acceleration). It integrates directly into `HardwareEngine / QEMU`.
...
- **Subsystem Binding**: `HardwareEngine / QEMU`
...
```json
{
  "skill": "linux-vfio-iommu-userspace-pci",
  "tier": "Tier 3: Low-Level Systems, Linux Kernel & Hardware Acceleration",
  "source": "Linux Kernel Docs (kernel.org)",
  "integration_target": "HardwareEngine / QEMU",
