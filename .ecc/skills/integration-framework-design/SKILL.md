...
description: 'Use when designing a reusable integration layer in Salesforce that serves multiple
  external APIs through a shared callout infrastructure. Triggers: ''how to design
  a reusable integration layer in Salesforce'', ''architect an Apex callout framework
  for multiple APIs'', ''create a centralized error handling pattern for integrations'',
  ''service interface pattern for external APIs'', ''factory pattern for dynamic API
  resolution'', ''centralized callout dispatcher''. NOT for individual API implementation
  (use apex/callouts-and-http-integrations), NOT for Named Credential setup (use integration/named-credentials-setup),
  NOT for async callout patterns (use apex/continuation-callouts).'
---
