...
description: "Design input validation inside Screen Flow screens using component-level <validationRule>\
  \ (formula + errorMessage), isRequired, cross-field rules on the second field, reactive\
  \ components, and screen-level Decision fallbacks for cross-screen checks. Trigger\
  \ keywords: screen flow validation rule, EndDate after StartDate validation in flow\
  \ screen, block Next button until input valid. NOT for record-level Validation Rules\
  \ \u2014 use admin/validation-rules. NOT for choosing screen components or overall\
  \ screen UX \u2014 use flow/screen-flows."
---
