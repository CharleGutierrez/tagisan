...
description: "Debug and diagnose Microsoft Teams for Employee Service (ITSM) configuration failures\
  \ \u2014 run a pass/fail validation checklist against the org and report exact remediation.\
  \ Use this for: 'IT Desk login not working in Teams', 'UEL/CCP user can't log in\
  \ to IT Service', 'Teams feature not showing / not enabled', 'CI Hub / Service Catalog\
  \ / My Assets tab not loading', 'Teams Agentforce agent not responding', 'Teams\
  \ swarming not working', 'Teams SSO failing', or any request to troubleshoot a Teams\
  \ ITSM setup. Routes the problem to the right checklist (login, feature enablement,\
  \ tab loading, Agentforce, swarming, SSO) and checks the exact CORS, External Client\
  \ App, feature toggle, PSL, profile, auth-provider, named-credential, and messaging-channel\
  \ settings each requires. DO NOT TRIGGER for first-time enablement (service-itsm-teams-configure\
  \ / -itservice-configure / -itdesk-configure) or building the embedded agent from\
  \ scratch (service-itsm-teams-employee-agent-configure)."
---
