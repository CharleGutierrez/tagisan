...
description: "Use when building, deploying, or reviewing a React web app that runs natively on\
  \ Salesforce via Salesforce Multi-Framework and the UIBundle metadata type \u2014\
  \ scaffolding with `sf template generate ui-bundle`, data access through @salesforce/sdk-data\
  \ (createDataSDK, GraphQL, Apex fetch), App Launcher / Custom Application / Experience\
  \ targeting, and embedding Agentforce chat with the Agentforce Conversation Client.\
  \ Open Beta: scratch orgs and sandboxes only, English default language, no production\
  \ deploys. NOT for standard Lightning Web Component development (use lwc/* skills),\
  \ NOT for Lightning Out 1.0 / Visualforce embedding, and NOT for authoring LightningTypeBundle\
  \ UI overrides (use agentforce/agentforce-custom-lightning-types)."
---
