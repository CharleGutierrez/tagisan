...
description: "Confirm the customer's WhatsApp Business Account is shared with Salesforce on Meta's\
  \ side. Pure instruction + one user-confirmation prompt \u2014 no API calls, no\
  \ token handling. The actual sharing happens in Meta Business Suite (UI-only; Meta\
  \ provides no Graph endpoint to automate this step). Renders step-by-step instructions\
  \ with Salesforce's Business Manager ID substituted in, then asks the user to confirm\
  \ whether \"Salesforce\" appears in the WABA's Partners list."
---
