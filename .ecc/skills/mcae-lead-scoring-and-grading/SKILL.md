...
description: "Use this skill when configuring MCAE (Account Engagement / Pardot) lead scoring models,\
  \ grading profiles, score decay rules, or automation rules that fire on score/grade\
  \ thresholds. Covers: scoring point values per activity, score decay for inactivity,\
  \ Profiles for fit grading, combined MQL definitions (Score + Grade), and automation\
  \ rules vs completion actions. NOT for defining the MQL model before build \u2014\
  \ use admin/lead-scoring-requirements. NOT for Engagement Studio nurture programs\
