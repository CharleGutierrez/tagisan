...
- **Do not use `runOnJS` for new Reanimated 4 work**. It is deprecated and re-exported for compatibility, so first check the installed versions and migration scope. Prefer `scheduleOnRN(fn, ...args)` from `react-native-worklets` in new or actively migrated code. This applies everywhere: scroll handlers, gesture callbacks, `useAnimatedReaction`, `useFrameCallback`, and any other worklet context.
