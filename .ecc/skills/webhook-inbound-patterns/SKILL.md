...
description: "Use when implementing an inbound webhook receiver in Salesforce: routing via Apex\
  \ REST and Salesforce Sites, authenticating webhook payloads via HMAC, ensuring\
  \ idempotent processing, and handling the 5-second response window. NOT for Salesforce\
  \ POSTing a webhook out to a third-party endpoint \u2014 use integration/outbound-webhook-from-salesforce.\
  \ NOT for designing a general-purpose Apex REST API rather than a webhook sink \u2014\
  \ use apex/apex-rest-services."
---
