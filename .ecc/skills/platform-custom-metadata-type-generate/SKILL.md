...
description: "Use this skill when users need to create, generate, or validate Salesforce Custom\
  \ Metadata Type metadata \u2014 the __mdt object, its fields, and its deployable\
  \ records. Trigger when users mention custom metadata types, CMDT, __mdt objects,\
  \ custom metadata records, .md-meta.xml files, or reference/config data that must\
  \ ship between orgs. Also trigger for admin-maintained mapping, lookup, or crosswalk\
  \ tables and field mappings (\"map fields from A to B\", code lookups) \u2014 config\
  \ admins change without a code deploy belongs in a CMDT, never hardcoded in Apex\
  \ or Flow \u2014 and when troubleshooting CMDT record/type deploy errors. DO NOT\
  \ TRIGGER for Custom Settings \u2014 route to platform-custom-setting-generate;\
  \ for regular custom objects holding business records \u2014 use platform-custom-object-generate;\
  \ or for secrets and API keys \u2014 recommend a Named Credential, which this skill\
  \ never generates."
---
