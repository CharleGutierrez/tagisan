---
name: agent-state-machine-checkpointing
description: "Serializing agent execution state graphs to disk for crash resumption and time-travel"
version: 1.0.0
...
triggers:
  - "agent"
  - "agent state machine checkpointing"
  - "agent-state-machine-checkpointing"
  - "agent-state-machine-checkpointing execute"
  - "agent-state-machine-checkpointing verify"
  - "checkpointing"
  - "consensus"
  - "debate"
  - "langgraph"
...
# Agent State Machine Checkpointing Skill

The `agent-state-machine-checkpointing` skill equips Tagisan (`tgs`) autonomous agents and developers with authoritative capabilities sourced from **LangGraph Documentation** (Tier 6: Multi-Agent Swarms, MoA & Cognitive Architecture). It integrates directly into `Session Persistence`.
...
- **Subsystem Binding**: `Session Persistence`
...
```json
{
  "skill": "agent-state-machine-checkpointing",
  "tier": "Tier 6: Multi-Agent Swarms, MoA & Cognitive Architecture",
  "source": "LangGraph Documentation",
  "integration_target": "Session Persistence",
