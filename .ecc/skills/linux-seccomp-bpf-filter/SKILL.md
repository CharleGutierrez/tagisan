---
name: linux-seccomp-bpf-filter
description: "Strict Berkeley Packet Filter syscall whitelisting for untrusted subprocesses"
version: 1.0.0
...
triggers:
  ...
  - "linux-seccomp-bpf-filter"
  - "linux-seccomp-bpf-filter execute"
  - "linux-seccomp-bpf-filter verify"
---

# Linux Seccomp Bpf Filter Skill

The `linux-seccomp-bpf-filter` skill equips Tagisan (`tgs`) autonomous agents and developers with authoritative capabilities sourced from **Linux Kernel Docs (kernel.org)** (Tier 3: Low-Level Systems, Linux Kernel & Hardware Acceleration). It integrates directly into `AgentShield / Sandbox`.
...
```json
{
  "skill": "linux-seccomp-bpf-filter",
  "tier": "Tier 3: Low-Level Systems, Linux Kernel & Hardware Acceleration",
  "source": "Linux Kernel Docs (kernel.org)",
