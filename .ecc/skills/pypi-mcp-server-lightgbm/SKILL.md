---
name: pypi-mcp-server-lightgbm
description: "Fast distributed gradient boosting and leaf-wise tree splitting analysis"
version: 1.0.0
...
triggers:
  ...
  - "model-context-protocol"
  - "pypi"
  - "pypi mcp server lightgbm"
  - "pypi-mcp-server-lightgbm"
  - "pypi-mcp-server-lightgbm execute"
  - "pypi-mcp-server-lightgbm verify"
  - "server"
---

# Pypi Mcp Server Lightgbm Skill

The `pypi-mcp-server-lightgbm` skill equips Tagisan (`tgs`) autonomous agents and developers with authoritative capabilities sourced from **PyPI** (Tier 1: Model Context Protocol (MCP) & Web Registries). It integrates directly into `MCP stdio client / ToolRegistry`.
...
```json
{
  "skill": "pypi-mcp-server-lightgbm",
  "tier": "Tier 1: Model Context Protocol (MCP) & Web Registries",
  "source": "PyPI",
