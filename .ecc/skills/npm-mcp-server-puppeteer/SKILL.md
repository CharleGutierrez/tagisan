---
name: npm-mcp-server-puppeteer
description: "Automated browser interaction, DOM scraping, and screenshot capture"
version: 1.0.0
...
triggers:
  ...
  - "model-context-protocol"
  - "npm"
  - "npm mcp server puppeteer"
  - "npm-mcp-server-puppeteer"
  - "npm-mcp-server-puppeteer execute"
  - "npm-mcp-server-puppeteer verify"
  - "puppeteer"
  - "server"
---

# Npm Mcp Server Puppeteer Skill

The `npm-mcp-server-puppeteer` skill equips Tagisan (`tgs`) autonomous agents and developers with authoritative capabilities sourced from **NPM Registry** (Tier 1: Model Context Protocol (MCP) & Web Registries). It integrates directly into `MCP stdio client / ToolRegistry`.
...
```json
{
  "skill": "npm-mcp-server-puppeteer",
  "tier": "Tier 1: Model Context Protocol (MCP) & Web Registries",
  "source": "NPM Registry",
