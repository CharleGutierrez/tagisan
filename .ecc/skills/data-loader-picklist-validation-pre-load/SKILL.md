...
description: "When and how to pre-validate a CSV against Salesforce picklist rules and record-type\
  \ assignments BEFORE running a Data Loader / Bulk API insert or upsert \u2014 restricted\
  \ picklists, record-type-scoped values, inactive values, Global Value Sets, dependent\
  \ picklists, multi-select delimiters, API name vs label, and the 255-char per-value\
  \ limit. NOT for mapping CSV headers to field API names \u2014 use data/data-loader-csv-column-mapping.\
  \ NOT for cleaning up bad picklist values already in the org \u2014 use admin/picklist-field-integrity-issues."
---
