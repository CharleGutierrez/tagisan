...
description: "Query Catalog, database, and table metadata resources in Alibaba Cloud Data Lake\
  \ Formation (DLF). Provides read-only queries via the DLF OpenAPI Python SDK, supporting\
  \ listing and viewing Catalogs, databases, tables with their detailed information\
  \ and Schema definitions. Use cases: \"list available Catalogs\", \"list databases\"\
  , \"view table schema\", \"search tables\", \"search tables by name\", \"fuzzy search\"\
  , \"view DLF metadata\", \"what databases are in the data lake\", \"what columns\
  \ does a table have\", \"find tables whose name contains xxx\". This Skill only\
  \ contains read-only operations \u2014 no create, modify, or delete operations."
---
