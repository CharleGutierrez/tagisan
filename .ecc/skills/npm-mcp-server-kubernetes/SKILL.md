---
name: npm-mcp-server-kubernetes
description: "Kubernetes cluster state inspection and manifest validation"
version: 1.0.0
...
triggers:
  ...
  - "npm-mcp-server-kubernetes"
  - "npm-mcp-server-kubernetes execute"
  - "npm-mcp-server-kubernetes verify"
  - "server"
---

# Npm Mcp Server Kubernetes Skill

The `npm-mcp-server-kubernetes` skill equips Tagisan (`tgs`) autonomous agents and developers with authoritative capabilities sourced from **NPM Registry** (Tier 1: Model Context Protocol (MCP) & Web Registries). It integrates directly into `MCP stdio client / ToolRegistry`.
...
```json
{
  "skill": "npm-mcp-server-kubernetes",
  "tier": "Tier 1: Model Context Protocol (MCP) & Web Registries",
  "source": "NPM Registry",
