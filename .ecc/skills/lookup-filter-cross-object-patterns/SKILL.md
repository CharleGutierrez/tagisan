...
description: 'Design or repair lookup filters that constrain a child lookup using fields from the
  parent record (or a sibling record on the same object). Triggers: ''limit lookup
  based on another field'', ''cross-object lookup filter'', ''lookup filter $Source
  vs $User''. NOT for choosing lookup vs master-detail - use admin/lookup-and-relationship-design.
  NOT for duplicate matching rule criteria - use admin/duplicate-management.'
---
