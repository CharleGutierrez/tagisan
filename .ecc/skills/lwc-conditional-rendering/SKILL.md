...
description: "Use when writing, reviewing, or migrating Lightning Web Component templates that\
  \ branch between UI states with `lwc:if`, `lwc:elseif`, and `lwc:else`, including\
  \ getter-backed booleans, loading/error/ready state machines, keep-state vs reset-state\
  \ toggles, and legacy `if:true` / `if:false` cleanup. Triggers: 'lwc:if vs if:true',\
  \ 'lwc:elseif not working', 'conditional rendering in lwc', 'complex boolean in\
  \ template'. NOT for choosing between component types at runtime \u2014 use lwc/lwc-dynamic-components.\
  \ NOT for list rendering \u2014 use `for:each` or `iterator:it`."
---
