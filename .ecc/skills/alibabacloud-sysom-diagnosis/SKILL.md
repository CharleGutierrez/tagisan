...
description: "Use when troubleshooting Linux server performance or stability issues \u2014 CPU\
  \ saturation, high load, scheduling delay, memory pressure, OOM events, high RSS,\
  \ page cache / shared memory growth, memory cgroup residue, Java heap issues, disk\
  \ IO saturation or latency, packet loss, network jitter, or a server that is slow,\
  \ stuck, or unstable. Performs diagnosis and surfaces recommendations; does not\
