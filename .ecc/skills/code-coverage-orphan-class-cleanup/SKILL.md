...
description: 'Use when an org''s overall Apex coverage is sliding toward 75% because of orphaned
  (uncovered, unreferenced) classes inflating the denominator. Triggers: ''production
  deploy blocked at 74% coverage'', ''find apex classes with 0% coverage'', ''orphan
  apex class report'', ''org coverage dropping despite green test runs''. NOT for
  writing tests on actively used classes (use apex/test-class-standards) or for raising
