...
description: "Use this skill when designing or troubleshooting flows that need to suspend execution\
  \ and wait for an external signal \u2014 either a time-based alarm or a platform\
  \ event. Trigger keywords: pause element, wait element, paused flow interview. NOT\
  \ for flows that recur on a fixed schedule (use flow/scheduled-flows) or scheduled-path\
  \ setup \u2014 use flow/flow-time-based-patterns."
---
