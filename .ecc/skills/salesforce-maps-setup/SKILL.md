...
description: "Use when configuring Salesforce Maps (formerly MapAnything) \u2014 territory planning,\
  \ route optimization, live tracking, geo-grid visualizations, and check-in/check-out\
  \ workflows for Sales or Service field reps not on Field Service. Covers package\
  \ installation order (Maps + Maps Advanced + Maps Routing/Live Tracking add-ons),\
  \ the MapsTerritoryPlan / MapsAdvancedRoute / MapsLayer object family, base-data\
  \ syncs (Geocoding and Routing services), and integration with Sales and Service\
  \ Cloud records. Triggers: 'Salesforce Maps setup', 'MapAnything migration', 'territory\
  \ planning by polygon', 'route optimization for sales reps', 'live tracking field\
  \ reps', 'plot accounts on a map', 'check-in to the closest account'. NOT for Field\
  \ Service Lightning territory and scheduling (use admin/fsl-scheduling-optimization-design\
  \ and data/fsl-territory-data-setup) \u2014 Maps and FSL are different products.\
  \ NOT for Consumer Goods Cloud retail visit planning (use admin/consumer-goods-cloud-setup)\
  \ \u2014 RoutePlan/Visit objects are CG-specific. NOT for Tableau / CRM Analytics\
  \ geo charts."
---
