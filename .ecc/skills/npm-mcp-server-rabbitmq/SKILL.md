---
name: npm-mcp-server-rabbitmq
description: "RabbitMQ queue inspection, message consumption, and exchange topology"
version: 1.0.0
...
triggers:
  ...
  - "model-context-protocol"
  - "npm"
  - "npm mcp server rabbitmq"
  - "npm-mcp-server-rabbitmq"
  - "npm-mcp-server-rabbitmq execute"
  - "npm-mcp-server-rabbitmq verify"
  - "rabbitmq"
  - "server"
---

# Npm Mcp Server Rabbitmq Skill

The `npm-mcp-server-rabbitmq` skill equips Tagisan (`tgs`) autonomous agents and developers with authoritative capabilities sourced from **NPM Registry** (Tier 1: Model Context Protocol (MCP) & Web Registries). It integrates directly into `MCP stdio client / ToolRegistry`.
