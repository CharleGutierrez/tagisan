...
description: 'Use when choosing between Bulk API 2.0 and REST/SOAP API for large-volume data operations,
  sizing batch jobs, monitoring bulk job status, and handling failed records in bulk
  pipelines. Triggers: bulk api, large data load, batch insert, bulk job, serial vs
  parallel mode. NOT for Data Loader UI steps (use data-import-and-management) or
  real-time single-record integrations (use apex/callouts-and-http-integrations).'
---
