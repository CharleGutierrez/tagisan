...
description: "Use when designing Integration Procedures (IPs) with platform cache to cut latency\
  \ and callout load. Covers Cache Block boundary, cache key design, per-user vs org-wide\
  \ partitions, invalidation on data changes, and safe fallback on cache miss. NOT\
  \ for general IP authoring or LWC client-side caching \u2014 use omnistudio/integration-procedures."
---
