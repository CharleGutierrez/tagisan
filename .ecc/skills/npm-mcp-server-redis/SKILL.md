---
name: npm-mcp-server-redis
description: "Redis key management, memory usage profiling, and stream consumption"
version: 1.0.0
...
triggers:
  ...
  - "npm-mcp-server-redis"
  - "npm-mcp-server-redis execute"
  - "npm-mcp-server-redis verify"
  - "redis"
  - "server"
---

# Npm Mcp Server Redis Skill

The `npm-mcp-server-redis` skill equips Tagisan (`tgs`) autonomous agents and developers with authoritative capabilities sourced from **NPM Registry** (Tier 1: Model Context Protocol (MCP) & Web Registries). It integrates directly into `MCP stdio client / ToolRegistry`.
