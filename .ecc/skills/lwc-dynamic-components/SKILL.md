...
description: "Dynamic LWC component creation using the `lwc:component` directive, lazy-loaded dynamic\
  \ imports (`import()`), and runtime component resolution for conditional rendering\
  \ at scale. Triggers: 'render different components based on record type', 'dynamically\
  \ load lwc at runtime', 'lwc:component lwc:is constructor', 'lazy load component\
  \ only when needed', 'dynamic import lwc'. NOT for lwc:if over a fixed component\
  \ set \u2014 use lwc/lwc-conditional-rendering. NOT for page performance tuning\
  \ \u2014 use lwc/lwc-performance."
---
