...
description: "Use when designing or reasoning about an Agentforce Grid worksheet \u2014 the spreadsheet-like\
  \ Agentforce workspace where rows are jobs and columns are sequential Data / AI\
  \ / Action steps that process left to right \u2014 to run bulk record updates, generate\
  \ insights, or test multi-turn AI conversations over CRM or Data Cloud (Data 360)\
  \ data without Apex or Flow. Trigger keywords: Agentforce Grid, workbook, worksheet,\
  \ grid column, data column, AI column, action column, bulk AI update, Flex Credits\
  \ per row, metered compute. NOT for building the agent, its topics, or its actions\
  \ themselves (use agentforce/* action and topic skills), NOT for single-record prompt\
  \ design (use the prompt-builder skills), NOT for deployable metadata/Apex (Grid\
  \ is a no-code Studio tool, not a metadata type), and NOT for AI governance/ethics\
  \ policy (use admin/ai-ethics-and-governance-requirements)."
---
# Agentforce Grid
