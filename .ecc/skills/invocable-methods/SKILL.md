...
description: "Use when designing or reviewing Apex actions exposed to Flow or similar orchestration\
  \ layers via `@InvocableMethod`, especially around wrapper DTOs, bulk-safe list\
  \ contracts, and error behavior. Triggers: 'InvocableMethod', 'InvocableVariable',\
  \ 'Flow Apex action', 'bulk-safe invocable', 'Apex action input/output'. NOT for\
  \ wiring the action into Flow Builder \u2014 use flow/flow-action-framework. NOT\
  \ for an Agentforce agent action \u2014 use agentforce/custom-agent-actions-apex."
---
