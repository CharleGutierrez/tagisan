...
description: "Choosing between virtualizing external data into Salesforce (External Objects via\
  \ Salesforce Connect / OData 2.0 / 4.0 / cross-org adapter) and replicating it into\
  \ a custom object. Covers indirect lookup keys, per-transaction callout caps, and\
  \ what External Objects cannot do (no triggers, no validation rules, no record-triggered\
  \ flows, limited reporting and search). NOT for plain REST callouts with no External\
  \ Object \u2014 use apex/callouts-and-http-integrations. NOT for one-time ETL or\
  \ a data migration that copies the data in \u2014 use data/data-migration-planning."
---
