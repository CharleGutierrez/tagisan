...
description: "Reason about how record-triggered flows interleave with the Salesforce Save Order\
  \ (validation, before-save flows, before triggers, duplicate rules, after-save flows,\
  \ workflow, after triggers, assignment, auto-response, escalation). Trigger keywords:\
  \ save order, before-save flow, after-save flow, dml order, trigger vs flow order.\
  \ NOT for choosing before-save vs after-save for one flow \u2014 use flow/record-triggered-flow-patterns.\
