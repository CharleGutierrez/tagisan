...
description: Use to enable the Consumer Goods Cloud TPM Promotion Business Object (BO) API framework
  in a Salesforce org, interview the user for the custom Workflow Step they want to
  add, generate and deploy that Apex class, wire it into the create/update/copy workflows,
  and prove it end-to-end via headless REST ingest. Ships a worked reference example
  (SetCommentValue) but the customization is user-supplied per org. Triggers on "deploy
  the promotion BO API", "set up promotion BO API", "headless promotion create", "add
  a BO API workflow step".
...
---
