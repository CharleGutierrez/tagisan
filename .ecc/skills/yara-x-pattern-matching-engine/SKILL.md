---
name: yara-x-pattern-matching-engine
description: "Scanning downloaded files and memory buffers using compiled YARA-X rules"
version: 1.0.0
...
triggers:
  ...
  - "yara-x-pattern-matching-engine"
  - "yara-x-pattern-matching-engine execute"
  - "yara-x-pattern-matching-engine verify"
---

# Yara X Pattern Matching Engine Skill

The `yara-x-pattern-matching-engine` skill equips Tagisan (`tgs`) autonomous agents and developers with authoritative capabilities sourced from **YARA-X Project Reference** (Tier 7: Cybersecurity, EDR & Nation-State APT Defense). It integrates directly into `AgentShield / MalwareScan`.
...
- **Subsystem Binding**: `AgentShield / MalwareScan`
...
```json
{
  "skill": "yara-x-pattern-matching-engine",
  "tier": "Tier 7: Cybersecurity, EDR & Nation-State APT Defense",
  "source": "YARA-X Project Reference",
  "integration_target": "AgentShield / MalwareScan",
