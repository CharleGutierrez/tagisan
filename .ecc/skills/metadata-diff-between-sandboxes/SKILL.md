...
description: "Use when comparing metadata between two Salesforce orgs (UAT vs Prod, dev sandbox\
  \ vs full copy, fork sandbox vs source) to surface drift, identify items needing\
  \ deployment, or build a destructive-changes manifest. Triggers: 'compare two. NOT\
  \ for code-level diffs in version control or for deploying packages \u2014 use devops/metadata-api-coverage-gaps."
---
