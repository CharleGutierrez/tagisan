---
name: smithery-milvus-vector-search
description: "High-throughput billion-scale vector collection search and IVF-PQ"
version: 1.0.0
...
triggers:
  ...
  - "search"
  - "smithery"
  - "smithery milvus vector search"
  - "smithery-milvus-vector-search"
  - "smithery-milvus-vector-search execute"
  - "smithery-milvus-vector-search verify"
---

# Smithery Milvus Vector Search Skill

The `smithery-milvus-vector-search` skill equips Tagisan (`tgs`) autonomous agents and developers with authoritative capabilities sourced from **Smithery.ai** (Tier 1: Model Context Protocol (MCP) & Web Registries). It integrates directly into `MCP stdio client / VectorStoreBackend`.
...
- **Subsystem Binding**: `MCP stdio client / VectorStoreBackend`
...
```json
{
  "skill": "smithery-milvus-vector-search",
  "tier": "Tier 1: Model Context Protocol (MCP) & Web Registries",
  "source": "Smithery.ai",
  "integration_target": "MCP stdio client / VectorStoreBackend",
