...
description: 'Use when designing or reviewing org-wide large data volume (LDV) concerns: query
  optimizer behavior and selectivity, custom indexes and skinny tables, sharing and
  ownership skew, bulk data loads, archival to Big Objects, and divisions. Triggers:
  LDV architecture review, skinny table request, ownership skew hurting sharing, millions
  of records on custom object, bulk load performance, 40 million rows on one object
  and queries crawl, tens of millions of records and everything is slow, object has
  grown too large and reports time out. NOT for rewriting individual SOQL statements
  or interpreting Query Plan output in isolation (use data/soql-query-optimization),
  sales-only Opportunity and Account skew playbooks (use architect/high-volume-sales-data-architecture),
  or Marketing Cloud data extensions.'
---
