...
description: "Use when user explicitly asks Flink/Ververica/Realtime Compute Console workspace\
  \ operations: \u8349\u7A3F(draft), SQL\u6821\u9A8C/\u6267\u884C, \u90E8\u7F72(deployment),\
  \ \u4F5C\u4E1A(job), Session Cluster, namespace, \u8868(table), \u6210\u5458(member),\
  \ \u53D8\u91CF(variable), \u6216 checkpoint timeout \u8BCA\u65AD, especially with\
  \ workspace/deployment/job IDs (w-*, d-*, j-*, sc-*, draft-*). Also use when prompt\
  \ asks to test/verify Flink Console lifecycle flow, safety guardrails, or parameter\
  \ validation for these operations. This includes prompts such as create draft, deploy\
  \ draft, list deployments, start/stop job, create/list session cluster, get tables,\
  \ list variables. Also use when prompt explicitly asks to run `python scripts/flink_ververica_ops.py`\
  \ for Flink Console workspace operations. Do not trigger for unrelated \"workspace\"\
  \ contexts or generic cloud/platform tasks (ECS, OSS, RDS, Kafka, Spark, Kubernetes,\
  \ billing, weather). Do not trigger for Flink instance lifecycle operations (create/scale/delete/renew);\
  \ those belong to alibabacloud-flink-instance-manage."
---
