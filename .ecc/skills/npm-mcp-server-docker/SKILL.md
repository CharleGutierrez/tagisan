...
description: "Docker container and image manipulation through native Unix socket"
version: 1.0.0
...
triggers:
  ...
  - "npm-mcp-server-docker"
  - "npm-mcp-server-docker execute"
  - "npm-mcp-server-docker verify"
  - "server"
---

# Npm Mcp Server Docker Skill

The `npm-mcp-server-docker` skill equips Tagisan (`tgs`) autonomous agents and developers with authoritative capabilities sourced from **NPM Registry** (Tier 1: Model Context Protocol (MCP) & Web Registries). It integrates directly into `MCP stdio client / ToolRegistry`.
