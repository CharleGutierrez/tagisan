...
description: "Use to enable the five Omni-Channel base settings on a Salesforce org via the Metadata\
  \ API. The canonical writer scripts/configure-and-report.sh detects, deploys, and\
  \ re-verifies in one call (run mode) or detects only (plan mode); it is idempotent\
  \ (deploys only when a toggle is false or missing) and guards writes with a sandbox/CDO/trial/DE\
  \ safe_to_write check. It always surfaces the UI-only login-behavior gap in its\
  \ report. Triggers: enable Omni-Channel, turn on Omni base settings, configure OmniChannelSettings,\
  \ deploy Settings:OmniChannel. Do not use to configure downstream Omni entities\
  \ (queues, presence, permissions \u2014 use the entity-specific skill) or to deploy\
  \ other Settings types (use platform-metadata-deploy)."
---
