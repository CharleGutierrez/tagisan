...
description: "Cross-tool decision matrix for field-update automation in Salesforce \u2014 Before-Save\
  \ Flow vs After-Save Flow vs Apex Trigger vs the deprecated Workflow Rule + Field\
  \ Update. Covers the recursion / re-entrancy rules, governor cost per pattern (Before-Save\
  \ flow is governor-free for the same record), the order-of-execution slot each tool\
  \ occupies, and the Workflow-Rule-to-Flow migration playbook for field-update actions.\
  \ NOT for the full record-save order of execution \u2014 use apex/order-of-execution-deep-dive.\
  \ NOT for building the record-triggered Flow once the tool is chosen \u2014 use\
