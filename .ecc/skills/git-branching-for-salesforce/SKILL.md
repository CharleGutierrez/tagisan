...
description: "Use when selecting or reviewing a Git branching strategy for a Salesforce project\
  \ \u2014 choosing between trunk-based development, GitFlow, or environment branching,\
  \ and mapping branches to orgs, packages, and CI pipelines. Trigger keywords: 'git\
  \ branching strategy Salesforce', 'trunk-based vs GitFlow', 'branch per package',\
  \ 'feature branch model SFDX', 'branching for unlocked packages', 'git workflow\
  \ Salesforce DX'. NOT for CI/CD pipeline implementation (use devops/github-actions-for-salesforce\
  \ or devops/gitlab-ci-for-salesforce), NOT for environment topology decisions (use\
  \ devops/environment-strategy), NOT for scratch org lifecycle management."
---
