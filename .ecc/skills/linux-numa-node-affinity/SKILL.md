---
name: linux-numa-node-affinity
description: "Non-Uniform Memory Access node binding preventing inter-socket interconnect bottlenecks"
version: 1.0.0
...
triggers:
  ...
  - "kernel"
  - "linux"
  - "linux numa node affinity"
  - "linux-numa-node-affinity"
  - "linux-numa-node-affinity execute"
  - "linux-numa-node-affinity verify"
  - "low-level"
---

# Linux Numa Node Affinity Skill

The `linux-numa-node-affinity` skill equips Tagisan (`tgs`) autonomous agents and developers with authoritative capabilities sourced from **Linux Kernel Docs (kernel.org)** (Tier 3: Low-Level Systems, Linux Kernel & Hardware Acceleration). It integrates directly into `Native Rust / GGUF Mmap`.
...
- **Subsystem Binding**: `Native Rust / GGUF Mmap`
...
```json
{
  "skill": "linux-numa-node-affinity",
  "tier": "Tier 3: Low-Level Systems, Linux Kernel & Hardware Acceleration",
  "source": "Linux Kernel Docs (kernel.org)",
  "integration_target": "Native Rust / GGUF Mmap",
