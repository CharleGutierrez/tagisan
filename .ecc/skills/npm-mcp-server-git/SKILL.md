---
name: npm-mcp-server-git
description: "Local git repository branch inspection, commit diffing, and blame analysis"
version: 1.0.0
...
triggers:
  ...
  - "npm mcp server git"
  - "npm-mcp-server-git"
  - "npm-mcp-server-git execute"
  - "npm-mcp-server-git verify"
  - "server"
---

# Npm Mcp Server Git Skill

The `npm-mcp-server-git` skill equips Tagisan (`tgs`) autonomous agents and developers with authoritative capabilities sourced from **NPM Registry** (Tier 1: Model Context Protocol (MCP) & Web Registries). It integrates directly into `MCP stdio client / ToolRegistry`.
