...
description: 'Configure the embedded Agentforce Employee Agent so it replies inside the Microsoft
  Teams ITSM custom client (''Salesforce Employee Assist'' / ''Ask AI Agent''). Use
  this for: ''set up employee agent in Teams'', ''embed Agentforce agent in Teams'',
  ''make the IT Service Employee Agent reply in Teams'', ''Teams Ask AI Agent not
  responding'', ''agent joins then leaves without replying'', ''configure MIAW deployment
  for Teams employee agent'', ''Teams embedded messaging agent setup''. Builds the
  whole stack headlessly (zero Setup-UI clicks): the Web messaging channel with User
  Verification ON, the Enhanced Chat User Verification Key Set (JWKS_URL) it requires,
  the Teams_AgentForce custom-client deployment, the routing flow to the agent, and
  the Agent Access permission set that lets the portal user reach the agent. DO NOT
  TRIGGER for enabling the Teams feature Salesforce Go page toggle (service-itsm-teams-configure)
  or for configuring notification preferences.'
---
