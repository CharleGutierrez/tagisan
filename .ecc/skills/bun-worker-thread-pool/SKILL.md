---
name: bun-worker-thread-pool
description: "Isolated multi-threaded worker pools executing background JavaScript tools"
version: 1.0.0
...
triggers:
  - "bun"
  - "bun worker thread pool"
  - "bun-worker-thread-pool"
  - "bun-worker-thread-pool execute"
  - "bun-worker-thread-pool verify"
  - "frontend"
  - "full-stack"
  - "pool"
  - "thread"
...
# Bun Worker Thread Pool Skill

The `bun-worker-thread-pool` skill equips Tagisan (`tgs`) autonomous agents and developers with authoritative capabilities sourced from **Bun Documentation (bun.sh)** (Tier 8: Modern Full-Stack, Bun/TS & UI/UX Vibe Coding). It integrates directly into `Bun Runtime / Polyglot`.
