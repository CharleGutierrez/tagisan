---
name: pypi-mcp-server-duckdb
description: "DuckDB analytical querying and Parquet file analysis via Python runtime"
version: 1.0.0
...
triggers:
  ...
  - "model-context-protocol"
  - "pypi"
  - "pypi mcp server duckdb"
  - "pypi-mcp-server-duckdb"
  - "pypi-mcp-server-duckdb execute"
  - "pypi-mcp-server-duckdb verify"
  - "server"
---

# Pypi Mcp Server Duckdb Skill

The `pypi-mcp-server-duckdb` skill equips Tagisan (`tgs`) autonomous agents and developers with authoritative capabilities sourced from **PyPI** (Tier 1: Model Context Protocol (MCP) & Web Registries). It integrates directly into `MCP stdio client / ToolRegistry`.
