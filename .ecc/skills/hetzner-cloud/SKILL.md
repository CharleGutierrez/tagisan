---
name: hetzner-cloud
description: "Manage Hetzner Cloud infrastructure with the `hcloud` CLI \u2014 servers, networks,\
  \ firewalls, load balancers, volumes, DNS zones, SSH keys, primary/floating IPs,\
  \ snapshots, certificates, placement groups, storage boxes. Use whenever the user\
  \ mentions Hetzner, hcloud, VPS provisioning, or Hetzner location codes (fsn1, hel1,\
  \ nbg1, ash, hil, sin) \u2014 even if they don't say \"hcloud\". CLI-only; does\
  \ NOT cover Hetzner Robot (dedicated servers, separate product and API)."
---
# Hetzner Cloud
