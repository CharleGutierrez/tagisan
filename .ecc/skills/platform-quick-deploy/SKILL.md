...
description: "Deploy validated metadata to a Production Salesforce org without re-running tests.\
  \ TRIGGER when the user wants to deploy to production, says 'quick deploy', 'promote',\
  \ 'ship to prod', or has just validated and wants to push the change live. REQUIRES\
  \ a recent `sf project deploy validate` job ID (\u226410 days old, \u22643 days\
  \ for --use-most-recent). DO NOT TRIGGER for sandbox/scratch deploys (use platform-metadata-deploy)\
  \ or unvalidated deploys (use platform-deploy-validate first)."
---
