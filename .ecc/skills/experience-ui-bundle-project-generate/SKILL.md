...
description: "Generates a minimal, ready-to-develop SFDX starter project from template instead\
  \ of hand-scaffolding files. Use this skill when starting a brand-new Salesforce\
  \ UI bundle app (React or Angular) and the initial project must be scaffolded \u2014\
  \ trigger phrases include create, start, or scaffold a new UI bundle app, generate\
  \ a starter project, or use a prebuilt/starter template. DO NOT TRIGGER when: editing,\
  \ styling, or adding pages or components to an EXISTING app (use experience-ui-bundle-frontend-generate);\
  \ configuring ui-bundle.json or metadata files (use experience-ui-bundle-metadata-generate);\
  \ deploying to an org (use experience-ui-bundle-deploy); or when the user explicitly\
  \ says they want to hand-scaffold from scratch."
---
