...
description: "Migrate Lightning Web Components from SLDS 1 to SLDS 2 by running the SLDS linter\
  \ and fixing violations. Use this skill whenever users mention SLDS 2, SLDS uplift,\
  \ linter violations, LWC token migration, class overrides, hardcoded CSS values\
  \ that need SLDS hook replacement, or styling hook selection. Covers all styling\
  \ hook categories \u2014 color, spacing, sizing, typography, borders, radius, and\
  \ shadows. Also use when users mention no-hardcoded-values, no-slds-class-overrides,\
  \ lwc-to-slds-hooks, no-deprecated-tokens-slds1, or ask about SLDS component migration\
  \ \u2014 even if they don't explicitly say \"uplift\" or \"migration\"."
---
# Goal
