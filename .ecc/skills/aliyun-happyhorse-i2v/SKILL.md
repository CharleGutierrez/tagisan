...
description: Use when generating videos from a single first-frame image with DashScope HappyHorse
  1.0 image-to-video model (happyhorse-1.0-i2v). Use when implementing first-frame
  video generation with optional text guidance via the video-synthesis async API on
