...
description: "Use when an LWC needs to programmatically manipulate Service Console workspace tabs\
  \ and the utility bar \u2014 opening tabs and subtabs, refreshing or closing tabs,\
  \ setting tab labels and icons, detecting console context, and integrating with\
  \ the utility bar \u2014 via the `lightning/platformWorkspaceApi` and `lightning/platformUtilityBarApi`\
  \ modules. Triggers: 'openSubtab in LWC', 'refresh console tab from LWC', 'IsConsoleNavigation\
  \ context detection', 'set tab label dynamically'. NOT for declarative Console App\
  \ setup (use admin/service-console-configuration), navigation to records outside\
  \ a console (use lwc/lwc-navigation-mixin or lwc/lightning-navigation-dead-link-handling),\
  \ or Aura `lightning:workspaceAPI` migration (the LWC modules supersede it for new\
  \ development)."
---
