...
description: "OpenAI Agents SDK (Python) development. Use when building AI agents, multi-agent\
  \ handoffs, function tools, guardrails, sessions, streaming, or tracing with the\
  \ `openai-agents` / `agents` Python package \u2014 including Azure OpenAI via LiteLLM.\
  \ Triggers on imports from `agents`, uses of `Runner.run_sync`/`Runner.run_streamed`,\
  \ `@function_tool`, `AgentOutputSchema`, `SQLiteSession`, or questions about the\
  \ openai-agents-python SDK. Python only \u2014 not the TypeScript `@openai/agents`\
  \ SDK."
---
