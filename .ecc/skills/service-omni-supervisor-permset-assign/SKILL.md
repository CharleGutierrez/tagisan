...
description: "Use to assign the Salesforce-shipped standard ContactCenterSupervisor PermissionSet\
  \ (default) to N existing supervisor users via PermissionSetAssignment DML. Idempotent\
  \ \u2014 SOQL detects existing (user, perm-set) pairs before POST, and DUPLICATE_VALUE\
  \ is treated as reused. The standard set carries its own permission-set license\
  \ and assigns cleanly on Service-Cloud-enabled orgs; a user whose license lacks\
  \ the entitlement surfaces FIELD_INTEGRITY_EXCEPTION so the operator can fix the\
  \ profile/license. Triggers: assign the supervisor permset, grant supervisor perms,\
  \ complete supervisor provisioning. Do not use on production orgs or to assign agent\
  \ permsets."
---
