...
description: "Top-level interactive coordinator for Employee Service (ITSM) channel setup. Presents\
  \ menu of channel setup options (Portal, Teams, Slack) and delegates to the corresponding\
  \ skill. Use this for: 'setup ITSM channels', 'configure employee service channels',\
  \ 'set up service cloud communication channels', 'I want to configure ITSM integrations',\
  \ or any general request for ITSM channel/integration setup where the user hasn't\
  \ specified exactly which channel they want. DO NOT TRIGGER for requests that already\
  \ name a specific channel or child skill (e.g. 'configure Teams', 'set up Slack',\
  \ 'create an employee portal', 'configure ITSM notifications') \u2014 route directly\
  \ to the matching child skill instead."
---
