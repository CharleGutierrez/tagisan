---
name: smithery-qdrant-vector-db
description: "Vector memory management, HNSW collection indexing, and KNN queries"
version: 1.0.0
...
triggers:
  ...
  - "qdrant"
  - "smithery"
  - "smithery qdrant vector db"
  - "smithery-ai"
  - "smithery-qdrant-vector-db"
  - "smithery-qdrant-vector-db execute"
  - "smithery-qdrant-vector-db verify"
---

# Smithery Qdrant Vector Db Skill

The `smithery-qdrant-vector-db` skill equips Tagisan (`tgs`) autonomous agents and developers with authoritative capabilities sourced from **Smithery.ai** (Tier 1: Model Context Protocol (MCP) & Web Registries). It integrates directly into `MCP stdio client / VectorStoreBackend`.
...
```json
{
  "skill": "smithery-qdrant-vector-db",
  "tier": "Tier 1: Model Context Protocol (MCP) & Web Registries",
  "source": "Smithery.ai",
