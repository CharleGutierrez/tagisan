...
| 影响内核版本 | 5.x ~ 6.4.x |
| 利用技术 | nf_tables batch处理异常分支导致的rule/set双重释放 |
...
### 利用原理
nf_tables_newrule在异常分支释放rule和其引用的anonymous set时未更新set状态为inactive，导致batch后续请求还能访问释放对象。公开稳定PoC暂未广泛发布。

该漏洞的触发条件为nf_tables batch异常分支rule/set双重释放。攻击者通过精心构造的系统调用序列，利用UAF (Use-After-Free)类型的缺陷，在内核空间中实现代码执行或数据篡改。

利用过程中，关键的内存操作为rule和anonymous set对象double-free后重用。最终的提权路径通过通过nf_tables batch UAF控制内核执行流完成，使非特权用户获得root权限。

### 关键技术点
1. nf_tables batch异常分支rule/set双重释放
2. rule和anonymous set对象double-free后重用
3. 通过nf_tables batch UAF控制内核执行流

## PoC改造计划

### CTF挑战模式适配
- **推荐测试模式**: write_root_file
- **模式选择理由**: nftables UAF可通过堆喷控制freed对象实现任意内核写入和完整LPE
...
### 三阶段验证集成
- **Phase 1 (Prepare)**: 创建netfilter namespace，准备batch操作环境
- **Phase 2 (Run)**: 触发batch异常分支导致rule/set double-free
- **Phase 3 (Post)**: 验证提权结果、清理nft规则和namespace
...
Source: https://github.com/google/security-research/tree/master/pocs/linux/kernelctf/CVE-2023-3390_lts

```c
/*
 * CVE-2023-3390 PoC - CTF Challenge Mode
 *
 * Vulnerability: nf_tables use-after-free in NFT_MSG_NEWRULE handling
 *
...
/*
 * Trigger nftables UAF condition (without spray)
 * Creates nftables ruleset, triggers rule creation error path
...
static int mode_uaf(poc_args_t *args) {
    poc_log("Triggering CVE-2023-3390 UAF + spray corruption check...");
...
static int mode_write_root_file(const poc_args_t *args) {
    ...
    poc_log("Triggering CVE-2023-3390 UAF vulnerability...");
    int uaf_rc = trigger_nf_tables_uaf();
    if (uaf_rc < 0) {
...
int main(int argc, char *argv[]) {
    ...
    poc_log("=== CVE-2023-3390 CTF PoC ===");
    poc_log("Vuln: nf_tables use-after-free in NFT_MSG_NEWRULE handling");
