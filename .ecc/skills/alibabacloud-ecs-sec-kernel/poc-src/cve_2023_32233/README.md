...
| 影响内核版本 | 5.1-rc1 ~ 6.3.1 |
| 利用技术 | nf_tables anonymous set删除时引用计数的双重递减 |

## 原始PoC分析

### 参考来源
- **PoC URL**: https://github.com/oferchen/POC-CVE-2023-32233/blob/main/poc.c
- **来源类型**: GitHub

### 利用原理
nf_tables中anonymous set处理存在UAF漏洞。删除PIPAPO set时通过NFT_JUMP verdict引起chain use count双重递减。

该漏洞的触发条件为nf_tables anonymous set删除引用计数双重递减。攻击者通过精心构造的系统调用序列，利用UAF (Use-After-Free)类型的缺陷，在内核空间中实现代码执行或数据篡改。

利用过程中，关键的内存操作为chain对象UAF后重用实现RIP劫持。最终的提权路径通过通过nf_tables UAF劫持内核执行流提权完成，使非特权用户获得root权限。

### 关键技术点
1. nf_tables anonymous set删除引用计数双重递减
2. chain对象UAF后重用实现RIP劫持
3. 通过nf_tables UAF劫持内核执行流提权

## PoC改造计划

### CTF挑战模式适配
- **推荐测试模式**: write_root_file
- **模式选择理由**: nf_tables匿名集合UAF可通过堆喷实现任意内核写入和完整LPE
...
### 三阶段验证集成
- **Phase 1 (Prepare)**: 创建netfilter namespace，配置nft规则和anonymous set
- **Phase 2 (Run)**: 触发set删除时引用计数双重递减UAF
...
Source: https://github.com/oferchen/POC-CVE-2023-32233/blob/main/poc.c
