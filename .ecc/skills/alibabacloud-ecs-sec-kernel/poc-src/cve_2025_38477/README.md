...
| 影响内核版本 | v5.0 ~ v6.8+ |
| 利用技术 | QFQ packet scheduler聚合计算的并发访问竞速 |

## 原始PoC分析

### 参考来源
- **PoC URL**: https://github.com/google/security-research/tree/master/pocs/linux/kernelctf/CVE-2025-38477_cos
- **来源类型**: Google security-research (kernelCTF)

### 利用原理
QFQ调度器访问aggregate数据结构时缺乏充分锁保护。aggregate正在被销毁时另一CPU通过enqueue路径引用该aggregate产生UAF。

攻击者可配置QFQ qdisc创建多个class/flow后并发执行flow删除和数据包发送操作，利用aggregate生命周期管理漏洞触发UAF实现本地提权。

### 关键技术点
1. QFQ aggregate访问缺乏充分锁保护
2. aggregate销毁与enqueue路径并发竞争
3. 并发flow删除和数据发送触发UAF
...
### 三阶段验证集成
- **Phase 1 (Prepare)**: 创建虚拟网络设备和QFQ qdisc配置、创建CTF目标文件、记录系统状态快照
- **Phase 2 (Run)**: 降权nobody执行PoC binary，通过QFQ aggregate竞争触发UAF并提权
