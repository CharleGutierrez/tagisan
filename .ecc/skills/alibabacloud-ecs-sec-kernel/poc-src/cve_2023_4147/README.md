...
| CVSS | 8.1 |
| 严重等级 | CRITICAL |
| 漏洞类型 | Use-After-Free / Double-Free |
| 影响内核版本 | 6.x |
| 利用技术 | io_uring IORING_OP_SOCKET操作中的双重释放 |
...
### 利用原理
io_uring子系统中IORING_OP_SOCKET操作在__sys_socket_file函数存在双重释放。需要特殊io_uring操作序列触发。正式PoC暂未公开发布。

该漏洞的触发条件为io_uring IORING_OP_SOCKET双重释放。攻击者通过精心构造的系统调用序列，利用Use-After-Free / Double-Free类型的缺陷，在内核空间中实现代码执行或数据篡改。

利用过程中，关键的内存操作为socket file对象double-free后重用。最终的提权路径通过通过io_uring double-free劫持内核执行流完成，使非特权用户获得root权限。

### 关键技术点
1. io_uring IORING_OP_SOCKET双重释放
2. socket file对象double-free后重用
3. 通过io_uring double-free劫持内核执行流

## PoC改造计划

### CTF挑战模式适配
- **推荐测试模式**: write_root_file
- **模式选择理由**: nftables规则UAF/Double-Free可通过堆喷实现任意内核写入和完整LPE
...
### 三阶段验证集成
- **Phase 1 (Prepare)**: 创建io_uring实例，准备SQE操作序列
- **Phase 2 (Run)**: 提交特殊IORING_OP_SOCKET序列触发double-free
- **Phase 3 (Post)**: 验证提权结果、销毁io_uring实例
...
```c
/*
 * CVE-2023-4147 PoC - CTF Challenge Mode
 *
 * Vulnerability: nf_tables chain insertion use-after-free
 *
...
static int mode_uaf(poc_args_t *args) {
    poc_log("Triggering CVE-2023-4147 UAF + spray corruption check...");
...
static int mode_write_root_file(const poc_args_t *args) {
    ...
    poc_log("Triggering CVE-2023-4147 UAF vulnerability...");
...
int main(int argc, char *argv[]) {
    ...
    poc_log("=== CVE-2023-4147 CTF PoC ===");
    poc_log("Vuln: nf_tables chain insertion use-after-free");
