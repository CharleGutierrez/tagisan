...
| 影响内核版本 | v5.15 ~ v6.8+ |
| 利用技术 | io_uring io_req_prep_async provided buffer rings的UAF |

## 原始PoC分析

### 参考来源
- **PoC URL**: https://github.com/google/security-research/tree/master/pocs/linux/kernelctf/CVE-2025-40364_lts_cos
- **来源类型**: Google security-research (kernelCTF)

### 利用原理
io_uring在异步request准备(io_req_prep_async)中对provided buffer管理存在缺陷。请求从同步路径转移到异步路径时buffer引用被错误释放导致UAF。

攻击者可通过io_uring操作序列——创建buffer ring、提交请求、触发async prep路径——利用buffer生命周期漏洞实现UAF实现本地提权。

### 关键技术点
1. io_req_prep_async中buffer ring引用处理错误
2. 异步路径转换时buffer被错误释放
3. io_uring操作序列可稳定触发UAF
...
- **Phase 2 (Run)**: 降权nobody执行PoC binary，通过io_uring async prep UAF实现提权
