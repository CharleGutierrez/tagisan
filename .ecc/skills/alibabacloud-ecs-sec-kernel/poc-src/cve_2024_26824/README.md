...
| CVSS | 7.8 |
| 严重等级 | HIGH |
| 漏洞类型 | Memory Corruption / Invalid Free |
| 影响内核版本 | v4.19 ~ v6.8 |
| 利用技术 | crypto algif_hash零长度输入时SGL错误释放 |

## 原始PoC分析

### 参考来源
- **PoC URL**: https://github.com/google/security-research/tree/master/pocs/linux/kernelctf/CVE-2024-26824_mitigation
- **来源类型**: Google security-research (kernelCTF)

### 利用原理
内核加密子系统algif_hash接口在处理零长度输入时对scatter-gather list进行了错误释放。零长度hash请求导致SGL指针状态不一致，对bogus指针的free可被利用实现对任意地址的kfree。

攻击者可通过AF_ALG socket接口发送精心构造的零长度hash请求触发对受控地址的free操作，结合堆喷射和fake object构造最终实现本地提权。

### 关键技术点
1. 零长度hash输入导致SGL指针状态不一致
2. 对未初始化SGL执行free产生可控的kfree
3. 通过AF_ALG接口无需特殊权限即可触发
...
### 三阶段验证集成
- **Phase 1 (Prepare)**: 加载crypto相关模块(algif_hash)、创建CTF目标文件、记录系统状态快照
- **Phase 2 (Run)**: 降权nobody执行PoC binary，通过algif_hash零长度SGL漏洞实现提权
...
Source: https://github.com/google/security-research/blob/master/pocs/linux/kernelctf/CVE-2024-26824_mitigation/exploit/mitigation-v4-6.6/exploit.c
