...
| 影响内核版本 | v5.7 ~ v6.7 |
| 利用技术 | TLS接收路径中异步解密与部分读取的竞争导致UAF |

## 原始PoC分析

### 参考来源
- **PoC URL**: https://github.com/google/security-research/tree/master/pocs/linux/kernelctf/CVE-2024-26582_mitigation
- **来源类型**: Google security-research (kernelCTF)

### 利用原理
内核TLS (kTLS)实现在process_rx_list处理部分读取请求时与异步解密操作存在竞争。当partial read触发async decrypt路径时，skb引用计数管理不当导致UAF。

攻击者可通过控制TLS连接的接收时机和读取大小精确触发该竞争窗口。利用UAF原语可实现内核堆对象的控制，最终实现本地提权。

### 关键技术点
1. TLS process_rx_list部分读取与async decrypt竞争
2. skb引用计数在并发路径中管理不当
3. 精确控制接收时机触发UAF实现提权
...
- **Phase 2 (Run)**: 降权nobody执行PoC binary，通过TLS partial read竞争触发UAF并提权
...
Source: https://github.com/google/security-research/blob/master/pocs/linux/kernelctf/CVE-2024-26582_mitigation/exploit/mitigation-v4-6.6/exploit.c
