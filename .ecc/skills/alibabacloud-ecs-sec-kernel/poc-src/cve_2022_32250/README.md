...
| 影响内核版本 | 5.15 ~ 5.18.1 |
| 利用技术 | nf_tables中NFT_STATEFUL_EXPR检查缺陷导致UAF |

## 原始PoC分析

### 参考来源
- **PoC URL**: https://github.com/ysanatomic/CVE-2022-32250-LPE/blob/main/exploit.c
- **来源类型**: GitHub

### 利用原理
net/netfilter/nf_tables_api.c中不正确的NFT_STATEFUL_EXPR检查导致释放后使用漏洞。需要user/net namespace创建权限。

该漏洞的触发条件为nf_tables NFT_STATEFUL_EXPR检查缺陷。攻击者通过精心构造的系统调用序列，利用UAF (Use-After-Free)类型的缺陷，在内核空间中实现代码执行或数据篡改。

利用过程中，关键的内存操作为UAF后nft expression对象重用。最终的提权路径通过通过nf_tables UAF劫持内核执行流实现提权完成，使非特权用户获得root权限。

### 关键技术点
1. nf_tables NFT_STATEFUL_EXPR检查缺陷
2. UAF后nft expression对象重用
3. 通过nf_tables UAF劫持内核执行流实现提权

## PoC改造计划

### CTF挑战模式适配
- **推荐测试模式**: write_root_file
- **模式选择理由**: nf_tables UAF可通过堆喷控制freed对象实现任意内核写入和完整LPE
...
### 三阶段验证集成
- **Phase 1 (Prepare)**: 创建netfilter namespace环境，准备nft规则
- **Phase 2 (Run)**: 触发NFT_STATEFUL_EXPR UAF实现提权
...
Source: https://github.com/ysanatomic/CVE-2022-32250-LPE/blob/main/exploit.c
