...
| 影响内核版本 | 5.x ~ 6.5.x |
| 利用技术 | perf_event系统中read_size整数溢出导致堆溢出 |

## 原始PoC分析

### 参考来源
- **PoC URL**: https://github.com/google/security-research/blob/master/pocs/linux/kernelctf/CVE-2023-6931_lts_cos/docs/exploit.md
- **来源类型**: GitHub

### 利用原理
Performance Events子系统中perf_read_group函数的read_size计算存在整数溢出，导致堆越界写。需要特殊perf_event配置组合触发。

该漏洞的触发条件为perf_read_group read_size整数溢出。攻击者通过精心构造的系统调用序列，利用Heap Out-of-Bounds Write类型的缺陷，在内核空间中实现代码执行或数据篡改。

利用过程中，关键的内存操作为堆越界写入覆盖相邻内核对象。最终的提权路径通过通过perf_event堆溢出修改内核数据实现提权完成，使非特权用户获得root权限。

### 关键技术点
1. perf_read_group read_size整数溢出
2. 堆越界写入覆盖相邻内核对象
3. 通过perf_event堆溢出修改内核数据实现提权
...
### 三阶段验证集成
- **Phase 1 (Prepare)**: 创建特殊perf_event配置组合
- **Phase 2 (Run)**: 通过read_size溢出触发堆越界写入实现提权
- **Phase 3 (Post)**: 验证提权结果、关闭perf_event fd
...
Source: https://github.com/google/security-research/blob/master/pocs/linux/kernelctf/CVE-2023-6931_lts_cos/exploit/cos-105-17412.226.28/exploit.c
...
#define SOCK_DEF_WRITE_SPACE 0xbf0f50
#define __SK_DESTRUCT 0xbf2530
#define RTNETLINK_BIND 0xc27770
#define RTNETLINK_BIND_RET 0xc2777f
