...
| 影响内核版本 | v4.15 ~ v6.8+ |
| 利用技术 | qdisc class emptying时通知错误导致状态不一致 |

## 原始PoC分析

### 参考来源
- **PoC URL**: https://github.com/google/security-research/tree/master/pocs/linux/kernelctf/CVE-2025-38350_cos
- **来源类型**: Google security-research (kernelCTF)

### 利用原理
网络调度器在子class变空时通知机制存在缺陷，错误更新parent内部计数器或指针导致qdisc状态不一致。后续enqueue/dequeue操作放大不一致最终导致内存损坏。

攻击者可构造特定qdisc树结构精确控制数据流使child class在特定时刻变空，利用错误通知机制触发状态不一致实现内存损坏和本地提权。

### 关键技术点
1. child class emptying通知逻辑错误
2. parent qdisc内部状态被错误更新
3. qdisc树嵌套结构放大状态不一致效果
...
### 三阶段验证集成
- **Phase 1 (Prepare)**: 创建虚拟网络设备和嵌套qdisc结构、创建CTF目标文件、记录系统状态快照
- **Phase 2 (Run)**: 降权nobody执行PoC binary，通过class emptying通知错误触发内存损坏并提权
