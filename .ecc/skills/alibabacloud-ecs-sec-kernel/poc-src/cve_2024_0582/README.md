...
| CVSS | 7.8 |
| 严重等级 | HIGH |
| 漏洞类型 | Use-After-Free / Memory Mapping Error |
| 影响内核版本 | v6.4 ~ v6.6.5 |
| 利用技术 | io_uring PBUF_RING mmap后unmap导致页面UAF |

## 原始PoC分析

### 参考来源
- **PoC URL**: https://github.com/ysanatomic/io_uring_LPE-CVE-2024-0582/blob/main/exploit.c
- **来源类型**: GitHub (ysanatomic)

### 利用原理
io_uring的provided buffer ring机制在处理mmap映射时存在生命周期管理缺陷。当用户空间映射provided buffer ring后，在ring仍注册时进行munmap操作，后续io_uring操作将访问已释放页面。

攻击者可通过此UAF原语实现页面级别任意读写：先触发UAF释放目标页面，然后通过页面分配器重新分配给敏感内核对象，最终修改这些对象实现提权。该漏洞利用稳定性高，成功率极佳。

### 关键技术点
1. io_uring PBUF_RING映射后munmap不会取消注册
2. 释放后的物理页面可被重新分配给内核对象
3. 通过io_uring读写已被回收的物理页面实现提权
...
### 三阶段验证集成
- **Phase 1 (Prepare)**: 验证内核版本在漏洞范围内、创建CTF目标文件、记录系统状态快照
- **Phase 2 (Run)**: 降权nobody执行PoC binary，通过io_uring buffer ring UAF实现提权写入
...
Source: https://github.com/ysanatomic/io_uring_LPE-CVE-2024-0582/blob/main/exploit.c
