...
| CVSS | 7.8 |
| 严重等级 | HIGH |
| 漏洞类型 | Double-Free / Memory Corruption |
| 影响内核版本 | v5.6 ~ v6.8 |
| 利用技术 | netfilter pipapo克隆集合销毁路径中的元素双重释放 |

## 原始PoC分析

### 参考来源
- **PoC URL**: https://github.com/google/security-research/tree/master/pocs/linux/kernelctf/CVE-2024-26809_lts_cos
- **来源类型**: Google security-research (kernelCTF)

### 利用原理
nft_pipapo集合类型在克隆集合的销毁路径中存在元素清理逻辑错误。pipapo集合的clone操作复制元素引用但不增加引用计数，clone set destroy时再次释放产生double-free。

这个double-free原语非常稳定，攻击者可通过nftables事务操作精确控制clone和destroy时机。利用double-free实现堆布局控制，最终覆写关键内核数据结构实现本地提权。

### 关键技术点
1. pipapo set clone不增加元素引用计数
2. clone set destroy路径双重释放元素
3. stable double-free原语实现堆控制
...
- **Phase 2 (Run)**: 降权nobody执行PoC binary，通过pipapo clone destroy触发double-free并提权
...
Source: https://github.com/google/security-research/blob/master/pocs/linux/kernelctf/CVE-2024-26809_lts_cos/exploit/cos-105-17412.294.34/exploit.c
