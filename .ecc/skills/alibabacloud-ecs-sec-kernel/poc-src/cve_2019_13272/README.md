...
| 影响内核版本 | 3.x ~ 5.1.16 |
| 利用技术 | ptrace_link凭证处理缺陷，通过PTRACE_TRACEME触发 |

## 原始PoC分析

### 参考来源
- **PoC URL**: https://github.com/rapid7/metasploit-framework/blob/master/data/exploits/CVE-2019-13272/poc.c
- **来源类型**: GitHub

### 利用原理
kernel/ptrace.c中ptrace_link函数错误处理进程凭证记录，允许通过pkexec+PTRACE_TRACEME获取root权限。

该漏洞的触发条件为PTRACE_TRACEME凭证处理缺陷。攻击者通过精心构造的系统调用序列，利用UAF (Use-After-Free)类型的缺陷，在内核空间中实现代码执行或数据篡改。

利用过程中，关键的内存操作为cred结构体引用计数管理错误导致UAF。最终的提权路径通过通过pkexec+PTRACE_TRACEME继承特权凭证完成，使非特权用户获得root权限。

### 关键技术点
1. PTRACE_TRACEME凭证处理缺陷
2. cred结构体引用计数管理错误导致UAF
3. 通过pkexec+PTRACE_TRACEME继承特权凭证

## PoC改造计划

### CTF挑战模式适配
- **推荐测试模式**: write_root_file
- **模式选择理由**: ptrace凭证操作漏洞可直接获得root权限，具备任意文件写入能力
...
### 三阶段验证集成
- **Phase 1 (Prepare)**: 检查ptrace权限和pkexec可用性
- **Phase 2 (Run)**: fork子进程通过PTRACE_TRACEME获取父进程特权凭证
- **Phase 3 (Post)**: 验证提权结果、清理子进程
...
Source: https://bugs.chromium.org/p/project-zero/issues/detail?id=1903
