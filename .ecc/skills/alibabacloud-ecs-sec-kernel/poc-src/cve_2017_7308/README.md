...
| 影响内核版本 | 3.x ~ 4.10.6 |
| 利用技术 | AF_PACKET packet_set_ring函数中ring buffer配置校验缺陷 |

## 原始PoC分析

### 参考来源
- **PoC URL**: https://github.com/bcoles/kernel-exploits/blob/master/CVE-2017-7308/poc.c
- **来源类型**: GitHub

### 利用原理
packet_set_ring函数对block-size数据验证不足，允许越界写入。包含KASLR/SMEP/SMAP绕过。

该漏洞的触发条件为AF_PACKET ring buffer block-size校验缺陷。攻击者通过精心构造的系统调用序列，利用Heap Out-of-Bounds Write类型的缺陷，在内核空间中实现代码执行或数据篡改。

利用过程中，关键的内存操作为堆越界写入覆盖相邻slab对象。最终的提权路径通过通过OOB write控制内核函数指针实现提权完成，使非特权用户获得root权限。

### 关键技术点
1. AF_PACKET ring buffer block-size校验缺陷
2. 堆越界写入覆盖相邻slab对象
3. 通过OOB write控制内核函数指针实现提权
...
### 三阶段验证集成
- **Phase 1 (Prepare)**: 创建AF_PACKET套接字，准备ring buffer配置
- **Phase 2 (Run)**: 设置恶意block-size触发堆越界写入实现提权
- **Phase 3 (Post)**: 验证提权结果、释放ring buffer、关闭套接字
...
Source: https://github.com/xairy/kernel-exploits/tree/master/CVE-2017-7308
...
void packet_socket_rx_ring_init(int s, unsigned int block_size,
	...
	if (rv < 0) {
		dprintf("[-] setsockopt(PACKET_VERSION): %m\n");
		exit(EXIT_FAILURE);
	}
	...
	if (rv < 0) {
		dprintf("[-] setsockopt(PACKET_RX_RING): %m\n");
		exit(EXIT_FAILURE);
	}
}
...
int main(int argc, char *argv[]) {
	...
	dprintf("Linux Kernel AF_PACKET packet_set_ring heap out-of-bounds write local root (CVE-2017-7308)\n");
