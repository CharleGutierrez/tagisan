...
| 影响内核版本 | 5.x ~ 6.6.x |
| 利用技术 | nf_tables中PIPAPO set删除时chain use count双重递减 |

## 原始PoC分析

### 参考来源
- **PoC URL**: https://github.com/google/security-research/blob/master/pocs/linux/kernelctf/CVE-2023-6817_mitigation/docs/exploit.md
- **来源类型**: GitHub

### 利用原理
nf_tables nftables组件中PIPAPO set删除时chain引用计数错误递减，导致chain对象UAF。Google kCTF mitigated-v3版本exploit包含多层防护绕过。

该漏洞的触发条件为nf_tables PIPAPO set删除chain引用计数错误。攻击者通过精心构造的系统调用序列，利用UAF (Use-After-Free)类型的缺陷，在内核空间中实现代码执行或数据篡改。

利用过程中，关键的内存操作为chain对象UAF后通过堆喷射重用。最终的提权路径通过通过chain UAF劫持内核执行流实现提权完成，使非特权用户获得root权限。

### 关键技术点
1. nf_tables PIPAPO set删除chain引用计数错误
2. chain对象UAF后通过堆喷射重用
3. 通过chain UAF劫持内核执行流实现提权

## PoC改造计划

### CTF挑战模式适配
- **推荐测试模式**: write_root_file
- **模式选择理由**: nftables集合元素UAF可通过堆喷实现任意内核写入和完整LPE
...
### 三阶段验证集成
- **Phase 1 (Prepare)**: 创建netfilter namespace，配置PIPAPO set和chain
- **Phase 2 (Run)**: 删除PIPAPO set触发chain引用计数双重递减UAF
...
### 核心修改
1. 适配kCTF mitigated exploit为CTF模式
2. 添加CLI参数解析
3. 添加多层防护绕过适配
...
Source: https://github.com/google/security-research/blob/master/pocs/linux/kernelctf/CVE-2023-6817_mitigation/exploit/mitigation-v3-6.1.55/exploit.c
