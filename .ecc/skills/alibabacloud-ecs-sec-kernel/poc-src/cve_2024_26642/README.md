...
| CVSS | 7.8 |
| 严重等级 | HIGH |
| 漏洞类型 | Resource Exhaustion / Logic Error |
| 影响内核版本 | v5.6 ~ v6.8 |
| 利用技术 | nf_tables匿名集合timeout标志处理错误 |

## 原始PoC分析

### 参考来源
- **PoC URL**: https://github.com/google/security-research/tree/master/pocs/linux/kernelctf/CVE-2024-26642_cos
- **来源类型**: Google security-research (kernelCTF)

### 利用原理
nf_tables子系统在处理匿名集合(anonymous set)的timeout标志时存在验证缺陷。匿名集合本不应支持timeout功能，但缺少验证允许用户设置timeout标志触发GC对这些集合的不当处理。

利用该漏洞可导致资源耗尽或触发内存损坏。通过精心构造的nftables规则序列，攻击者可利用这种状态不一致实现权限提升。

### 关键技术点
1. 匿名集合不应支持timeout但缺少验证
2. timeout触发GC对匿名集合的不当处理
3. 状态不一致可导致内存损坏和提权
...
- **Phase 2 (Run)**: 降权nobody执行PoC binary，通过nftables匿名集合timeout漏洞实现提权
...
Source: https://github.com/google/security-research/blob/master/pocs/linux/kernelctf/CVE-2024-26642_cos/exploit/cos-105-17412.294.23/exploit.c
