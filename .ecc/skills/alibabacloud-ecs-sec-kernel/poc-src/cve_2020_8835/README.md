...
| 影响内核版本 | 5.5.0 ~ 5.8.x |
| 利用技术 | eBPF verifier对32位操作寄存器边界计算错误 |

## 原始PoC分析

### 参考来源
- **PoC URL**: https://github.com/zilong3033/CVE-2020-8835/blob/main/poc.c
- **来源类型**: GitHub

### 利用原理
kernel/bpf/verifier.c中对32位操作的寄存器边界限制不正确，允许绕过验证器获得任意读写。

该漏洞的触发条件为BPF verifier 32位寄存器边界计算错误。攻击者通过精心构造的系统调用序列，利用BPF Verifier Bypass类型的缺陷，在内核空间中实现代码执行或数据篡改。

利用过程中，关键的内存操作为绕过验证器后通过BPF map实现任意内核读写。最终的提权路径通过利用BPF任意读写修改task credentials完成，使非特权用户获得root权限。

### 关键技术点
1. BPF verifier 32位寄存器边界计算错误
2. 绕过验证器后通过BPF map实现任意内核读写
3. 利用BPF任意读写修改task credentials
...
### 三阶段验证集成
- **Phase 1 (Prepare)**: 检查BPF可用性和unprivileged_bpf权限
- **Phase 2 (Run)**: 加载恶意BPF程序绕过32位边界检查实现提权
...
Source: https://github.com/zilong3033/CVE-2020-8835/blob/main/poc.c
