...
| 漏洞类型 | Use-After-Free |
| 影响内核版本 | v5.14 ~ v6.6 |
| 利用技术 | nftables verdict处理中的引用计数错误导致UAF |

## 原始PoC分析

### 参考来源
- **PoC URL**: https://github.com/Notselwyn/CVE-2024-1086/blob/main/src/main.c
- **来源类型**: GitHub (Notselwyn)

### 利用原理
nf_tables在处理ruleset替换时verdict对象的引用计数管理存在缺陷。nft_verdict_init和nft_verdict_destroy之间的引用计数不对称，导致chain对象可能被提前释放而仍有verdict引用指向该chain。

Notselwyn的exploit实现了极高的成功率。利用过程包括：触发chain对象的UAF、通过页级堆喷射(page-level heap spray)控制释放后的内存、构造fake对象实现任意读写、最终修改进程凭证实现提权。该漏洞已被勒索软件组织积极利用。

### 关键技术点
1. nftables verdict对象引用计数不对称导致chain UAF
2. 页级堆喷射(page-level heap spray)控制释放内存
3. 修改cred结构体实现稳定提权
...
### 三阶段验证集成
- **Phase 1 (Prepare)**: 加载nf_tables内核模块、验证用户命名空间可用、创建CTF目标文件、记录系统状态快照
- **Phase 2 (Run)**: 降权nobody执行PoC binary，通过nftables verdict UAF实现提权写入
- **Phase 3 (Post)**: 验证CTF flag写入结果、清理nftables规则和集合、清理临时文件、恢复系统状态
...
Source: https://github.com/Notselwyn/CVE-2024-1086/blob/main/src/main.c
