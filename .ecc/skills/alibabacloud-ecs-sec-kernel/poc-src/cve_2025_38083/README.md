...
| 影响内核版本 | v4.15 ~ v6.8+ |
| 利用技术 | PRIO qdisc prio_tune()中优先级带重配置的竞速 |

## 原始PoC分析

### 参考来源
- **PoC URL**: https://github.com/google/security-research/tree/master/pocs/linux/kernelctf/CVE-2025-38083_cos_mitigation
- **来源类型**: Google security-research (kernelCTF)

### 利用原理
PRIO qdisc在并发执行prio_tune()重配置优先级带时缺乏同步保护。数据路径正在访问旧子qdisc时配置路径释放旧数组产生竞争导致UAF。

攻击者可并发执行PRIO qdisc配置变更和数据包发送操作，利用prio_tune()中竞争窗口触发子qdisc UAF实现本地提权。

### 关键技术点
1. prio_tune()并发执行时缺乏同步保护
2. 优先级带重配置与数据路径竞争
3. 子qdisc数组替换期间的UAF
...
### 三阶段验证集成
- **Phase 1 (Prepare)**: 创建虚拟网络设备和PRIO qdisc、创建CTF目标文件、记录系统状态快照
- **Phase 2 (Run)**: 降权nobody执行PoC binary，通过PRIO prio_tune竞争触发子qdisc UAF并提权
