...
| 影响内核版本 | v5.6 ~ v6.8 |
| 利用技术 | nf_tables GC序列中的mutex释放时序错误 |

## 原始PoC分析

### 参考来源
- **PoC URL**: https://github.com/google/security-research/tree/master/pocs/linux/kernelctf/CVE-2024-26925_lts_cos
- **来源类型**: Google security-research (kernelCTF)

### 利用原理
nf_tables的GC机制与规则修改操作之间存在mutex排序问题。GC序列执行中释放mutex时机不当，允许并发规则修改在GC完成前修改被回收对象导致UAF。

攻击者可通过精心设计带有timeout的set元素并发触发GC和规则修改操作，利用mutex释放窗口实现稳定的UAF并通过堆喷射最终实现本地提权。

### 关键技术点
1. GC序列中mutex释放时机允许并发修改
2. GC处理超时元素时被并发删除导致UAF
3. 通过并发nftables操作稳定触发竞争窗口
...
### 三阶段验证集成
- **Phase 1 (Prepare)**: 加载nf_tables模块、创建CTF目标文件、记录系统状态快照
- **Phase 2 (Run)**: 降权nobody执行PoC binary，通过nftables GC竞争触发UAF并提权
...
Source: https://github.com/google/security-research/blob/master/pocs/linux/kernelctf/CVE-2024-26925_lts_cos/exploit/lts-6.1.81/exp.c
