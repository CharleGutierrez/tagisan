...
| 影响内核版本 | v5.6 ~ v6.8 |
| 利用技术 | netfilter nft_set元素timer与并发操作竞争 |

## 原始PoC分析

### 参考来源
- **PoC URL**: https://github.com/google/security-research/tree/master/pocs/linux/kernelctf/CVE-2024-27397_mitigation
- **来源类型**: Google security-research (kernelCTF)

### 利用原理
nf_tables集合元素的timeout定时器与并发元素操作存在竞争条件。定时器回调尝试删除超时元素时另一CPU可能正在访问该元素，由于缺乏同步机制导致UAF。

利用需要创建带有短超时值的set元素，在元素即将超时时发起并发查询操作。通过精确时序控制和多CPU并发可稳定触发UAF实现本地提权。

### 关键技术点
1. set元素timeout回调与并发访问缺乏同步
2. 定时器释放元素时另一CPU正在访问
3. 短超时值配合并发操作稳定触发竞争
...
- **Phase 2 (Run)**: 降权nobody执行PoC binary，通过nft_set timer竞争触发UAF并提权
...
Source: https://github.com/google/security-research/blob/master/pocs/linux/kernelctf/CVE-2024-27397_mitigation/exploit/mitigation-v3-6.1.55/exploit.c
