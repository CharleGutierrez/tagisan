"""CVE-2026-43284 Post phase handler
...
class CVE202643284PostHandler(BasePostHandler):
    """CVE-2026-43284 Post phase: verify CTF result, clean up, verify system state"""
    ...
    def global_verify(self, prepare_result: PrepareResult) -> GlobalVerifyResult:
        ...
        # 4. Check dmesg for xfrm/esp related anomalies
    ...
    def _verify_ctf_result(self, state_snapshot: dict) -> bool:
        ...
        if test_mode == "read_root_file":
            ...
            try:
                ...
                if actual_inner and actual_inner == expected_inner:
                    logger.info("CTF write_root_file PASSED: inner value matches")
                    return True
                elif initial_content and content == initial_content.strip():
                    logger.warning("CTF write_root_file FAILED: file still has initial content")
                    return False
                else:
                    logger.warning("CTF write_root_file FAILED: expected=%s, got=%s",
                                   expected_inner[:16], actual_inner[:16] if actual_inner else "")
                    return False
            except OSError as e:
