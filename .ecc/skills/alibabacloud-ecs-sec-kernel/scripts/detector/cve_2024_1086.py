"""
CVE-2024-1086 Detector - netfilter nf_tables use-after-free via nft_verdict_init()

VulnerabilityDescription:
    netfilter nf_tables use-after-free via nft_verdict_init() - Local Privilege Escalation

ImpactVersion: 3.15.0 <= kernel < 6.8.0
CVSS: 7.8 (HIGH)
...
class CVE20241086Detector(BaseDetector):
    """CVE-2024-1086 (netfilter nf_tables use-after-free via nft_verdict) Detector"""

    cve_id = "CVE-2024-1086"
    cvss_score = 7.8
    severity = "HIGH"
    description = (
        "netfilter nf_tables use-after-free via nft_verdict_init() - "
        "Local Privilege Escalation"
    )
    vuln_type = "local_privilege_escalation"
    affected_versions = {
        "generic": {"min": "3.15.0", "fixed": "6.8.0"},
    }

    has_poc = True
    has_exp = False
    poc_bin = "poc-bin/cve_2024_1086.bin"
    poc_mode = "uaf"

    _MODULE_NAME = "nf_tables"
    _CONFIG_KEY = "CONFIG_NF_TABLES"
    _MIN_VERSION = "3.15.0"
    _FIXED_VERSION = "6.8.0"
    ...
    def get_remediation(self) -> Remediation:
        return Remediation(
            ...
            steps=[
                ...
                "",
                "# 3. Verify fix",
                "uname -r  # Confirm new kernel version >= 6.8.0",
            ],
            backup_steps=[
            ...
            verify_steps=[
                "uname -r  # Confirm kernel version >= 6.8.0",
            ]
        )
    ...
    def _get_poc_verifier(self):
        from ..poc.cve_2024_1086_poc import CVE20241086PoCVerifier
        return CVE20241086PoCVerifier()

    def prepare(self, kernel_info: KernelInfo) -> PrepareResult:
        from .cve_2024_1086_prepare import CVE20241086PrepareHandler
        handler = CVE20241086PrepareHandler(timeout=30)
        return handler.execute()

    def post(self, kernel_info: KernelInfo,
             prepare_result: Optional[PrepareResult] = None) -> PostResult:
        from .cve_2024_1086_post import CVE20241086PostHandler
        handler = CVE20241086PostHandler(
