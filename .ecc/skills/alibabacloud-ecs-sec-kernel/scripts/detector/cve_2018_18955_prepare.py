"""
CVE-2018-18955 Prepare phase handler

Record system state snapshot, create temp test directory.
user namespace isCoreKernelSuccesscan，noneedLoadAdditionalModule。
"""
import logging
import os
import subprocess
...
class CVE201818955PrepareHandler(BasePrepareHandler):
    """CVE-2018-18955 Prepare phase: record system state and create test environment"""
    ...
    def get_operations(self, kernel_info: dict) -> List[PrepareOperation]:
        ...
        # Operation 2: RecordCurrent sysctl Value
        operations.append(PrepareOperation(
            action="snapshot",
            target="sysctl_userns",
            result="pending",
        ))
    ...
    def _execute_operation(self, op: PrepareOperation) -> PrepareOperation:
        ...
        elif op.action == "snapshot" and op.target == "sysctl_userns":
            return self._snapshot_sysctl_userns(op)
        else:
    ...
    def _snapshot_sysctl_userns(self, op: PrepareOperation) -> PrepareOperation:
        """Record user namespace sysctl CurrentValue"""
        try:
            result = subprocess.run(
            ...
            if result.returncode == 0:
                op.result = f"success (value={result.stdout.strip()})"
            else:
                op.result = "success (sysctl not readable)"
        except (subprocess.TimeoutExpired, FileNotFoundError, OSError):
            op.result = "success (sysctl unavailable)"
