"""Detector for CVE-2023-6817 - netfilter nft_set use-after-free on flag update. Updating set element flags with concurrent set destruction causes UAF, enabling local privilege escalation."""
import logging
...
class CVE20236817Detector(BaseDetector):
    cve_id = "CVE-2023-6817"
    severity = "CRITICAL"
    cvss_score = 7.8
    has_poc = True
    poc_mode = "uaf"
    description = "netfilter nft_set use-after-free on flag update. Updating set element flags with concurrent set destruction causes UAF, enabling local privilege escalation."
    ...
    def get_remediation(self):
        """Return remediation guidance for CVE-2023-6817."""
        from ..core.result import Remediation
        return Remediation(
            priority="IMMEDIATE",
            steps=[
                "Upgrade kernel to version >= 6.7 (nf_tables set element deactivation UAF fix).",
    ...
    def _get_poc_verifier(self):
        from ..poc.cve_2023_6817_poc import CVECve20236817PoCVerifier
        return CVECve20236817PoCVerifier()
