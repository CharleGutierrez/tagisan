"""
CVE-2022-0847 Detector - Dirty Pipe - improper initialization in pipe buffer allowing writing to read-only files

VulnerabilityDescription:
    Dirty Pipe - improper initialization in pipe buffer allowing writing to read-only files - Local Privilege Escalation

ImpactVersion: 5.8.0 <= kernel < 5.16.11
CVSS: 7.8 (HIGH)
exploitationCondition: CONFIG_PIPE Enable

detectionMethod（全程Read-only）:
    1. KernelVersionRangeMatch
    2. CONFIG_PIPE Configuration check
...
class CVE20220847Detector(BaseDetector):
    """CVE-2022-0847 (Dirty Pipe - improper initialization in pipe buffe) Detector"""

    cve_id = "CVE-2022-0847"
    cvss_score = 7.8
    severity = "HIGH"
    description = (
        "Dirty Pipe - improper initialization in pipe buffer allowing writing to read-only files - "
        "Local Privilege Escalation"
    )
    vuln_type = "local_privilege_escalation"
    affected_versions = {
        "generic": {"min": "5.8.0", "fixed": "5.16.11"},
    }

    has_poc = True
    has_exp = False
    poc_bin = "poc-bin/cve_2022_0847.bin"
    poc_mode = "write_root_file"

    _MODULE_NAME = "pipe"
    _CONFIG_KEY = "CONFIG_PIPE"
    _MIN_VERSION = "5.8.0"
    _FIXED_VERSION = "5.16.11"
    ...
    def detect(self, kernel_info: KernelInfo) -> DetectResult:
        ...
        if config_value:
            evidence_list.append(Evidence(
                type="config_enabled",
                description=f"{self._CONFIG_KEY}={config_value} - pipe enabled",
    ...
    def get_remediation(self) -> Remediation:
        return Remediation(
            ...
            steps=[
                ...
                "",
                "# 3. Verify fix",
                "uname -r  # Confirm new kernel version >= 5.16.11",
            ],
            backup_steps=[
            ...
            verify_steps=[
                "uname -r  # Confirm kernel version >= 5.16.11",
            ]
        )
    ...
    def _get_poc_verifier(self):
        from ..poc.cve_2022_0847_poc import CVE20220847PoCVerifier
        return CVE20220847PoCVerifier()

    def prepare(self, kernel_info: KernelInfo) -> PrepareResult:
        from .cve_2022_0847_prepare import CVE20220847PrepareHandler
        handler = CVE20220847PrepareHandler(timeout=30)
        return handler.execute()

    def post(self, kernel_info: KernelInfo,
             prepare_result: Optional[PrepareResult] = None) -> PostResult:
        from .cve_2022_0847_post import CVE20220847PostHandler
        handler = CVE20220847PostHandler(
        ...
        if not force_run:
            ...
            if not config_enabled and kernel_info.config is not None:
                return PoCResult(
                    ...
                    error_message="CONFIG_PIPE not enabled"
                )
