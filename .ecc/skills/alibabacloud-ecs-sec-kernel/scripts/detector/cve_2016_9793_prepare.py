"""
CVE-2016-9793 Prepare phase handler

Create root-owned target file for CTF challenge.
CVE-2016-9793 is a SO_SNDBUFFORCE/SO_RCVBUFFORCE integer overflow vulnerability
leading to heap memory corruption.

For CTF verification:
- write_root_file: PoC attempts to use socket buffer overflow to bypass VFS
  write permissions via heap corruption
...
If the PoC manages to modify content via heap corruption path, it proves the
...
class CVE20169793PrepareHandler(BasePrepareHandler):
    """CVE-2016-9793 Prepare phase: create root-owned CTF target"""
    ...
    def _create_target_file(self, op: PrepareOperation) -> PrepareOperation:
        ...
        CVE-2016-9793 is a heap corruption vulnerability.
