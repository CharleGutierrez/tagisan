"""
CVE-2017-16995 Prepare phase handler

Create root-owned target file for CTF challenge.
CVE-2017-16995 is an eBPF verifier ALU32 range tracking bypass vulnerability
leading to arbitrary kernel memory access and local privilege escalation.

For CTF verification:
- write_root_file: PoC attempts to use eBPF verifier bypass + OOB access
  to bypass VFS write permissions (simplified detection path)
...
If the PoC manages to modify content via the eBPF OOB access path, it proves
...
class CVE201716995PrepareHandler(BasePrepareHandler):
    """CVE-2017-16995 Prepare phase: create root-owned CTF target"""
    ...
    def _create_target_file(self, op: PrepareOperation) -> PrepareOperation:
        ...
        CVE-2017-16995 is an eBPF verifier bypass vulnerability.
        For CTF: write mode uses 0644 (nobody can read but not write).

        File must be >= 4096 bytes (one page) for mmap operations.
        """
        target_path = op.target
        try:
            if self._test_mode == "write_root_file":
                # Write initial content that PoC will attempt to overwrite
    ...
    def _snapshot_system_state(self, op: PrepareOperation) -> PrepareOperation:
        """Record system state snapshot (kernel version, BPF status)"""
        snapshot_parts = []
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                snapshot_parts.append(f"unprivileged_bpf_disabled={val}")
            else:
                snapshot_parts.append("unprivileged_bpf_disabled=unknown")
        except (subprocess.TimeoutExpired, OSError):
            snapshot_parts.append("unprivileged_bpf_disabled=unavailable")
