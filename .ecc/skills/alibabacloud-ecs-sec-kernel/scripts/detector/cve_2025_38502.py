"""Detector for CVE-2025-38502 - BPF cgroup local storage map out-of-bounds access. Incorrect bounds check
in cgroup local storage operations allows OOB read/write, enabling local
...
class CVE202538502Detector(BaseDetector):
    cve_id = "CVE-2025-38502"
    severity = "CRITICAL"
    cvss_score = 7.8
    has_poc = True
    poc_mode = "uaf"
    description = "BPF cgroup local storage map out-of-bounds access. Incorrect bounds check in cgroup local storage operations allows OOB read/write, enabling local privilege escalation via crafted BPF programs."
    ...
    def get_remediation(self):
        """Return remediation guidance for CVE-2025-38502."""
        from ..core.result import Remediation
        return Remediation(
            priority="IMMEDIATE",
            steps=[
                "Upgrade kernel to version >= 6.15 (bpf verifier scalar precision tracking fix).",
    ...
    def _get_poc_verifier(self):
        from ..poc.cve_2025_38502_poc import CVECve202538502PoCVerifier
        return CVECve202538502PoCVerifier()
