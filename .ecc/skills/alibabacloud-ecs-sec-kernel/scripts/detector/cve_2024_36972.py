"""Detector for CVE-2024-36972 - AF_UNIX race condition updating oob_skb without sk_receive_queue lock.
Concurrent access to out-of-band socket buffer without proper locking
leads to use-after-free and local privilege escalation.
"""
import logging
...
class CVE202436972Detector(BaseDetector):
    cve_id = "CVE-2024-36972"
    severity = "CRITICAL"
    cvss_score = 7.8
    has_poc = True
    poc_mode = "uaf"
    description = "AF_UNIX race condition updating oob_skb without sk_receive_queue lock. Concurrent access to out-of-band socket buffer without proper locking leads to use-after-free and local privilege escalation."
    ...
    def get_remediation(self):
        """Return remediation guidance for CVE-2024-36972."""
        from ..core.result import Remediation
        return Remediation(
            priority="IMMEDIATE",
            steps=[
                "Upgrade kernel to version >= 6.10 (af_unix OOB recv queue race fix).",
    ...
    def _get_poc_verifier(self):
        from ..poc.cve_2024_36972_poc import CVECve202436972PoCVerifier
        return CVECve202436972PoCVerifier()
