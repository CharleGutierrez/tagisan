"""
CVE-2023-0386 Detector - OverlayFS fuse file copy_up does not check permissions correctly

VulnerabilityDescription:
    OverlayFS fuse file copy_up does not check permissions correctly - Local Privilege Escalation

ImpactVersion: 5.11.0 <= kernel < 6.2.0
CVSS: 7.8 (HIGH)
exploitationCondition: CONFIG_OVERLAY_FS Enable

detectionMethod（全程Read-only）:
    1. KernelVersionRangeMatch
    2. CONFIG_OVERLAY_FS Configuration check
...
class CVE20230386Detector(BaseDetector):
    """CVE-2023-0386 (OverlayFS fuse file copy_up does not check permiss) Detector"""

    cve_id = "CVE-2023-0386"
    cvss_score = 7.8
    severity = "HIGH"
    description = (
        "OverlayFS fuse file copy_up does not check permissions correctly - "
        "Local Privilege Escalation"
    )
    vuln_type = "local_privilege_escalation"
    affected_versions = {
        "generic": {"min": "5.11.0", "fixed": "6.2.0"},
    }

    has_poc = True
    has_exp = False
    poc_bin = "poc-bin/cve_2023_0386.bin"
    poc_mode = "write_root_file"

    _MODULE_NAME = "overlay"
    _CONFIG_KEY = "CONFIG_OVERLAY_FS"
    _MIN_VERSION = "5.11.0"
    _FIXED_VERSION = "6.2.0"
    ...
    def detect(self, kernel_info: KernelInfo) -> DetectResult:
        ...
        if config_value:
            evidence_list.append(Evidence(
                type="config_enabled",
                description=f"{self._CONFIG_KEY}={config_value} - overlay enabled",
    ...
    def get_remediation(self) -> Remediation:
        return Remediation(
            ...
            steps=[
                ...
                "",
                "# 3. Verify fix",
                "uname -r  # Confirm new kernel version >= 6.2.0",
            ],
            backup_steps=[
            ...
            temporary_mitigation="disable overlay module",
            ...
            verify_steps=[
                "uname -r  # Confirm kernel version >= 6.2.0",
            ]
        )
    ...
    def _get_poc_verifier(self):
        from ..poc.cve_2023_0386_poc import CVE20230386PoCVerifier
        return CVE20230386PoCVerifier()

    def prepare(self, kernel_info: KernelInfo) -> PrepareResult:
        from .cve_2023_0386_prepare import CVE20230386PrepareHandler
        handler = CVE20230386PrepareHandler(timeout=30)
        return handler.execute()

    def post(self, kernel_info: KernelInfo,
             prepare_result: Optional[PrepareResult] = None) -> PostResult:
        from .cve_2023_0386_post import CVE20230386PostHandler
        handler = CVE20230386PostHandler(
