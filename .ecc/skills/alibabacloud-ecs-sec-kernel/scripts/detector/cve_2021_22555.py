"""
CVE-2021-22555 Detector - netfilter x_tables heap out-of-bounds write in xt_compat_target_from_user

VulnerabilityDescription:
    netfilter x_tables heap out-of-bounds write in xt_compat_target_from_user - Local Privilege Escalation

ImpactVersion: 2.6.19 <= kernel < 5.12.0
CVSS: 7.8 (HIGH)
exploitationCondition: CONFIG_NETFILTER_XTABLES Enable

detectionMethod（全程Read-only）:
    1. KernelVersionRangeMatch
    2. CONFIG_NETFILTER_XTABLES Configuration check
...
class CVE202122555Detector(BaseDetector):
    """CVE-2021-22555 (netfilter x_tables heap out-of-bounds write in xt_) Detector"""

    cve_id = "CVE-2021-22555"
    cvss_score = 7.8
    severity = "HIGH"
    description = (
        "netfilter x_tables heap out-of-bounds write in xt_compat_target_from_user - "
        "Local Privilege Escalation"
    )
    vuln_type = "local_privilege_escalation"
    affected_versions = {
        "generic": {"min": "2.6.19", "fixed": "5.12.0"},
    }
    ...
    def detect(self, kernel_info: KernelInfo) -> DetectResult:
        ...
        if config_value:
            evidence_list.append(Evidence(
                type="config_enabled",
                description=f"{self._CONFIG_KEY}={config_value} - x_tables enabled",
    ...
    def get_remediation(self) -> Remediation:
        return Remediation(
            ...
            steps=[
                ...
                "",
                "# 3. Verify fix",
                "uname -r  # Confirm new kernel version >= 5.12.0",
            ],
            backup_steps=[
            ...
            temporary_mitigation="disable x_tables module",
            ...
            verify_steps=[
                "uname -r  # Confirm kernel version >= 5.12.0",
            ]
        )
    ...
    def _get_poc_verifier(self):
        from ..poc.cve_2021_22555_poc import CVE202122555PoCVerifier
        return CVE202122555PoCVerifier()

    def prepare(self, kernel_info: KernelInfo) -> PrepareResult:
        from .cve_2021_22555_prepare import CVE202122555PrepareHandler
        handler = CVE202122555PrepareHandler(timeout=30)
        return handler.execute()

    def post(self, kernel_info: KernelInfo,
             prepare_result: Optional[PrepareResult] = None) -> PostResult:
        from .cve_2021_22555_post import CVE202122555PostHandler
        handler = CVE202122555PostHandler(
        ...
        if not force_run:
            ...
            if not config_enabled and kernel_info.config is not None:
                return PoCResult(
                    ...
                    error_message="CONFIG_NETFILTER_XTABLES not enabled"
                )
