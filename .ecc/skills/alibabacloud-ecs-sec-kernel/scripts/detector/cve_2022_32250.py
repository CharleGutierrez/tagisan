"""
CVE-2022-32250 Detector - nf_tables use-after-free via incorrect NFT_STATEFUL_EXPR check

VulnerabilityDescription:
    nf_tables use-after-free via incorrect NFT_STATEFUL_EXPR check - Local Privilege Escalation

ImpactVersion: 4.1.0 <= kernel < 5.18.2
CVSS: 7.8 (HIGH)
exploitationCondition: CONFIG_NF_TABLES Enable

detectionMethod（全程Read-only）:
    1. KernelVersionRangeMatch
    2. CONFIG_NF_TABLES Configuration check
...
class CVE202232250Detector(BaseDetector):
    """CVE-2022-32250 (nf_tables use-after-free via incorrect NFT_STATEFU) Detector"""

    cve_id = "CVE-2022-32250"
    cvss_score = 7.8
    severity = "HIGH"
    description = (
        "nf_tables use-after-free via incorrect NFT_STATEFUL_EXPR check - "
        "Local Privilege Escalation"
    )
    vuln_type = "local_privilege_escalation"
    affected_versions = {
        "generic": {"min": "4.1.0", "fixed": "5.18.2"},
    }

    has_poc = True
    has_exp = False
    poc_bin = "poc-bin/cve_2022_32250.bin"
    poc_mode = "uaf"

    _MODULE_NAME = "nf_tables"
    _CONFIG_KEY = "CONFIG_NF_TABLES"
    _MIN_VERSION = "4.1.0"
    _FIXED_VERSION = "5.18.2"
    ...
    def detect(self, kernel_info: KernelInfo) -> DetectResult:
        ...
        if config_value:
            evidence_list.append(Evidence(
                type="config_enabled",
                description=f"{self._CONFIG_KEY}={config_value} - nf_tables enabled",
    ...
    def get_remediation(self) -> Remediation:
        return Remediation(
            ...
            steps=[
                ...
                "",
                "# 3. Verify fix",
                "uname -r  # Confirm new kernel version >= 5.18.2",
            ],
            backup_steps=[
            ...
            temporary_mitigation="disable nf_tables module",
            ...
            verify_steps=[
                "uname -r  # Confirm kernel version >= 5.18.2",
            ]
        )
    ...
    def _get_poc_verifier(self):
        from ..poc.cve_2022_32250_poc import CVE202232250PoCVerifier
        return CVE202232250PoCVerifier()

    def prepare(self, kernel_info: KernelInfo) -> PrepareResult:
        from .cve_2022_32250_prepare import CVE202232250PrepareHandler
        handler = CVE202232250PrepareHandler(timeout=30)
        return handler.execute()

    def post(self, kernel_info: KernelInfo,
             prepare_result: Optional[PrepareResult] = None) -> PostResult:
        from .cve_2022_32250_post import CVE202232250PostHandler
        handler = CVE202232250PostHandler(
        ...
        if not force_run:
            ...
            if not config_enabled and kernel_info.config is not None:
                return PoCResult(
                    ...
                    error_message="CONFIG_NF_TABLES not enabled"
                )
