"""Detector for CVE-2023-4622

af_unix socket race condition use-after-free. Race between unix_stream_sendmsg and garbage collection leads to UAF of socket buffer, allowing local privilege escalation.
"""
import logging
...
class Cve20234622Detector(BaseDetector):
    cve_id = "CVE-2023-4622"
    severity = "CRITICAL"
    cvss_score = 7.8
    has_poc = True
    poc_mode = "uaf"
    description = "af_unix socket race condition use-after-free. Race between unix_stream_sendmsg and garbage collection leads to UAF of socket buffer, allowing local privilege escalation."
    ...
    def get_remediation(self):
        """Return remediation guidance for CVE-2023-4622."""
        from ..core.result import Remediation
        return Remediation(
            priority="IMMEDIATE",
            steps=[
                "Upgrade kernel to version >= 6.6 (AF_UNIX garbage collection UAF fix).",
    ...
    def _get_poc_verifier(self):
        from ..poc.cve_2023_4622_poc import Cve20234622PoCVerifier
        return Cve20234622PoCVerifier()
