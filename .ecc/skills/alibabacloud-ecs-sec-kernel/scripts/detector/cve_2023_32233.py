"""
CVE-2023-32233 Detector - nf_tables use-after-free in nft_set batch operations

VulnerabilityDescription:
    nf_tables use-after-free in nft_set batch operations - Local Privilege Escalation

ImpactVersion: 5.9.0 <= kernel < 6.4.0
CVSS: 7.8 (HIGH)
...
class CVE202332233Detector(BaseDetector):
    """CVE-2023-32233 (nf_tables use-after-free in nft_set batch operatio) Detector"""

    cve_id = "CVE-2023-32233"
    cvss_score = 7.8
    severity = "HIGH"
    description = (
        "nf_tables use-after-free in nft_set batch operations - "
        "Local Privilege Escalation"
    )
    vuln_type = "local_privilege_escalation"
    affected_versions = {
        "generic": {"min": "5.9.0", "fixed": "6.4.0"},
    }
    ...
    def get_remediation(self) -> Remediation:
        return Remediation(
            ...
            steps=[
                ...
                "",
                "# 3. Verify fix",
                "uname -r  # Confirm new kernel version >= 6.4.0",
            ],
            backup_steps=[
            ...
            verify_steps=[
                "uname -r  # Confirm kernel version >= 6.4.0",
            ]
        )
    ...
    def _get_poc_verifier(self):
        from ..poc.cve_2023_32233_poc import CVE202332233PoCVerifier
        return CVE202332233PoCVerifier()

    def prepare(self, kernel_info: KernelInfo) -> PrepareResult:
        from .cve_2023_32233_prepare import CVE202332233PrepareHandler
        handler = CVE202332233PrepareHandler(timeout=30)
        return handler.execute()

    def post(self, kernel_info: KernelInfo,
             prepare_result: Optional[PrepareResult] = None) -> PostResult:
        from .cve_2023_32233_post import CVE202332233PostHandler
        handler = CVE202332233PostHandler(
