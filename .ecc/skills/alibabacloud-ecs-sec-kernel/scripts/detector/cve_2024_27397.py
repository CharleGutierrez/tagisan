"""Detector for CVE-2024-27397 - netfilter nft_set timer race condition use-after-free. Race between set element timeout and concurrent set element operations leads to UAF, enabling local privilege escalation."""
import logging
...
class CVE202427397Detector(BaseDetector):
    cve_id = "CVE-2024-27397"
    severity = "CRITICAL"
    cvss_score = 7.8
    has_poc = True
    poc_mode = "uaf"
    description = "netfilter nft_set timer race condition use-after-free. Race between set element timeout and concurrent set element operations leads to UAF, enabling local privilege escalation."
    ...
    def get_remediation(self):
        """Return remediation guidance for CVE-2024-27397."""
        from ..core.result import Remediation
        return Remediation(
            priority="IMMEDIATE",
            steps=[
                "Upgrade kernel to version >= 6.9 (nf_tables gc transaction race fix).",
    ...
    def _get_poc_verifier(self):
        from ..poc.cve_2024_27397_poc import CVECve202427397PoCVerifier
        return CVECve202427397PoCVerifier()
