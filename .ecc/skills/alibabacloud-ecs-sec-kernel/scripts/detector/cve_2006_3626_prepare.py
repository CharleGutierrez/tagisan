"""
CVE-2006-3626 Prepare phase handler

Create root-owned target file for CTF challenge.
CVE-2006-3626 is a procfs /proc/self/environ race condition vulnerability.

For CTF verification:
- read_root_file: PoC attempts to read the root file via environ race
- write_root_file: Unsupported (this vulnerability is read-only)

The target file is owned by root:root with mode 0400 (nobody cannot access).
If the PoC manages to read the content via the procfs race, it proves the
...
class CVE20063626PrepareHandler(BasePrepareHandler):
    """CVE-2006-3626 Prepare phase: create root-owned CTF target"""
    ...
    def get_operations(self, kernel_info: dict) -> List[PrepareOperation]:
        ...
        # For read_root_file: 0400 root:root - PoC must bypass VFS to read
    ...
    def _create_target_file(self, op: PrepareOperation) -> PrepareOperation:
        ...
        CVE-2006-3626 is a read-only vulnerability (environ race condition).
        Only read_root_file mode is supported.

        File permissions:
        - read mode (0400): nobody cannot access - PoC must bypass VFS via
          the procfs race condition to read the content
        """
        target_path = op.target
        try:
            # CVE-2006-3626 only supports read_root_file mode
            if self._test_mode == "read_root_file":
                # Write ctf_value as flag content for PoC to read out
                content = self._ctf_value
                file_mode = 0o400  # nobody cannot access at all - real CTF
            else:
                # write_root_file is unsupported, but create file anyway
    ...
    def _snapshot_system_state(self, op: PrepareOperation) -> PrepareOperation:
        ...
        # environ accessibility
        environ_ok = self._check_environ_accessible()
        snapshot_parts.append(f"environ_accessible={environ_ok}")
