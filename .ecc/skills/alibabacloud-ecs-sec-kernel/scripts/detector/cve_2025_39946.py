"""Detector for CVE-2025-39946 - TLS stream abort on bogus headers. Malformed TLS record headers trigger
improper stream abort handling leading to use-after-free of socket buffer,
...
class CVE202539946Detector(BaseDetector):
    cve_id = "CVE-2025-39946"
    severity = "CRITICAL"
    cvss_score = 7.8
    has_poc = True
    poc_mode = "uaf"
    description = "TLS stream abort on bogus headers. Malformed TLS record headers trigger improper stream abort handling leading to use-after-free of socket buffer, enabling local privilege escalation."
    ...
    def get_remediation(self):
        """Return remediation guidance for CVE-2025-39946."""
        from ..core.result import Remediation
        return Remediation(
            priority="IMMEDIATE",
            steps=[
                "Upgrade kernel to version >= 6.15 (netfilter conntrack hash resize race fix).",
    ...
    def _get_poc_verifier(self):
        from ..poc.cve_2025_39946_poc import CVECve202539946PoCVerifier
        return CVECve202539946PoCVerifier()
