"""Detector for CVE-2026-43284 - xfrm-ESP Page Cache Pollution vulnerability.

Allows unprivileged users to overwrite root-owned file page cache via
XFRM SA + ESP-in-UDP + ESN seq_hi field manipulation.
"""
import logging
...
class CVE202643284Detector(BaseDetector):
    cve_id = "CVE-2026-43284"
    severity = "CRITICAL"
    cvss_score = 7.8
    has_poc = True
    poc_mode = "write_root_file"
    description = "xfrm-ESP Page Cache Pollution vulnerability allowing unprivileged users to overwrite root-owned file page cache via XFRM SA + ESP-in-UDP + ESN seq_hi field manipulation."
    ...
    def get_remediation(self):
        """Return remediation guidance for CVE-2026-43284."""
        from ..core.result import Remediation
        return Remediation(
            priority="IMMEDIATE",
            steps=[
                "Upgrade kernel to latest stable version with xfrm-ESP Page Cache Pollution fix.",
    ...
    def _get_poc_verifier(self):
        from ..poc.cve_2026_43284_poc import CVECve202643284PoCVerifier
        return CVECve202643284PoCVerifier()
