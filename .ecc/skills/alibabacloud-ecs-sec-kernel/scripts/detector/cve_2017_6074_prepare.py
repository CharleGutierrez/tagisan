"""
CVE-2017-6074 Prepare phase handler

Create root-owned target file for CTF challenge.
CVE-2017-6074 is a DCCP double-free vulnerability in dccp_rcv_state_process
leading to heap corruption and local privilege escalation.

For CTF verification:
- write_root_file: PoC attempts to use DCCP double-free + heap corruption
  to bypass VFS write permissions via corrupted socket state
...
If the PoC manages to modify content via the DCCP corruption path, it proves
...
class CVE20176074PrepareHandler(BasePrepareHandler):
    """CVE-2017-6074 Prepare phase: create root-owned CTF target"""
    ...
    def _create_target_file(self, op: PrepareOperation) -> PrepareOperation:
        ...
        CVE-2017-6074 is a DCCP double-free vulnerability.
    ...
    def _snapshot_system_state(self, op: PrepareOperation) -> PrepareOperation:
        """Record system state snapshot (kernel version, DCCP module status)"""
        snapshot_parts = []
        ...
        try:
            ...
            if result.returncode == 0:
                dccp_loaded = "dccp" in result.stdout
                snapshot_parts.append(f"dccp_module={'loaded' if dccp_loaded else 'not_loaded'}")
            else:
                snapshot_parts.append("dccp_module=unknown")
        except (subprocess.TimeoutExpired, OSError):
            snapshot_parts.append("dccp_module=unknown")
