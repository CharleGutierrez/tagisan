"""Detector for CVE-2024-0582 - io_uring PBUF_RING mmap use-after-free. When a provided buffer ring is unmapped while still registered, subsequent io_uring operations access freed pages, enabling local privilege escalation."""
import logging
...
class CVE20240582Detector(BaseDetector):
    cve_id = "CVE-2024-0582"
    severity = "CRITICAL"
    cvss_score = 7.8
    has_poc = True
    poc_mode = "uaf"
    description = "io_uring PBUF_RING mmap use-after-free. When a provided buffer ring is unmapped while still registered, subsequent io_uring operations access freed pages, enabling local privilege escalation."
    ...
    def get_remediation(self):
        """Return remediation guidance for CVE-2024-0582."""
        from ..core.result import Remediation
        return Remediation(
            priority="IMMEDIATE",
            steps=[
                "Upgrade kernel to version >= 6.8 (io_uring mapped buffer UAF fix).",
    ...
    def _get_poc_verifier(self):
        from ..poc.cve_2024_0582_poc import CVECve20240582PoCVerifier
        return CVECve20240582PoCVerifier()
