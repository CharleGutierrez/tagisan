"""
CVE-2020-8835 Detector - eBPF verifier improper register bounds calculation allowing arbitrary read/write

VulnerabilityDescription:
    eBPF verifier improper register bounds calculation allowing arbitrary read/write - Local Privilege Escalation

ImpactVersion: 5.5.0 <= kernel < 5.6.0
CVSS: 7.8 (HIGH)
...
class CVE20208835Detector(BaseDetector):
    """CVE-2020-8835 (eBPF verifier improper register bounds calculation) Detector"""

    cve_id = "CVE-2020-8835"
    cvss_score = 7.8
    severity = "HIGH"
    description = (
        "eBPF verifier improper register bounds calculation allowing arbitrary read/write - "
        "Local Privilege Escalation"
    )
    vuln_type = "local_privilege_escalation"
    affected_versions = {
        "generic": {"min": "5.5.0", "fixed": "5.6.0"},
    }

    has_poc = True
    has_exp = False
    poc_bin = "poc-bin/cve_2020_8835.bin"
    poc_mode = "uaf"

    _MODULE_NAME = "bpf"
    _CONFIG_KEY = "CONFIG_BPF_SYSCALL"
    _MIN_VERSION = "5.5.0"
    _FIXED_VERSION = "5.6.0"
    ...
    def get_remediation(self) -> Remediation:
        return Remediation(
            ...
            steps=[
                ...
                "",
                "# 3. Verify fix",
                "uname -r  # Confirm new kernel version >= 5.6.0",
            ],
            backup_steps=[
            ...
            verify_steps=[
                "uname -r  # Confirm kernel version >= 5.6.0",
            ]
        )
    ...
    def _get_poc_verifier(self):
        from ..poc.cve_2020_8835_poc import CVE20208835PoCVerifier
        return CVE20208835PoCVerifier()

    def prepare(self, kernel_info: KernelInfo) -> PrepareResult:
        from .cve_2020_8835_prepare import CVE20208835PrepareHandler
        handler = CVE20208835PrepareHandler(timeout=30)
        return handler.execute()

    def post(self, kernel_info: KernelInfo,
             prepare_result: Optional[PrepareResult] = None) -> PostResult:
        from .cve_2020_8835_post import CVE20208835PostHandler
        handler = CVE20208835PostHandler(
