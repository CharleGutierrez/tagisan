"""Detector for CVE-2024-26824 - Crypto algif_hash bogus SGL free on zero-length input. Improper handling
of zero-length scatter-gather list in hash algorithm interface leads to
...
class CVE202426824Detector(BaseDetector):
    cve_id = "CVE-2024-26824"
    severity = "CRITICAL"
    cvss_score = 7.8
    has_poc = True
    poc_mode = "uaf"
    description = "Crypto algif_hash bogus SGL free on zero-length input. Improper handling of zero-length scatter-gather list in hash algorithm interface leads to memory corruption and potential privilege escalation."
    ...
    def get_remediation(self):
        """Return remediation guidance for CVE-2024-26824."""
        from ..core.result import Remediation
        return Remediation(
            priority="IMMEDIATE",
            steps=[
                "Upgrade kernel to version >= 6.8 (crypto/algif_hash wait_for_data UAF fix).",
    ...
    def _get_poc_verifier(self):
        from ..poc.cve_2024_26824_poc import CVECve202426824PoCVerifier
        return CVECve202426824PoCVerifier()
