"""
CVE-2016-5195 Prepare phase handler

Create root-owned target file for CTF challenge.
CVE-2016-5195 (Dirty COW) is a page cache pollution vulnerability.

For CTF verification:
- write_root_file: PoC attempts to write to root file via COW race condition
...
If the PoC manages to write content via Dirty COW page cache pollution, it proves the
...
class CVE20165195PrepareHandler(BasePrepareHandler):
    """CVE-2016-5195 Prepare phase: create root-owned CTF target"""
    ...
    def _create_target_file(self, op: PrepareOperation) -> PrepareOperation:
        ...
        via COW race condition). Only write_root_file mode is supported.

        File permissions:
        - write mode (0644): nobody can read but not write - PoC must bypass VFS via
          the Dirty COW race condition to write content
        - File must be >= 4096 bytes (one page) for mmap operations to succeed
          and for the race to have a full page to corrupt
        """
