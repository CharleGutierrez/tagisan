"""Detector for CVE-2024-39503 - Netfilter ipset race between namespace cleanup and garbage collection
in list:set type. Concurrent GC and netns destruction leads to UAF
of set elements, enabling local privilege escalation.
"""
import logging
...
class CVE202439503Detector(BaseDetector):
    cve_id = "CVE-2024-39503"
    severity = "CRITICAL"
    cvss_score = 7.8
    has_poc = True
    poc_mode = "uaf"
    description = "Netfilter ipset race between namespace cleanup and garbage collection in list:set type. Concurrent GC and netns destruction leads to UAF of set elements, enabling local privilege escalation."
    ...
    def get_remediation(self):
        """Return remediation guidance for CVE-2024-39503."""
        from ..core.result import Remediation
        return Remediation(
            priority="IMMEDIATE",
            steps=[
                "Upgrade kernel to version >= 6.10 (nf_tables chain binding deactivation fix).",
    ...
    def _get_poc_verifier(self):
        from ..poc.cve_2024_39503_poc import CVECve202439503PoCVerifier
        return CVECve202439503PoCVerifier()
