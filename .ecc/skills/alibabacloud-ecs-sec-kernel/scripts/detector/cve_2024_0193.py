"""
CVE-2024-0193 Detector - nf_tables double-free in nft_trans_gc_catchall

VulnerabilityDescription:
    nf_tables double-free in nft_trans_gc_catchall - Local Privilege Escalation

ImpactVersion: 5.14.0 <= kernel < 6.7.0
CVSS: 7.8 (HIGH)
...
class CVE20240193Detector(BaseDetector):
    """CVE-2024-0193 (nf_tables double-free in nft_trans_gc_catchall) Detector"""

    cve_id = "CVE-2024-0193"
    cvss_score = 7.8
    severity = "HIGH"
    description = (
        "nf_tables double-free in nft_trans_gc_catchall - "
        "Local Privilege Escalation"
    )
    vuln_type = "local_privilege_escalation"
    affected_versions = {
        "generic": {"min": "5.14.0", "fixed": "6.7.0"},
    }
    ...
    def get_remediation(self) -> Remediation:
        return Remediation(
            ...
            steps=[
                ...
                "",
                "# 3. Verify fix",
                "uname -r  # Confirm new kernel version >= 6.7.0",
            ],
            backup_steps=[
            ...
            verify_steps=[
                "uname -r  # Confirm kernel version >= 6.7.0",
            ]
        )
    ...
    def _get_poc_verifier(self):
        from ..poc.cve_2024_0193_poc import CVE20240193PoCVerifier
        return CVE20240193PoCVerifier()

    def prepare(self, kernel_info: KernelInfo) -> PrepareResult:
        from .cve_2024_0193_prepare import CVE20240193PrepareHandler
        handler = CVE20240193PrepareHandler(timeout=30)
        return handler.execute()

    def post(self, kernel_info: KernelInfo,
             prepare_result: Optional[PrepareResult] = None) -> PostResult:
        from .cve_2024_0193_post import CVE20240193PostHandler
        handler = CVE20240193PostHandler(
