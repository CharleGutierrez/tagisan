"""Detector for CVE-2023-4147

nf_tables chain insertion use-after-free. When adding a rule to a bound chain that fails, the chain is freed but the rule still references it, leading to UAF and local privilege escalation.
"""
import logging
...
class Cve20234147Detector(BaseDetector):
    cve_id = "CVE-2023-4147"
    severity = "CRITICAL"
    cvss_score = 7.8
    has_poc = True
    poc_mode = "uaf"
    description = "nf_tables chain insertion use-after-free. When adding a rule to a bound chain that fails, the chain is freed but the rule still references it, leading to UAF and local privilege escalation."
    ...
    def get_remediation(self):
        """Return remediation guidance for CVE-2023-4147."""
        from ..core.result import Remediation
        return Remediation(
            priority="IMMEDIATE",
            steps=[
                "Upgrade kernel to version >= 6.5 (nf_tables rule addition race condition fix).",
    ...
    def _get_poc_verifier(self):
        from ..poc.cve_2023_4147_poc import Cve20234147PoCVerifier
        return Cve20234147PoCVerifier()
