"""
CVE-2010-3904 Post phase handler

Cleans up PoC execution residue, verifies CTF results, and verifies system state is restored.
"""
import glob
import logging
import os
...
class CVE20103904PostHandler(BasePostHandler):
    """CVE-2010-3904 Post phase: clean environment and verify system recovery"""

    def verify_ctf(self, context, poc_output) -> bool:
        """Verify CTF challenge result

        For CVE-2010-3904 (read_root_file mode):
        - PoC output flag {} value should match file actual content {} value

        Args:
            context: CTF context with mode, flag, target_file, write_value
            poc_output: PoC output string

        Returns:
            True if CTF verification passed
        """
        if context.mode == "read_root_file":
            # Extract CTF flag value from PoC output
            flag_match = re.search(r'ctf\{([^}]+)\}', poc_output or "")
            if not flag_match:
                logger.warning("No CTF flag found in PoC output")
                return False

            poc_flag_value = flag_match.group(1)
            ...
            try:
                ...
                # Extract flag from file content
                file_flag_match = re.search(r'ctf\{([^}]+)\}', file_content)
                if not file_flag_match:
                    logger.warning("No CTF flag found in target file")
                    return False

                file_flag_value = file_flag_match.group(1)

                # Compare flag values
                if poc_flag_value == file_flag_value:
                    logger.info("CTF flag matched: %s", poc_flag_value)
                    return True
                else:
                    logger.warning(
                        "CTF flag mismatch: PoC=%s, File=%s",
                        poc_flag_value, file_flag_value
                    )
                    return False
            ...
            # CVE-2010-3904 doesn't support write mode
            logger.warning("write_root_file mode not supported for CVE-2010-3904")
            return False
