"""
CVE-2015-1328 Prepare phase handler

Create root-owned target file for CTF challenge.
CVE-2015-1328 (OverlayFS) is a permission bypass vulnerability.

For CTF verification:
- write_root_file: PoC attempts to write to root file via overlayfs userns bypass
...
If the PoC manages to write content via overlayfs user namespace permission bypass,
...
class CVE20151328PrepareHandler(BasePrepareHandler):
    """CVE-2015-1328 Prepare phase: create root-owned CTF target"""
    ...
    def _create_target_file(self, op: PrepareOperation) -> PrepareOperation:
        ...
        bypass via user namespace). Only write_root_file mode is supported.

        File permissions:
        - write mode (0644): nobody can read but not write - PoC must bypass VFS via
          overlayfs user namespace mount to write content
        - File must be >= 4096 bytes (one page) for operations to succeed
        """
        ...
        try:
            ...
            if st.st_size < 4096:
                ...
                op.error = (
                    ...
                    "(overlayfs operations require sufficient space)"
                )
                return op
    ...
    def _snapshot_system_state(self, op: PrepareOperation) -> PrepareOperation:
        """Record system state snapshot (kernel version, overlay status)"""
        snapshot_parts = []
        ...
        try:
            with open("/proc/filesystems", "r", encoding="utf-8") as f:
                content = f.read()
                overlay_available = "overlay" in content
                snapshot_parts.append(f"overlayfs={'yes' if overlay_available else 'no'}")
        except OSError:
            snapshot_parts.append("overlayfs=unknown")
