"""
CVE-2014-3153 Prepare phase handler

Create root-owned target file for CTF challenge.
CVE-2014-3153 (Towelroot) is a futex_requeue race condition vulnerability.

For CTF verification:
- write_root_file: PoC attempts to write to root file via futex race
- read_root_file: PoC attempts to read root file via futex race

The target file is owned by root:root.
If the PoC manages to write/read content via futex_requeue race condition,
...
class CVE20143153PrepareHandler(BasePrepareHandler):
    """CVE-2014-3153 Prepare phase: create root-owned CTF target"""
    ...
    def _create_target_file(self, op: PrepareOperation) -> PrepareOperation:
        ...
        CVE-2014-3153 is a futex_requeue race condition that can potentially
        enable both read and write bypass of VFS permission checks.

        File permissions:
        - write mode (0644): nobody can read but not write - PoC must bypass via futex race
        - read mode (0400): nobody completely unable to access - PoC must bypass to read
        - File must be >= 4096 bytes (one page) for mmap operations to succeed
        """
        ...
        try:
            if self._test_mode == "write_root_file":
                ...
                # read_root_file: nobody completely unable to access
                self._initial_content = f"ctf_{secrets.token_hex(8)}"
                file_mode = 0o400

            # Pad to at least one page (4096 bytes) for mmap operations
            # Futex race condition may require target file >= page size
            ...
            if st.st_size < 4096:
                ...
                op.error = (
                    ...
                    "(futex race may require full page)"
                )
                return op
    ...
    def _snapshot_system_state(self, op: PrepareOperation) -> PrepareOperation:
        """Record system state snapshot (kernel version, etc.)"""
        snapshot_parts = []
