"""
CVE-2014-3153 Post phase handler
...
class CVE20143153PostHandler(BasePostHandler):
    """CVE-2014-3153 Post phase: Clean up environment and verify system state"""
    ...
    def self_check(self, run_user: str) -> SelfCheckResult:
        ...
        # 2. Check for residual futex related processes
    ...
    def verify_ctf(self, context, poc_output) -> bool:
        ...
        if context.mode == "write_root_file":
            ...
            # Verify that PoC output flag matches the CTF flag
    ...
    def global_verify(self, prepare_result: PrepareResult) -> GlobalVerifyResult:
        ...
        # 2. System already restored (futex is core subsystem, no need special restore)
        system_restored = True

        # 3. Check dmesg for futex related anomalies
        kernel_warnings = self._check_futex_kernel_log()
    ...
    def _check_futex_kernel_log(self) -> List[str]:
        """Check dmesg for futex related anomalies"""
        warnings: List[str] = []
        ...
        try:
            ...
            for line in tail_lines:
                # Check for futex related anomalies
