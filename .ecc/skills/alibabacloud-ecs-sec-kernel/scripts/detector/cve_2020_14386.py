"""
CVE-2020-14386 Detector - AF_PACKET net/packet/af_packet.c signedness bug in tpacket_rcv

VulnerabilityDescription:
    AF_PACKET net/packet/af_packet.c signedness bug in tpacket_rcv - Local Privilege Escalation

ImpactVersion: 4.6.0 <= kernel < 5.9.0
CVSS: 7.8 (HIGH)
...
class CVE202014386Detector(BaseDetector):
    """CVE-2020-14386 (AF_PACKET net/packet/af_packet.c signedness bug in) Detector"""

    cve_id = "CVE-2020-14386"
    cvss_score = 7.8
    severity = "HIGH"
    description = (
        "AF_PACKET net/packet/af_packet.c signedness bug in tpacket_rcv - "
        "Local Privilege Escalation"
    )
    vuln_type = "local_privilege_escalation"
    affected_versions = {
        "generic": {"min": "4.6.0", "fixed": "5.9.0"},
    }

    has_poc = True
    has_exp = False
    poc_bin = "poc-bin/cve_2020_14386.bin"
    poc_mode = "uaf"

    _MODULE_NAME = "af_packet"
    _CONFIG_KEY = "CONFIG_PACKET"
    _MIN_VERSION = "4.6.0"
    _FIXED_VERSION = "5.9.0"
    ...
    def get_remediation(self) -> Remediation:
        return Remediation(
            ...
            steps=[
                ...
                "",
                "# 3. Verify fix",
                "uname -r  # Confirm new kernel version >= 5.9.0",
            ],
            backup_steps=[
            ...
            temporary_mitigation="disable af_packet module",
            ...
            verify_steps=[
                "uname -r  # Confirm kernel version >= 5.9.0",
            ]
        )
    ...
    def _get_poc_verifier(self):
        from ..poc.cve_2020_14386_poc import CVE202014386PoCVerifier
        return CVE202014386PoCVerifier()

    def prepare(self, kernel_info: KernelInfo) -> PrepareResult:
        from .cve_2020_14386_prepare import CVE202014386PrepareHandler
        handler = CVE202014386PrepareHandler(timeout=30)
        return handler.execute()

    def post(self, kernel_info: KernelInfo,
             prepare_result: Optional[PrepareResult] = None) -> PostResult:
        from .cve_2020_14386_post import CVE202014386PostHandler
        handler = CVE202014386PostHandler(
