"""
CVE-2018-18955 PoC Orchestration
...
class CVE201818955PoCVerifier(BasePoCVerifier):
    """CVE-2018-18955 PoC Orchestrator"""
    ...
    def get_prepare_handler(self):
        """Return CVE-2018-18955 dedicated Prepare handler"""
        from ..detector.cve_2018_18955_prepare import CVE201818955PrepareHandler
        return CVE201818955PrepareHandler()

    def get_post_handler(self):
        """Return CVE-2018-18955 dedicated Post handler"""
        from ..detector.cve_2018_18955_post import CVE201818955PostHandler
        return CVE201818955PostHandler()

    def pre_check(self, kernel_info: KernelInfo) -> Optional[PoCResult]:
        """
        Pre-check:
        1. Kernel version is in affected range
        2. CONFIG_USER_NS Configuration enabled
        """
        # Version check
        ...
        if not config_enabled and kernel_info.config is not None:
            return PoCResult(
                ...
                error_message=(
                    ...
                    f"user namespace subsystem not available"
                )
            )
    ...
    def post_process(self, result: PoCResult,
        ...
        if result.status == "EXPLOITABLE":
            logger.info(
                "CVE-2018-18955 EXPLOITABLE on kernel %s",
                kernel_info.version
            )
        return result
