"""
CVE-2026-43284 PoC Orchestration
...
class CVECve202643284PoCVerifier(BasePoCVerifier):
    """CVE-2026-43284 PoC Orchestrator"""

    cve_id = "CVE-2026-43284"
    poc_bin = "poc-bin/cve_2026_43284.bin"
    timeout = 30
    ...
    def get_prepare_handler(self):
        """Return CVE-2026-43284 dedicated Prepare handler"""
        from ..detector.cve_2026_43284_prepare import CVE202643284PrepareHandler
        return CVE202643284PrepareHandler()

    def get_post_handler(self):
        """Return CVE-2026-43284 dedicated Post handler"""
        from ..detector.cve_2026_43284_post import CVE202643284PostHandler
        return CVE202643284PostHandler()
    ...
    def prepare(self, kernel_info: KernelInfo,
        ...
        """
        from ..detector.cve_2026_43284_prepare import CVE202643284PrepareHandler
        handler = CVE202643284PrepareHandler(
    ...
    def post(self, kernel_info: KernelInfo,
        ...
        handler = CVE202643284PostHandler(
    ...
    def post_process(self, result: PoCResult,
        ...
        if result.status == "EXPLOITABLE":
            logger.info(
                "CVE-2026-43284 EXPLOITABLE on kernel %s",
                kernel_info.version
            )
            result.description = (
                f"xfrm-ESP page cache pollution exploitable on "
