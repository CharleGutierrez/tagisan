"""
CVE-2022-0847 PoC orchestration
...
class CVE20220847PoCVerifier(BasePoCVerifier):
    """CVE-2022-0847 PoC Orchestrator"""
    ...
    def get_prepare_handler(self):
        """Return CVE-2022-0847 dedicated Prepare handler"""
        from ..detector.cve_2022_0847_prepare import CVE20220847PrepareHandler
        return CVE20220847PrepareHandler()

    def get_post_handler(self):
        """Return CVE-2022-0847 dedicated Post handler"""
        from ..detector.cve_2022_0847_post import CVE20220847PostHandler
        return CVE20220847PostHandler()

    def pre_check(self, kernel_info: KernelInfo) -> Optional[PoCResult]:
        """
        Pre-check:
        1. Kernel version in affected range
        2. CONFIG_PIPE Configuration enabled
        """
        # Version check
        ...
        if not config_enabled and kernel_info.config is not None:
            return PoCResult(
                ...
                error_message=(
                    ...
                    f"pipe subsystem not available"
                )
            )
    ...
    def post_process(self, result: PoCResult,
        ...
        if result.status == "EXPLOITABLE":
            logger.info(
                "CVE-2022-0847 EXPLOITABLE on kernel %s",
                kernel_info.version
            )
        return result
