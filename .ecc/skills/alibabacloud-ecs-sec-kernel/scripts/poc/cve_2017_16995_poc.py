"""
CVE-2017-16995 PoC orchestration
...
class CVE201716995PoCVerifier(BasePoCVerifier):
    """CVE-2017-16995 PoC Orchestrator"""
    ...
    def get_prepare_handler(self):
        """Return CVE-2017-16995 dedicated Prepare handler"""
        from ..detector.cve_2017_16995_prepare import CVE201716995PrepareHandler
        # test_mode and ctf_value will be set by the orchestrator
        return CVE201716995PrepareHandler(
            test_mode=self._test_mode if hasattr(self, '_test_mode') else "write_root_file",
            ctf_value=self._ctf_value if hasattr(self, '_ctf_value') else ""
        )

    def get_post_handler(self):
        """Return CVE-2017-16995 dedicated Post handler"""
        from ..detector.cve_2017_16995_post import CVE201716995PostHandler
        return CVE201716995PostHandler()

    def pre_check(self, kernel_info: KernelInfo) -> Optional[PoCResult]:
        """
        Pre-check:
        1. Kernel version in affected range
        2. CONFIG_BPF_SYSCALL Configuration enabled
        """
        # Version check
        ...
        if not config_enabled and kernel_info.config is not None:
            return PoCResult(
                ...
                error_message=(
                    ...
                    f"eBPF subsystem not available"
                )
            )
    ...
    def post_process(self, result: PoCResult,
        ...
        if result.status == "EXPLOITABLE":
            logger.info(
                "CVE-2017-16995 EXPLOITABLE on kernel %s",
                kernel_info.version
            )
        return result
