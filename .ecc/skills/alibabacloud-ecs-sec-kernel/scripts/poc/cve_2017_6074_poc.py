"""
CVE-2017-6074 PoC orchestration
...
class CVE20176074PoCVerifier(BasePoCVerifier):
    """CVE-2017-6074 PoC Orchestrator"""
    ...
    def get_prepare_handler(self):
        """Return CVE-2017-6074 dedicated Prepare handler"""
        from ..detector.cve_2017_6074_prepare import CVE20176074PrepareHandler
        return CVE20176074PrepareHandler()

    def get_post_handler(self):
        """Return CVE-2017-6074 dedicated Post handler"""
        from ..detector.cve_2017_6074_post import CVE20176074PostHandler
        return CVE20176074PostHandler()

    def pre_check(self, kernel_info: KernelInfo) -> Optional[PoCResult]:
        """
        Pre-check:
        1. Kernel version in affected range
        2. CONFIG_IP_DCCP Configuration enabled
        """
        # Version check
        ...
        if not config_enabled and kernel_info.config is not None:
            return PoCResult(
                ...
                error_message=(
                    ...
                    f"DCCP subsystem not available"
                )
            )
    ...
    def post_process(self, result: PoCResult,
        ...
        if result.status == "EXPLOITABLE":
            logger.info(
                "CVE-2017-6074 EXPLOITABLE on kernel %s",
                kernel_info.version
            )
        return result
