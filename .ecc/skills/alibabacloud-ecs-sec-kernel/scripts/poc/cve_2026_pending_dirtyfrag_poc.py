"""
CVE-2026-PENDING-DIRTYFRAG PoC Orchestration
...
The PoC (uid=nobody) attempts to overwrite it via xfrm-ESP splice page cache
...
class CVE2026PendingDirtyFragPoCVerifier(BasePoCVerifier):
    """CVE-2026-PENDING-DIRTYFRAG PoC Orchestrator"""
    ...
    _ESP_MIN_VERSION = "4.10.0"  # cac2661c53f3 (2017-01-17)
    _ESP_MODULES = ["esp4", "esp6"]
    _ESP_CONFIG = "CONFIG_XFRM"

    # RxRPC variant constants (secondary check)
    _RXRPC_MIN_VERSION = "6.4.0"  # 2dc334f1a63a (2023-06)
    _RXRPC_MODULE = "rxrpc"
    ...
    def get_prepare_handler(self):
        """Return CVE-2026-PENDING-DIRTYFRAG dedicated Prepare handler"""
        from ..detector.cve_2026_pending_dirtyfrag_prepare import CVE2026PendingDirtyFragPrepareHandler
        return CVE2026PendingDirtyFragPrepareHandler()

    def get_post_handler(self):
        """Return CVE-2026-PENDING-DIRTYFRAG dedicated Post handler"""
        from ..detector.cve_2026_pending_dirtyfrag_post import CVE2026PendingDirtyFragPostHandler
        return CVE2026PendingDirtyFragPostHandler()
    ...
    def pre_check(self, kernel_info: KernelInfo) -> Optional[PoCResult]:
        ...
        if not esp_version_ok:
            # Check RxRPC as fallback
            rxrpc_version_ok = version_in_range(
                kernel_info.version, self._RXRPC_MIN_VERSION, fixed_version=None
            )
            if not rxrpc_version_ok:
                return PoCResult(
                    ...
                    error_message=(
                        ...
                        f"RxRPC requires >= {self._RXRPC_MIN_VERSION}"
                    )
                )
        ...
        if not esp_modules_loaded and not esp_config_enabled:
            ...
            rxrpc_config = check_config_enabled(
                self._RXRPC_CONFIG, kernel_info.config
            )
            if not rxrpc_loaded and not rxrpc_config:
                return PoCResult(
                    status="NOT_EXPLOITABLE",
                    confidence=0.8,
                    error_message=(
                        ...
                        f"RxRPC module not loaded/configured"
                    )
                )
    ...
    def post_process(self, result: PoCResult,
        ...
        if result.status == "EXPLOITABLE":
            logger.info(
                "CVE-2026-PENDING-DIRTYFRAG EXPLOITABLE on kernel %s",
                kernel_info.version
            )
            result.description = (
                f"DirtyFrag xfrm-ESP page cache pollution exploitable on "
                f"kernel {kernel_info.version}. "
                f"Page cache write verified via CTF target file."
            )
        elif result.status == "NOT_EXPLOITABLE":
            result.description = (
                f"DirtyFrag not exploitable on kernel {kernel_info.version}. "
