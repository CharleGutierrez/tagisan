"""
CVE-2020-14386 PoC orchestration
...
class CVE202014386PoCVerifier(BasePoCVerifier):
    """CVE-2020-14386 PoC Orchestrator"""
    ...
    def get_prepare_handler(self):
        """Return CVE-2020-14386 dedicated Prepare handler"""
        from ..detector.cve_2020_14386_prepare import CVE202014386PrepareHandler
        return CVE202014386PrepareHandler()

    def get_post_handler(self):
        """Return CVE-2020-14386 dedicated Post handler"""
        from ..detector.cve_2020_14386_post import CVE202014386PostHandler
        return CVE202014386PostHandler()

    def pre_check(self, kernel_info: KernelInfo) -> Optional[PoCResult]:
        """
        Pre-check:
        1. Kernel version in affected range
        2. CONFIG_PACKET Configuration enabled
        """
        # Version check
        ...
        if not config_enabled and kernel_info.config is not None:
            return PoCResult(
                ...
                error_message=(
                    ...
                    f"AF_PACKET subsystem not available"
                )
            )
    ...
    def post_process(self, result: PoCResult,
        ...
        if result.status == "EXPLOITABLE":
            logger.info(
                "CVE-2020-14386 EXPLOITABLE on kernel %s",
                kernel_info.version
            )
        return result
