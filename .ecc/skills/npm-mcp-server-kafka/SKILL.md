---
name: npm-mcp-server-kafka
description: "Apache Kafka topic offset monitoring, message production, and partition audit"
version: 1.0.0
...
triggers:
  ...
  - "npm-mcp-server-kafka"
  - "npm-mcp-server-kafka execute"
  - "npm-mcp-server-kafka verify"
  - "server"
---

# Npm Mcp Server Kafka Skill

The `npm-mcp-server-kafka` skill equips Tagisan (`tgs`) autonomous agents and developers with authoritative capabilities sourced from **NPM Registry** (Tier 1: Model Context Protocol (MCP) & Web Registries). It integrates directly into `MCP stdio client / ToolRegistry`.
