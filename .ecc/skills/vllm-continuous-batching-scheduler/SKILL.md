---
name: vllm-continuous-batching-scheduler
description: "Iteration-level continuous request scheduling maximizing GPU compute utilization"
version: 1.0.0
...
triggers:
  ...
  - "cuda"
  - "gguf"
  - "vllm continuous batching scheduler"
  - "vllm-continuous-batching-scheduler"
  - "vllm-continuous-batching-scheduler execute"
  - "vllm-continuous-batching-scheduler verify"
---

# Vllm Continuous Batching Scheduler Skill

The `vllm-continuous-batching-scheduler` skill equips Tagisan (`tgs`) autonomous agents and developers with authoritative capabilities sourced from **vLLM Documentation** (Tier 5: Local Tensor Engines, Quantization & GGUF Acceleration). It integrates directly into `tgs serve Daemon`.
...
```json
{
  "skill": "vllm-continuous-batching-scheduler",
  "tier": "Tier 5: Local Tensor Engines, Quantization & GGUF Acceleration",
  "source": "vLLM Documentation",
