...
description: 'Use when building, reviewing, or troubleshooting a custom Lightning Web Component
  that runs inside a Flow screen element, covering @api props exposed to Flow, FlowAttributeChangeEvent
  for output, validate() for user input validation, and flow navigation events. Triggers:
  ''lwc in flow screen'', ''FlowAttributeChangeEvent'', ''flow screen component not
  updating'', ''flow validate method'', ''flow navigation from lwc''. NOT for custom
  property editors (use custom-property-editor-for-flow), NOT for embedding a flow
  inside an LWC (use flow/screen-flows), NOT for auto-launched flows.'
---
