---
name: vllm-paged-attention-kv-cache
description: "Virtual memory block allocation for KV cache eliminating memory fragmentation"
version: 1.0.0
...
triggers:
  ...
  - "vllm-paged-attention-kv-cache"
  - "vllm-paged-attention-kv-cache execute"
  - "vllm-paged-attention-kv-cache verify"
---

# Vllm Paged Attention Kv Cache Skill

The `vllm-paged-attention-kv-cache` skill equips Tagisan (`tgs`) autonomous agents and developers with authoritative capabilities sourced from **vLLM Documentation** (Tier 5: Local Tensor Engines, Quantization & GGUF Acceleration). It integrates directly into `Colibrì Inference Engine`.
...
- **Subsystem Binding**: `Colibrì Inference Engine`
...
```json
{
  "skill": "vllm-paged-attention-kv-cache",
  "tier": "Tier 5: Local Tensor Engines, Quantization & GGUF Acceleration",
  "source": "vLLM Documentation",
  "integration_target": "Colibrì Inference Engine",
