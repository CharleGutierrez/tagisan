...
description: "Use when creating, reviewing, or troubleshooting Prompt Builder templates (Field\
  \ Generation, Record Summary, Sales Email, or Flex types), including grounding with\
  \ merge fields, Flow, or Apex. Trigger keywords: prompt template, Prompt Builder,\
  \ field generation, record summary, sales email template, flex template, grounding,\
  \ merge fields, LLM template, Einstein generative AI. NOT for versioning, promoting\
  \ or rolling back a template \u2014 use agentforce/prompt-template-versioning. NOT\
  \ for Data Cloud vector-search RAG pipelines that ground an agent (chunking, embeddings,\
  \ retrievers) \u2014 use agentforce/rag-patterns-in-salesforce."
---
