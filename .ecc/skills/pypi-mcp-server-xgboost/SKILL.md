---
name: pypi-mcp-server-xgboost
description: "Gradient boosted decision tree training, inference, and feature importance"
version: 1.0.0
...
triggers:
  ...
  - "model-context-protocol"
  - "pypi"
  - "pypi mcp server xgboost"
  - "pypi-mcp-server-xgboost"
  - "pypi-mcp-server-xgboost execute"
  - "pypi-mcp-server-xgboost verify"
  - "server"
  - "tools"
---

# Pypi Mcp Server Xgboost Skill

The `pypi-mcp-server-xgboost` skill equips Tagisan (`tgs`) autonomous agents and developers with authoritative capabilities sourced from **PyPI** (Tier 1: Model Context Protocol (MCP) & Web Registries). It integrates directly into `MCP stdio client / ToolRegistry`.
