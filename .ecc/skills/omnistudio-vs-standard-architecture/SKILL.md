...
description: "Pick and then document the OmniStudio runtime path an architect has to live with:\
  \ Standard Runtime (natively embedded, Spring '25+, no namespace) versus the legacy\
  \ vlocity_ins__ / industries__ managed package and the migration debt every new\
  \ component on it accrues \u2014 gated first on whether the org actually holds a\
  \ qualifying Industries license. Ends in a written ADR that records the license\
  \ gate, the runtime path, team ramp cost, and the fallback to Screen Flow + LWC\
  \ for a guided wizard when OmniStudio is not available. NOT for the OmniStudio-vs-standard-tooling\
  \ capability matrix itself (use architect/omnistudio-vs-standard-decision), NOT\
  \ for OmniScript, FlexCard, or Integration Procedure implementation."
---
