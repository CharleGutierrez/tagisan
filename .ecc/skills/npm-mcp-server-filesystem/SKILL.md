---
name: npm-mcp-server-filesystem
description: "Controlled sandboxed filesystem read/write with path restriction"
version: 1.0.0
...
triggers:
  ...
  - "npm-mcp-server-filesystem"
  - "npm-mcp-server-filesystem execute"
  - "npm-mcp-server-filesystem verify"
  - "server"
---

# Npm Mcp Server Filesystem Skill

The `npm-mcp-server-filesystem` skill equips Tagisan (`tgs`) autonomous agents and developers with authoritative capabilities sourced from **NPM Registry** (Tier 1: Model Context Protocol (MCP) & Web Registries). It integrates directly into `MCP stdio client / ToolRegistry`.
...
```json
{
  "skill": "npm-mcp-server-filesystem",
  "tier": "Tier 1: Model Context Protocol (MCP) & Web Registries",
  "source": "NPM Registry",
