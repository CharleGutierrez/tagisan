...
description: "Use when planning, coordinating, or governing Salesforce releases: version numbering,\
  \ rollback strategy, release notes, go/no-go criteria, release calendar, and sandbox\
  \ preview alignment. NOT for Salesforce's own seasonal platform release (Spring/Summer/Winter)\
  \ readiness \u2014 release notes triage, Release Updates, Sandbox Preview opt-in\
  \ (use admin/salesforce-release-preparation); this skill governs YOUR release train,\
  \ not Salesforce's. NOT for deployment mechanics (use devops/post-deployment-validation\
  \ or devops/change-set-deployment)."
---
# Release Management
